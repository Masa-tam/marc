#!/usr/bin/env python3
"""Run the fixed AVL/Red-Black/Scapegoat Silesia experiment."""
...
RESULT_SCHEMA = "marc-silesia-scapegoat-tree-experiment-v1"
CHECKPOINT_SCHEMA = "marc-silesia-scapegoat-tree-checkpoint-v1"
FRAME_SIZE = 1_048_576
WINDOWS = (65_536, 262_144, 1_048_576)
ITERATIONS = 1
...
TOOL_SOURCES = (
    "run_silesia_scapegoat_tree_experiment.py",
...
def _command(
    ...
    return [
        str(benchmark), "--frames", strategy, str(member),
...
def _validate_report(
    ...
    if _require_integer(report, WORKSPACE_KEYS[strategy]) < 0:
        raise RunnerError("negative strategy workspace")
...
def _index_records(
    ...
    for record_index, record in enumerate(checkpoint["records"]):
        ...
        if member is None or strategy not in STRATEGIES \
                or window_size not in WINDOWS:
        ...
        _validate_report(report, strategy, member_path.stat().st_size,
                         window_size)
        key = (member.name, window_size, strategy)
        if key != expected_grid[record_index]:
    ...
    for member in manifest:
        for window_size in WINDOWS:
            baseline = records.get((member.name, window_size, STRATEGIES[0]))
            for strategy in STRATEGIES[1:]:
                candidate = records.get((member.name, window_size, strategy))
                if candidate is not None and baseline is None:
                    raise RunnerError(
                        f"checkpoint {strategy} record has no AVL baseline"
                    )
...
def _comparisons(aggregates: Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for window_size in WINDOWS:
        ...
        scapegoat = indexed[(STRATEGIES[2], window_size)]
        result.append({
            "window_bytes": window_size,
            "red_black_to_avl_throughput_ratio": _ratio(
                red_black["mib_per_second"], avl["mib_per_second"],
            ),
            "scapegoat_to_avl_throughput_ratio": _ratio(
                scapegoat["mib_per_second"], avl["mib_per_second"],
            ),
            "red_black_to_avl_workspace_ratio": _ratio(
                red_black["maximum_workspace_bytes"],
                avl["maximum_workspace_bytes"],
            ),
            "scapegoat_to_avl_workspace_ratio": _ratio(
                scapegoat["maximum_workspace_bytes"],
                avl["maximum_workspace_bytes"],
            ),
            "token_count": avl["token_count"],
            "matched_bytes": avl["matched_bytes"],
            "matched_byte_coverage": _ratio(
                avl["matched_bytes"], avl["input_bytes"],
            ),
            ...
            "scapegoat_maximum_final_height": (
                scapegoat["scapegoat_tree_maximum_final_height"]
            ),
            "scapegoat_maximum_structural_nodes_per_update": (
                scapegoat[
...
def main(arguments: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Verify local Silesia data and run the fixed 1-MiB-frame AVL/"
            "Red-Black/Scapegoat comparison; performs no network access."
        )
    )
            ...
            for window_size in WINDOWS:
                for strategy in STRATEGIES:
                    ...
                    if strategy != STRATEGIES[0]:
                        ...
                        if baseline is None:
                            raise RunnerError(
                                f"{strategy} has no AVL baseline"
                            )
                        _require_exact_pair(
