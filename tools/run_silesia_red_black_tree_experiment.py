#!/usr/bin/env python3
"""Run the fixed AVL/Red-Black Silesia experiment without network access."""
...
RESULT_SCHEMA = "marc-silesia-red-black-tree-experiment-v1"
CHECKPOINT_SCHEMA = "marc-silesia-red-black-tree-checkpoint-v1"
...
TOOL_SOURCES = (
    "run_silesia_red_black_tree_experiment.py",
...
def _require_exact_pair(
    avl_report: dict[str, Any], red_black_report: dict[str, Any],
    member_name: str, window_size: int,
) -> None:
    for key in SUMMARY_KEYS:
        if avl_report.get(key) != red_black_report.get(key):
            raise RunnerError(
...
def _index_records(
    ...
    for member in manifest:
        for window_size in WINDOWS:
            avl_record = records.get((member.name, window_size, STRATEGIES[0]))
            red_black_record = records.get(
                (member.name, window_size, STRATEGIES[1])
            )
            if red_black_record is not None and avl_record is None:
                raise RunnerError("checkpoint Red-Black record has no AVL baseline")
            if avl_record is not None and red_black_record is not None:
                _require_exact_pair(
                    avl_record["report"], red_black_record["report"],
...
def main(arguments: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Red-Black comparison; performs no network access."
        )
    )
            ...
            for window_size in WINDOWS:
                for strategy in STRATEGIES:
                    ...
                    if strategy == STRATEGIES[1]:
                        ...
                        if baseline is None:
                            raise RunnerError("Red-Black has no AVL baseline")
                        _require_exact_pair(
