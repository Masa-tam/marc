#!/usr/bin/env python3
"""Run the fixed 64-MiB global BinaryTree/HashChain Silesia experiment."""
...
RESULT_SCHEMA = "marc-silesia-binary-tree-64m-experiment-v1"
CHECKPOINT_SCHEMA = "marc-silesia-binary-tree-64m-checkpoint-v1"
FRAME_SIZE = 67_108_864
WINDOWS = (16_777_216, 67_108_864)
ITERATIONS = 1
...
TOOL_SOURCES = (
    "run_silesia_binary_tree_64m_experiment.py",
...
def _index_records(
    ...
    for index, record in enumerate(checkpoint["records"]):
        ...
        member_name, expected_window, expected_strategy = expected_grid[index]
        member = members[member_name]
        report = record["report"]
        key = (record.get("member"), report.get("window_bytes"),
               report.get("strategy"))
        if key != (member_name, expected_window, expected_strategy):
            raise RunnerError("checkpoint records are not a canonical prefix")
        member_path = corpus / member.name
        expected_command = _command(
            benchmark, member_path, expected_strategy, expected_window,
        )
        ...
        _validate_report(
            report, expected_strategy, member_path.stat().st_size,
            expected_window,
        )
        records[key] = record
        if expected_strategy == STRATEGIES[1]:
            baseline = records[(member_name, expected_window, STRATEGIES[0])]
            _require_exact_pair(
                baseline["report"], report, member_name, expected_window,
            )
    return records
...
def _comparisons(
    aggregates: Sequence[dict[str, Any]], records: Sequence[dict[str, Any]],
) -> list[dict[str, Any]]:
    indexed = {
    ...
    for window_size in WINDOWS:
        ...
        members = sorted({key[0] for key in reports if key[1] == window_size})
        wins = sum(
            reports[(member, window_size, STRATEGIES[1])][TIME_KEYS[STRATEGIES[1]]]
            < reports[(member, window_size, STRATEGIES[0])][TIME_KEYS[STRATEGIES[0]]]
...
def _window_progression(comparisons: Sequence[dict[str, Any]]) -> dict[str, Any]:
    first, second = comparisons
    first_tokens = first["token_count"]
    return {
        "from_window_bytes": first["window_bytes"],
        "to_window_bytes": second["window_bytes"],
        "token_count_reduction": first_tokens - second["token_count"],
        "token_count_reduction_fraction": (
            (first_tokens - second["token_count"]) / first_tokens
            if first_tokens != 0 else 0.0
        ),
        "matched_byte_coverage_change": (
            second["matched_byte_coverage"] - first["matched_byte_coverage"]
        ),
    }
...
def main(arguments: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description=(
        "Verify local Silesia data and run the fixed 64-MiB global "
    ...
    try:
        ...
        if parsed.max_new_points is not None:
            return finish_batch()
        ordered_records = [records[key] for key in _grid(manifest)]
        aggregates = _aggregate_with_summary(ordered_records)
        comparisons = _comparisons(aggregates, ordered_records)
        result = {
            "schema": RESULT_SCHEMA,
            ...
            "window_progression": _window_progression(comparisons),
        }
