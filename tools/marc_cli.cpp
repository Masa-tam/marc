...
constexpr std::uint64_t lzss_contextual_adaptive_huffman_payload_size =
    (lzss_contextual_adaptive_huffman_frame_size * UINT64_C(267)
...
bool configure(
    ...
    config.max_block_size = lzss_contextual_tans_decision_count;
    config.max_compressed_payload_size = lzss_contextual_tans_payload_size;
    config.max_internal_buffered_bytes = lzss_contextual_tans_buffered_size;
    config.max_lz_distance = UINT64_C(1) << 16;
    config.max_lz_match_length = 258;
    return true;
}
...
bool configure(
    ...
    config.max_block_size = lzss_contextual_blocked_huffman_decision_count;
    config.max_compressed_payload_size =
...
bool configure(
    ...
    config.max_compressed_payload_size =
        lzss_contextual_adaptive_huffman_payload_size;
    config.max_internal_buffered_bytes =
...
bool process_file(const marc_direction direction,
    ...
    if (codec == Codec::checksum_raw) {
        ...
        if (!configure(direction, source_size, lzss_contextual_range_config))
            return false;
