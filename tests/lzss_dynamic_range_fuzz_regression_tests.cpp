...
TEST(LzssDynamicRangeFuzzRegression, EveryCanonicalTruncationIsAtomic) {
    const auto encoded = canonical_stream();
    for (std::size_t size = 0; size < encoded.size(); ++size)
        expect_atomic_failure(std::span<const std::byte>{encoded}.first(size));
}

TEST(LzssDynamicRangeFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lzss_dynamic_range_stream_prefix_size;
...
TEST(LzssDynamicRangeFuzzRegression, InvalidDescriptorIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lzss_dynamic_range_stream_prefix_size + frame_header_size
