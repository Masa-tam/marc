...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lz77_dynamic_range_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        const auto plan = plan_lz77_dynamic_range_frame(
            stream, {}, limits, sequence, committed, raw, staging);
        EXPECT_EQ(plan.error, Lz77DynamicRangeFrameValidationError::none);
        const auto offset = output.size();
        output.resize(offset + plan.serialized_size);
        EXPECT_EQ(encode_lz77_dynamic_range_frame(
                      stream, {}, limits, sequence, committed, raw, staging,
...
TEST(Lz77DynamicRangeFrameStreamingEncoder,
...
TEST(Lz77DynamicRangeFrameStreamingEncoder,
    ...
    EXPECT_EQ(first.output_produced, lz77_dynamic_range_stream_prefix_size);
...
TEST(Lz77DynamicRangeFrameStreamingEncoder,
    ...
    Lz77DynamicRangeFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    Lz77DynamicRangeFrameStreamingEncoder short_dictionary{
    ...
    Lz77DynamicRangeFrameStreamingEncoder short_encoded{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
    ...
    Lz77DynamicRangeFrameStreamingEncoder aggregate_limited{
...
TEST(Lz77DynamicRangeFrameStreamingEncoder, HandlesEmptyAndProtocolErrors) {
    std::array<std::byte, 1> unused{};
    std::array<std::byte, lz77_dynamic_range_stream_prefix_size> output{};
    Lz77DynamicRangeFrameStreamingEncoder empty{
    ...
    EXPECT_EQ(result.output_produced, lz77_dynamic_range_stream_prefix_size);

    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 32> dictionary_staging{};
    std::array<std::byte, 2048> frame_encoded{};
    Lz77DynamicRangeFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded};
    ...
    Lz77DynamicRangeFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded};
