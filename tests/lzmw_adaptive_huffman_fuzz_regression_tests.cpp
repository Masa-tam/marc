...
using EncoderEntry = marc::dictionary::internal::LzmwEncoderEntry;
...
[[nodiscard]] std::vector<std::byte> canonical_stream() {
    ...
    std::array<std::byte, 4 * raw.size()> dictionary{};
...
TEST(LzmwAdaptiveHuffmanFuzzRegression, EveryCanonicalTruncationIsAtomic) {
...
TEST(LzmwAdaptiveHuffmanFuzzRegression, ExtremeFrameLengthsAreAtomic) {
...
TEST(LzmwAdaptiveHuffmanFuzzRegression, InvalidDescriptorIsAtomic) {
    auto encoded = canonical_stream();
