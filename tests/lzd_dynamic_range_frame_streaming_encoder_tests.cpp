...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lzd_dynamic_range_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        const auto plan = plan_lzd_dynamic_range_frame(
            stream, {}, limits, sequence, committed, raw, entries, staging);
        EXPECT_EQ(plan.error, LzdDynamicRangeFrameValidationError::none);
        const auto offset = output.size();
        output.resize(offset + plan.serialized_size);
        EXPECT_EQ(encode_lzd_dynamic_range_frame(
...
TEST(LzdDynamicRangeFrameStreamingEncoder,
    ...
    std::array<marc::dictionary::internal::LzdEncoderEntry, 1> entries{};
    LzdDynamicRangeFrameStreamingEncoder encoder{
...
TEST(LzdDynamicRangeFrameStreamingEncoder,
    ...
    EXPECT_EQ(first.output_produced, lzd_dynamic_range_stream_prefix_size);
...
TEST(LzdDynamicRangeFrameStreamingEncoder,
    ...
    LzdDynamicRangeFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    LzdDynamicRangeFrameStreamingEncoder short_dictionary{
        config(input.size()), {}, {}, frame_input,
        std::span<std::byte>{staging}.first<7>(), frame_encoded, entries};
    EXPECT_EQ(short_dictionary.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzdDynamicRangeFrameStreamingEncoder short_entries{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded, {}};
    EXPECT_EQ(short_entries.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);

    LzdDynamicRangeFrameStreamingEncoder short_encoded{
    ...
    ASSERT_EQ(plan.error, LzdDynamicRangeFrameValidationError::none);
    ...
    limits.max_internal_buffered_bytes = 2 + plan.dictionary_size
        ...
        - 1;
    LzdDynamicRangeFrameStreamingEncoder aggregate_limited{
...
TEST(LzdDynamicRangeFrameStreamingEncoder,
    ...
    std::array<std::byte, lzd_dynamic_range_stream_prefix_size> output{};
    LzdDynamicRangeFrameStreamingEncoder empty{
        config(0), {}, {}, {}, {}, unused, {}};
    auto result = empty.process(
        {}, output,
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
    EXPECT_EQ(result.status, marc::core::StreamStatus::end_of_stream);
    EXPECT_EQ(result.output_produced, lzd_dynamic_range_stream_prefix_size);
    ...
    std::array<std::byte, 2048> frame_encoded{};
    std::array<marc::dictionary::internal::LzdEncoderEntry, 1> entries{};
    LzdDynamicRangeFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
    ...
    LzdDynamicRangeFrameStreamingEncoder excess{
        config(1), {}, {}, frame_input, staging, frame_encoded, entries};
    result = excess.process(input, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::invalid_argument);

    LzdDynamicRangeFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
    ...
    LzdDynamicRangeFrameStreamingEncoder unknown{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
