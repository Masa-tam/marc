...
constexpr std::size_t maximum_payload_size = test_frame_size * 2 + 5;
constexpr std::size_t descriptor_size = 16;
...
marc_dynamic_range_config config(
    ...
    marc_dynamic_range_config result{};
    EXPECT_EQ(marc_dynamic_range_config_init(direction, &result),
              MARC_STATUS_OK);
    ...
    result.max_range_model_total = UINT64_C(1) << 15;
    result.max_internal_buffered_bytes =
        descriptor_size + maximum_payload_size;
    return result;
}

Workspace workspace_for(const marc_dynamic_range_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_dynamic_range_workspace_requirements(
...
Transform create(const marc_dynamic_range_config& settings,
                 Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_dynamic_range_create(
                  &settings,
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 64 + input.size() * 2
        + frames * (56 + descriptor_size + 5);
...
TEST(DynamicRangeCompletion, RequiredDataClassesRoundTripDeterministically) {
    expect_round_trip({});
...
TEST(DynamicRangeCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(DynamicRangeCompletion, MalformedFinalFrameIsNeverCommitted) {
