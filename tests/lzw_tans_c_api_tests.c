...
static void set_small_limits(marc_lzw_tans_config* config) {
...
int main(void) {
    ...
    assert(marc_lzw_tans_config_init(MARC_DIRECTION_ENCODE, &config)
           == MARC_STATUS_OK);
    ...
    assert(marc_lzw_tans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 2);
    assert(needed.secondary_bytes == 1126);
    ...
    assert(marc_lzw_tans_config_init(MARC_DIRECTION_DECODE, &config)
           == MARC_STATUS_OK);
    set_small_limits(&config);
    assert(marc_lzw_tans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
