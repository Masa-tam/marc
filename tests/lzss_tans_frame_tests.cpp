...
TEST(LzssTansFramePlanner, PlansExactHandVectorExtent) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, literal_a_token.size()> staging{};
    const auto result = marc::frame::plan_lzss_tans_frame(
...
TEST(LzssTansFramePlanner, PlansBlockThatSplitsLiteral) {
    constexpr std::array raw{std::byte{'A'}};
    constexpr std::uint32_t block_size = 1;
    std::array<std::byte, literal_a_token.size()> staging{};
    const auto result = marc::frame::plan_lzss_tans_frame(
    ...
    EXPECT_EQ(result.payload_size, 2U * 2U);
    EXPECT_EQ(result.serialized_size, 56U + 2U * 528U + 2U * 2U);
    EXPECT_EQ(staging, literal_a_token);
}

TEST(LzssTansFramePlanner, PlansGeneratedMatchDeterministically) {
    ...
    const auto first_plan = marc::frame::plan_lzss_tans_frame(
        stream_for(raw.size(), block_size), {}, {}, 0, 0, raw, first);
    const auto second_plan = marc::frame::plan_lzss_tans_frame(
        stream_for(raw.size(), block_size), {}, {}, 0, 0, raw, second);
    ASSERT_EQ(first_plan.error, LzssTansFrameValidationError::none);
    ASSERT_EQ(second_plan.error, LzssTansFrameValidationError::none);
...
TEST(LzssTansFramePlanner, ShortStagingFailsBeforeMutation) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, literal_a_token.size() - 1> staging{};
    staging.fill(std::byte{0xa5});
    const auto result = marc::frame::plan_lzss_tans_frame(
        stream_for(1), {}, {}, 0, 0, raw, staging);
    EXPECT_EQ(result.error, LzssTansFrameValidationError::
                                dictionary_staging_too_small);
    EXPECT_EQ(result.dictionary_size, literal_a_token.size());
    EXPECT_EQ(staging[0], std::byte{0xa5});
}

TEST(LzssTansFramePlanner, RejectsEmptyAndUnexpectedFrameExtent) {
    std::array<std::byte, 4> staging{};
    EXPECT_EQ(marc::frame::plan_lzss_tans_frame(
                  stream_for(1), {}, {}, 0, 0,
                  std::span<const std::byte>{}, staging).error,
              LzssTansFrameValidationError::input_size_mismatch);
...
TEST(LzssTansFramePlanner, EnforcesBlockCountAndWorkspaceBounds) {
    constexpr std::array raw{std::byte{'A'}};
    constexpr std::uint32_t block_size = 1;
    std::array<std::byte, literal_a_token.size()> staging{};
    ...
    auto result = marc::frame::plan_lzss_tans_frame(
        stream_for(1, block_size), {}, limits, 0, 0, raw, staging);
    EXPECT_EQ(result.error,
              LzssTansFrameValidationError::entropy_encode_error);
    EXPECT_EQ(result.entropy_encode_error,
              marc::entropy::internal::TansEncodeError::limit_exceeded);

    limits.max_blocks_per_frame = 2;
    limits.max_internal_buffered_bytes =
        2 * 528 + 2 * 2 + literal_a_token.size() - 1;
    result = marc::frame::plan_lzss_tans_frame(
        stream_for(1, block_size), {}, limits, 0, 0, raw, staging);
    EXPECT_EQ(result.error, LzssTansFrameValidationError::workspace_limit);
}

TEST(LzssTansFrameEncoder, EmitsExactIndependentHandVector) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, literal_a_token.size()> staging{};
    std::array<std::byte, 587> output{};
    const auto result = marc::frame::encode_lzss_tans_frame(
...
TEST(LzssTansFrameEncoder,
    ...
    constexpr std::uint32_t block_size = 1;
    std::array<std::byte, literal_a_token.size()> staging{};
    const auto plan = marc::frame::plan_lzss_tans_frame(
        stream_for(1, block_size), {}, {}, 0, 0, raw, staging);
...
TEST(LzssTansFrameEncoder, ShortSerializedOutputIsAtomic) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, literal_a_token.size()> staging{};
