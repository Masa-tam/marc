...
TEST(LzdRansFrameEncoder, EmitsExactIndependentVector) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, terminal_token_a.size()> staging{};
    std::array<std::byte, 593> output{};
    const auto result = marc::frame::encode_lzd_rans_frame(
...
TEST(LzdRansFrameEncoder, RoundTripsPhraseBlocksDeterministically) {
    constexpr std::array raw{
        std::byte{'A'}, std::byte{'B'}, std::byte{'A'},
        std::byte{'B'}, std::byte{'A'}, std::byte{'B'}};
    constexpr std::uint32_t block_size = 5;
    ...
    const auto plan = marc::frame::plan_lzd_rans_frame(
    ...
    ASSERT_EQ(marc::frame::encode_lzd_rans_frame(
                  stream, {}, {}, 0, 0, raw, workspace, encode_staging,
                  first).error,
...
TEST(LzdRansFrameEncoder, ShortOutputIsCompletelyUnchanged) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, terminal_token_a.size()> staging{};
    std::array<std::byte, 592> output{};
    output.fill(std::byte{0xa5});
    EXPECT_EQ(marc::frame::encode_lzd_rans_frame(
                  stream_for(raw.size()), {}, {}, 0, 0, raw, {}, staging,
                  output).error,
    ...
    std::array<std::byte, 593> planner_failure_output{};
