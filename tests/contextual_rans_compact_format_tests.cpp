...
using marc::entropy::internal::contextual_rans_compact_max_descriptor_size;
...
[[nodiscard]] std::vector<std::byte> serialize(
    ...
    std::size_t written{};
    EXPECT_EQ(
        marc::entropy::internal::serialize_contextual_rans_compact_descriptor(
...
TEST(ContextualRansCompactFormat, SerializesAndParsesOneLiteralVector) {
    ...
    ASSERT_EQ(marc::entropy::internal::parse_contextual_rans_compact_descriptor(
                  bytes, 2, 8, {}, parsed),
              ContextualRansCompactFormatError::none);
    EXPECT_TRUE(descriptors_equal(parsed, descriptor));
}

TEST(ContextualRansCompactFormat, SelectsSparseStrictlyAndDenseOnTie) {
...
TEST(ContextualRansCompactFormat, EveryDenseModelReachesExactMaximum) {
    ContextualRansDescriptor descriptor{};
    ...
    const auto bytes = serialize(descriptor);
    ASSERT_EQ(bytes.size(), contextual_rans_compact_max_descriptor_size);
    ContextualRansDescriptor parsed{};
    ASSERT_EQ(marc::entropy::internal::parse_contextual_rans_compact_descriptor(
                  bytes, 1, 8, {}, parsed),
              ContextualRansCompactFormatError::none);
...
TEST(ContextualRansCompactFormat, RejectsEveryStrictPrefixAndTrailingData) {
    ...
    for (std::size_t extent = 0; extent < valid.size(); ++extent) {
        ...
        EXPECT_NE(
            marc::entropy::internal::parse_contextual_rans_compact_descriptor(
    ...
    sentinel.flags = 0xa5;
    const auto before = sentinel;
    EXPECT_EQ(marc::entropy::internal::parse_contextual_rans_compact_descriptor(
                  trailing, 2, 8, {}, sentinel),
              ContextualRansCompactFormatError::trailing_data);
    EXPECT_TRUE(descriptors_equal(sentinel, before));
}

TEST(ContextualRansCompactFormat, RejectsMasksModesAndNoncanonicalRecords) {
    const auto valid = serialize(literal_a_descriptor());
    auto expect_error = [&](std::vector<std::byte> bytes,
                            const ContextualRansCompactFormatError expected) {
        auto sentinel = literal_a_descriptor();
        sentinel.flags = 0xa5;
        const auto before = sentinel;
        EXPECT_EQ(
    ...
    expect_error(malformed, ContextualRansCompactFormatError::trailing_data);
    malformed = valid;
    malformed[20] = std::byte{0x02};
    expect_error(malformed, ContextualRansCompactFormatError::invalid_mode);
    malformed = valid;
...
TEST(ContextualRansCompactFormat, RejectsMalformedSparseModels) {
...
TEST(ContextualRansCompactFormat, EnforcesFieldsLimitsAndAtomicOutput) {
    auto descriptor = literal_a_descriptor();
    std::size_t size = 0xa5a5;
    descriptor.decision_count = 0;
    EXPECT_EQ(
        marc::entropy::internal::validate_contextual_rans_compact_descriptor(
            descriptor, 0, 8, {}, size),
              ContextualRansCompactFormatError::invalid_decision_count);
    ...
    EXPECT_EQ(
        marc::entropy::internal::validate_contextual_rans_compact_descriptor(
            descriptor, 2, 8, limits, size),
              ContextualRansCompactFormatError::limit_exceeded);
    limits = {};
    ...
    EXPECT_EQ(
        ...
        ContextualRansCompactFormatError::output_too_small);
    EXPECT_TRUE(std::ranges::all_of(
        output, [](const auto value) { return value == std::byte{0xa5}; }));
    EXPECT_EQ(written, 0xa5a5U);

    descriptor.flags = 1;
    std::array<std::byte, contextual_rans_compact_max_descriptor_size> large{};
    std::ranges::fill(large, std::byte{0xa5});
    EXPECT_EQ(
        marc::entropy::internal::serialize_contextual_rans_compact_descriptor(
            descriptor, 2, 8, {}, large, written),
        ContextualRansCompactFormatError::unknown_flags);
    EXPECT_TRUE(std::ranges::all_of(
