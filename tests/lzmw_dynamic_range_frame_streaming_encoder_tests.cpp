...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lzmw_dynamic_range_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        const auto plan = plan_lzmw_dynamic_range_frame(
        ...
        EXPECT_EQ(encode_lzmw_dynamic_range_frame(
...
TEST(LzmwDynamicRangeFrameStreamingEncoder,
    ...
    std::array<marc::dictionary::internal::LzmwEncoderEntry, 1> entries{};
    LzmwDynamicRangeFrameStreamingEncoder encoder{
...
TEST(LzmwDynamicRangeFrameStreamingEncoder,
    ...
    EXPECT_EQ(first.output_produced, lzmw_dynamic_range_stream_prefix_size);
...
TEST(LzmwDynamicRangeFrameStreamingEncoder,
    ...
    LzmwDynamicRangeFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    LzmwDynamicRangeFrameStreamingEncoder short_dictionary{
    ...
    EXPECT_EQ(short_dictionary.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzmwDynamicRangeFrameStreamingEncoder short_entries{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded, {}};
    EXPECT_EQ(short_entries.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);

    LzmwDynamicRangeFrameStreamingEncoder short_encoded{
    ...
    limits.max_internal_buffered_bytes = 2 + plan.dictionary_size
        ...
        - 1;
    LzmwDynamicRangeFrameStreamingEncoder aggregate_limited{
...
TEST(LzmwDynamicRangeFrameStreamingEncoder,
    ...
    std::array<std::byte, lzmw_dynamic_range_stream_prefix_size> output{};
    LzmwDynamicRangeFrameStreamingEncoder empty{
        config(0), {}, {}, {}, {}, unused, {}};
    auto result = empty.process(
        {}, output,
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
    EXPECT_EQ(result.status, marc::core::StreamStatus::end_of_stream);
    EXPECT_EQ(result.output_produced, lzmw_dynamic_range_stream_prefix_size);
    ...
    std::array<std::byte, 2048> frame_encoded{};
    std::array<marc::dictionary::internal::LzmwEncoderEntry, 1> entries{};
    LzmwDynamicRangeFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
    ...
    LzmwDynamicRangeFrameStreamingEncoder excess{
        config(1), {}, {}, frame_input, staging, frame_encoded, entries};
    result = excess.process(input, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::invalid_argument);

    LzmwDynamicRangeFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
    ...
    LzmwDynamicRangeFrameStreamingEncoder unknown{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
