...
[[nodiscard]] marc_lzss_dynamic_range_config config(
    const marc_direction direction, const std::size_t original_size) {
    marc_lzss_dynamic_range_config result{};
    EXPECT_EQ(marc_lzss_dynamic_range_config_init(direction, &result),
              MARC_STATUS_OK);
...
[[nodiscard]] Workspace workspace_for(
    const marc_lzss_dynamic_range_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lzss_dynamic_range_workspace_requirements(
...
[[nodiscard]] Transform create(
    ...
    Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lzss_dynamic_range_create(
                  &settings,
...
TEST(LzssDynamicRangeCompletion,
...
TEST(LzssDynamicRangeCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(LzssDynamicRangeCompletion, MalformedFinalFrameIsNeverCommitted) {
