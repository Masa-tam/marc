...
} elseif ($manifest.schema_version -eq 51) {
    if ([string]$manifest.codec_set -ne 'marc-cli-v51') {
        throw "Unsupported interoperability codec set: $($manifest.codec_set)"
    }
    $expectedProfiles = $schema51Profiles
} elseif ($manifest.schema_version -eq 52) {
    if ([string]$manifest.codec_set -ne 'marc-cli-v52') {
        throw "Unsupported interoperability codec set: $($manifest.codec_set)"
    }
    $expectedProfiles = $schema52Profiles
} elseif ($manifest.schema_version -eq 53) {
    if ([string]$manifest.codec_set -ne 'marc-cli-v53') {
        throw "Unsupported interoperability codec set: $($manifest.codec_set)"
    }
    $expectedProfiles = $schema53Profiles
} elseif ($manifest.schema_version -eq 54) {
    if ([string]$manifest.codec_set -ne 'marc-cli-v54') {
        throw "Unsupported interoperability codec set: $($manifest.codec_set)"
    }
    $expectedProfiles = $schema54Profiles
} elseif ($manifest.schema_version -eq 55) {
    if ([string]$manifest.codec_set -ne 'marc-cli-v55') {
        throw "Unsupported interoperability codec set: $($manifest.codec_set)"
    }
    $expectedProfiles = $schema55Profiles
} elseif ($manifest.schema_version -eq 56) {
    if ([string]$manifest.codec_set -ne 'marc-cli-v56') {
        throw "Unsupported interoperability codec set: $($manifest.codec_set)"
    }
    $expectedProfiles = $schema56Profiles
} elseif ($manifest.schema_version -eq 57) {
    if ([string]$manifest.codec_set -ne 'marc-cli-v57') {
        throw "Unsupported interoperability codec set: $($manifest.codec_set)"
    }
    $expectedProfiles = $schema57Profiles
} elseif ($manifest.schema_version -eq 58) {
    if ([string]$manifest.codec_set -ne 'marc-cli-v58') {
        throw "Unsupported interoperability codec set: $($manifest.codec_set)"
    }
