...
[[nodiscard]] std::vector<std::byte> encoded_stream(
    ...
    std::vector<std::byte> encoded(lzd_dynamic_range_stream_prefix_size);
...
[[nodiscard]] std::size_t first_frame_size(
    ...
    const std::span<const std::byte, frame_header_size> bytes{
        encoded.data() + lzd_dynamic_range_stream_prefix_size,
        frame_header_size};
...
TEST(LzdDynamicRangeFrameStreamingDecoder,
    ...
    LzdDynamicRangeFrameStreamingDecoder decoder{
...
TEST(LzdDynamicRangeFrameStreamingDecoder,
    ...
    const auto second = lzd_dynamic_range_stream_prefix_size
...
TEST(LzdDynamicRangeFrameStreamingDecoder,
    ...
    std::array<marc::dictionary::internal::LzdPhraseEntry, 1> phrases{};
    std::array<std::uint32_t, 2> expansion{};
    LzdDynamicRangeFrameStreamingDecoder trailing_decoder{
        {}, frame_encoded, staging, decoded, phrases, expansion};
    std::array<std::byte, raw.size()> output{};
    ...
    LzdDynamicRangeFrameStreamingDecoder empty_decoder{
        {}, frame_encoded, staging, decoded, phrases, expansion};
    const auto empty = empty_decoder.process(
        empty_encoded, {},
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
    EXPECT_EQ(empty.status, marc::core::StreamStatus::end_of_stream);
    EXPECT_EQ(empty.input_consumed, lzd_dynamic_range_stream_prefix_size);
}
...
TEST(LzdDynamicRangeFrameStreamingDecoder,
    ...
    LzdDynamicRangeFrameStreamingDecoder short_frame{
    ...
    EXPECT_EQ(short_raw.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzdDynamicRangeFrameStreamingDecoder short_phrases{
        {}, frame_encoded, staging, decoded, {}, expansion};
    EXPECT_EQ(short_phrases.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzdDynamicRangeFrameStreamingDecoder short_expansion{
        {}, frame_encoded, staging, decoded, phrases,
        std::span<std::uint32_t>{expansion}.first<1>()};
    ...
    LzdDynamicRangeFrameStreamingDecoder limited{
    ...
    LzdDynamicRangeFrameStreamingDecoder reset{
        {}, frame_encoded, staging, decoded, phrases, expansion};
    ...
    LzdDynamicRangeFrameStreamingDecoder unknown{
        {}, frame_encoded, staging, decoded, phrases, expansion};
