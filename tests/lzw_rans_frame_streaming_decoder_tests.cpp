...
TEST(LzwRansFrameStreamingDecoder, DecodesOneByteInputAndOutput) {
...
TEST(LzwRansFrameStreamingDecoder,
     CommitsOnlyFramesBeforeLaterCorruption) {
    auto encoded = encoded_stream();
    const auto first_size =
        frame_size_at(encoded, lzw_rans_stream_prefix_size);
    const auto second_offset = lzw_rans_stream_prefix_size + first_size;
...
TEST(LzwRansFrameStreamingDecoder, ReportsWorkspaceAndAggregateErrors) {
    const auto encoded = encoded_stream();
    const auto first_offset = lzw_rans_stream_prefix_size;
    ...
    LzwRansFrameStreamingDecoder short_encoded{
        {},
    ...
    LzwRansFrameStreamingDecoder short_views{
    ...
    LzwRansFrameStreamingDecoder short_dictionary{
        {}, workspace.frame_encoded, workspace.views,
    ...
    LzwRansFrameStreamingDecoder short_decoded{
        {}, workspace.frame_encoded, workspace.views,
    ...
    LzwRansFrameStreamingDecoder short_phrases{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
    ...
    limits.max_internal_buffered_bytes =
        ...
        - 1;
    LzwRansFrameStreamingDecoder aggregate_limited{
...
TEST(LzwRansFrameStreamingDecoder, RejectsTruncationTrailingAndReset) {
    const auto encoded = encoded_stream();
    Workspace workspace{};
    std::array<std::byte, raw.size()> output{};
    const auto end =
    ...
    LzwRansFrameStreamingDecoder truncated{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
    EXPECT_EQ(
    ...
    LzwRansFrameStreamingDecoder trailing{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
    ...
    LzwRansFrameStreamingDecoder reset{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
    EXPECT_EQ(
    ...
    LzwRansFrameStreamingDecoder unknown{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
...
TEST(LzwRansFrameStreamingDecoder, HandlesEmptyFlushAndPrematureEnd) {
    const auto empty_encoded = encoded_stream({});
    LzwRansFrameStreamingDecoder empty{{}, {}, {}, {}, {}, {}};
    EXPECT_EQ(
        empty
    ...
    Workspace workspace{};
    LzwRansFrameStreamingDecoder starved{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
    EXPECT_EQ(
    ...
    const auto prefix_and_first_frame =
        lzw_rans_stream_prefix_size + first_size;
    LzwRansFrameStreamingDecoder premature{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
    std::array<std::byte, 1> output{};
