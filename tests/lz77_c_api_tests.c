...
int main(void) {
    static const uint8_t input[] = {
        0x41, 0x42, 0x43, 0x41, 0x42, 0x43, 0x58};
    ...
    assert(marc_lz77_config_init(MARC_DIRECTION_ENCODE, &config)
           == MARC_STATUS_OK);
    assert(config.struct_size == sizeof(config));
    assert(config.window_size == 65536 && config.min_match_length == 3);
    config.original_size = sizeof(input);
    config.frame_size = 4;
    assert(marc_lz77_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    ...
    assert(marc_lz77_create(&config, primary, secondary, &transform)
           == MARC_STATUS_OK);
    ...
    assert(encoded_size > 80);
    marc_transform_destroy(transform);
    free(primary.data);
    free(secondary.data);

    assert(marc_lz77_config_init(MARC_DIRECTION_DECODE, &config)
           == MARC_STATUS_OK);
