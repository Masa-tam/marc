...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lzss_dynamic_range_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        EXPECT_EQ(plan.error, LzssDynamicRangeFrameValidationError::none);
        const auto offset = output.size();
        output.resize(offset + plan.serialized_size);
        EXPECT_EQ(encode_lzss_dynamic_range_frame(
                      stream, {}, limits, sequence, committed, raw, staging,
...
TEST(LzssDynamicRangeFrameStreamingEncoder,
    ...
    EXPECT_EQ(first.output_produced, lzss_dynamic_range_stream_prefix_size);
...
TEST(LzssDynamicRangeFrameStreamingEncoder,
    ...
    LzssDynamicRangeFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    LzssDynamicRangeFrameStreamingEncoder short_dictionary{
    ...
    LzssDynamicRangeFrameStreamingEncoder short_encoded{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
    ...
    LzssDynamicRangeFrameStreamingEncoder aggregate_limited{
...
TEST(LzssDynamicRangeFrameStreamingEncoder, HandlesEmptyAndProtocolErrors) {
    std::array<std::byte, 1> unused{};
    std::array<std::byte, lzss_dynamic_range_stream_prefix_size> output{};
    LzssDynamicRangeFrameStreamingEncoder empty{
    ...
    EXPECT_EQ(result.output_produced, lzss_dynamic_range_stream_prefix_size);

    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 4> dictionary_staging{};
    std::array<std::byte, 2048> frame_encoded{};
    LzssDynamicRangeFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded};
    ...
    LzssDynamicRangeFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded};
    ...
    LzssDynamicRangeFrameStreamingEncoder unknown{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded};
    result = unknown.process({}, {}, UINT32_C(1) << 31);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::unsupported);

    LzssDynamicRangeFrameStreamingEncoder excess{
