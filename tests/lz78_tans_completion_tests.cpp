...
constexpr std::size_t maximum_descriptor_size = maximum_blocks * 528;
constexpr std::size_t maximum_payload_size =
    maximum_blocks * (2 + (test_block_size * 12 + 7) / 8);
constexpr std::size_t maximum_frame_size =
    56 + maximum_descriptor_size + maximum_payload_size;
...
[[nodiscard]] marc_lz78_tans_config config(
    const marc_direction direction, const std::size_t original_size) {
    marc_lz78_tans_config result{};
    EXPECT_EQ(marc_lz78_tans_config_init(direction, &result), MARC_STATUS_OK);
    ...
    result.max_frame_size = 4096;
    result.max_block_size = 4096;
    result.max_compressed_payload_size = 4096;
    result.max_dictionary_serialized_size = 4096;
...
[[nodiscard]] Workspace workspace_for(
    const marc_lz78_tans_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lz78_tans_workspace_requirements(
...
[[nodiscard]] Transform create(
    const marc_lz78_tans_config& settings, Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lz78_tans_create(
                  &settings,
...
TEST(Lz78TansCompletion, RequiredDataClassesRoundTripDeterministically) {
    expect_round_trip({});
...
TEST(Lz78TansCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(Lz78TansCompletion, MalformedFinalFrameIsNeverCommitted) {
