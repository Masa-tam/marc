...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lzw_adaptive_huffman_stream_prefix_size);
...
TEST(LzwAdaptiveHuffmanFrameStreamingEncoder,
...
TEST(LzwAdaptiveHuffmanFrameStreamingEncoder,
    ...
    LzwAdaptiveHuffmanFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    LzwAdaptiveHuffmanFrameStreamingEncoder short_dictionary{
    ...
    EXPECT_EQ(short_dictionary.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzwAdaptiveHuffmanFrameStreamingEncoder short_entries{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded, {}};
    EXPECT_EQ(short_entries.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);

    LzwAdaptiveHuffmanFrameStreamingEncoder short_encoded{
    ...
    limits.max_internal_buffered_bytes = 2 + plan.dictionary_size
        ...
        - 1;
    LzwAdaptiveHuffmanFrameStreamingEncoder aggregate_limited{
...
TEST(LzwAdaptiveHuffmanFrameStreamingEncoder,
    ...
    std::array<std::byte, lzw_adaptive_huffman_stream_prefix_size> output{};
    LzwAdaptiveHuffmanFrameStreamingEncoder empty{
        config(0), {}, {}, {}, {}, unused, {}};
    ...
    std::array<std::byte, 2048> frame_encoded{};
    std::array<marc::dictionary::internal::LzwEncoderEntry, 1> entries{};
    LzwAdaptiveHuffmanFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
    ...
    LzwAdaptiveHuffmanFrameStreamingEncoder excess{
        config(1), {}, {}, frame_input, staging, frame_encoded, entries};
    result = excess.process(input, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::invalid_argument);

    LzwAdaptiveHuffmanFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
    ...
    LzwAdaptiveHuffmanFrameStreamingEncoder unknown{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
