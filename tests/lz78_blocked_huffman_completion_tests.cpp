...
marc_lz78_blocked_huffman_config config(
    ...
    marc_lz78_blocked_huffman_config result{};
    EXPECT_EQ(marc_lz78_blocked_huffman_config_init(direction, &result),
              MARC_STATUS_OK);
...
Workspace workspace_for(
    const marc_lz78_blocked_huffman_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lz78_blocked_huffman_workspace_requirements(
...
Transform create(const marc_lz78_blocked_huffman_config& settings,
                 Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lz78_blocked_huffman_create(
                  &settings,
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 80 + input.size() * 8
...
TEST(Lz78BlockedHuffmanCompletion,
...
TEST(Lz78BlockedHuffmanCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(Lz78BlockedHuffmanCompletion, MalformedFinalFrameIsNeverCommitted) {
