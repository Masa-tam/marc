...
constexpr std::size_t maximum_payload_size = test_frame_size * 33;
...
marc_adaptive_huffman_config config(
    ...
    marc_adaptive_huffman_config result{};
    EXPECT_EQ(marc_adaptive_huffman_config_init(direction, &result),
              MARC_STATUS_OK);
...
Workspace workspace_for(const marc_adaptive_huffman_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_adaptive_huffman_workspace_requirements(
...
Transform create(const marc_adaptive_huffman_config& settings,
                 Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_adaptive_huffman_create(
                  &settings,
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 64 + input.size() * 33
        + frames * (56 + descriptor_size);
...
TEST(AdaptiveHuffmanCompletion,
...
TEST(AdaptiveHuffmanCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(AdaptiveHuffmanCompletion, MalformedFinalFrameIsNeverCommitted) {
