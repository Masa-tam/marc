...
void expect_atomic_failure(const std::span<const std::byte> input) {
    ...
    std::array<marc::entropy::internal::TansBlockView, 8> views{};
    std::array<std::byte, raw.size() * 16> dictionary{};
...
TEST(Lz77TansFuzzRegression, EveryCanonicalTruncationIsAtomic) {
    const auto encoded = canonical_stream();
    for (std::size_t size = 0; size < encoded.size(); ++size)
        expect_atomic_failure(std::span<const std::byte>{encoded}.first(size));
}

TEST(Lz77TansFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lz77_tans_stream_prefix_size;
...
TEST(Lz77TansFuzzRegression, InvalidDescriptorIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lz77_tans_stream_prefix_size + frame_header_size + 16] ^=
        std::byte{1};
    expect_atomic_failure(encoded);
}
