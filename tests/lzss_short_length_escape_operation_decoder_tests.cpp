...
[[nodiscard]] constexpr std::array<ModeledOperation, 6> hand_operations() {
    return {{
        ...
        {ModeledOperationKind::symbol, 21, 9, 8, 0},
        {ModeledOperationKind::bypass_bits, 0, 0, 0, 1},
        {ModeledOperationKind::symbol, 31, 17, 0, 0},
    }};
}

TEST(LzssShortLengthEscapeOperationDecoder, ValidatesAndInvertsHandVector) {
    const auto operations = hand_operations();
    const auto limits = marc::core::DecoderLimits{};
    const auto checked = validate_lzss_short_length_escape_operations(
    ...
    const auto inverted = invert_lzss_short_length_escape_operations(
...
TEST(LzssShortLengthEscapeOperationDecoder, AcceptsFourAndMaximumLength) {
    ...
    std::array<LzssTypedToken, 2> tokens{};
    const auto limits = marc::core::DecoderLimits{};
    auto inverted = invert_lzss_short_length_escape_operations(
    ...
    operations[5].context_id = 30;
    context.declared_raw_size = 259;
    context.declared_decision_count = 12;
    inverted = invert_lzss_short_length_escape_operations(
        operations, parameters, context, limits, tokens);
    ASSERT_EQ(inverted.error, LzssFieldContextError::none);
    EXPECT_EQ(tokens[1].length, 258U);
}

TEST(LzssShortLengthEscapeOperationDecoder, InvertsEveryPermittedLength) {
    const auto limits = marc::core::DecoderLimits{};
    for (std::uint32_t length = 3; length <= 258; ++length) {
        ...
        std::array<LzssTypedToken, 2> tokens{};
        const auto result = invert_lzss_short_length_escape_operations(
...
TEST(LzssShortLengthEscapeOperationDecoder, RejectsMalformedBeforeOutput) {
    auto operations = hand_operations();
    const auto limits = marc::core::DecoderLimits{};
    ...
    auto result = invert_lzss_short_length_escape_operations(
...
TEST(LzssShortLengthEscapeOperationDecoder, RejectsCountsLimitsAndAliases) {
    auto operations = hand_operations();
    const auto limits = marc::core::DecoderLimits{};
    std::array<LzssTypedToken, 2> output{};
    ...
    auto context = hand_context;
    context.declared_event_count = 5;
    auto result = validate_lzss_short_length_escape_operations(
    ...
    result = validate_lzss_short_length_escape_operations(
        operations, parameters, context, limits);
...
TEST(LzssShortLengthEscapeOperationDecoder, RejectsTruncationAndRawMismatch) {
    auto operations = hand_operations();
    const auto limits = marc::core::DecoderLimits{};
    auto context = hand_context;
    context.declared_event_count = 5;
    const auto truncated = validate_lzss_short_length_escape_operations(
        std::span{operations}.first(5), parameters, context, limits);
    EXPECT_EQ(truncated.error, LzssFieldContextError::truncated_token);

    context = hand_context;
    context.declared_raw_size = 5;
    const auto mismatch = validate_lzss_short_length_escape_operations(
        operations, parameters, context, limits);
    EXPECT_EQ(mismatch.error, LzssFieldContextError::raw_size_mismatch);

    operations[3].alphabet_size = 8;
    const auto alphabet = validate_lzss_short_length_escape_operations(
        operations, parameters, hand_context, limits);
    ...
    const auto invalid = validate_lzss_short_length_escape_operations(
