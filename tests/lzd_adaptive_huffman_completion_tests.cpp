...
#ifndef MARC_LZD_COMPLETION_MAXIMUM_PAYLOAD
...
[[nodiscard]] marc_lzd_adaptive_huffman_config config(
    const marc_direction direction, const std::size_t original_size) {
    marc_lzd_adaptive_huffman_config result{};
    EXPECT_EQ(marc_lzd_adaptive_huffman_config_init(direction, &result),
              MARC_STATUS_OK);
...
[[nodiscard]] Workspace workspace_for(
    const marc_lzd_adaptive_huffman_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lzd_adaptive_huffman_workspace_requirements(
...
[[nodiscard]] Transform create(
    ...
    Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lzd_adaptive_huffman_create(
                  &settings,
...
TEST(LzdAdaptiveHuffmanCompletion,
...
TEST(LzdAdaptiveHuffmanCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(LzdAdaptiveHuffmanCompletion, MalformedFinalFrameIsNeverCommitted) {
