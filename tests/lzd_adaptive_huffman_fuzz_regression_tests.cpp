...
using EncoderEntry = marc::dictionary::internal::LzdEncoderEntry;
...
[[nodiscard]] std::vector<std::byte> canonical_stream() {
    ...
    std::array<std::byte, 8 * ((raw.size() + 1) / 2)> dictionary{};
    std::array<std::byte, 4096> frame_encoded{};
    std::array<EncoderEntry, raw.size() / 2> entries{};
...
void expect_atomic_failure(const std::span<const std::byte> input) {
    ...
    std::array<Phrase, raw.size() / 2> phrases{};
    std::array<std::uint32_t, raw.size() / 2 + 1> expansion{};
    std::array<std::byte, raw.size()> output{};
    output.fill(std::byte{0xa5});
    LzdAdaptiveHuffmanFrameStreamingDecoder decoder{
        {}, encoded_frame, dictionary, decoded_frame, phrases, expansion};
...
TEST(LzdAdaptiveHuffmanFuzzRegression, EveryCanonicalTruncationIsAtomic) {
...
TEST(LzdAdaptiveHuffmanFuzzRegression, ExtremeFrameLengthsAreAtomic) {
...
TEST(LzdAdaptiveHuffmanFuzzRegression, InvalidDescriptorIsAtomic) {
    auto encoded = canonical_stream();
