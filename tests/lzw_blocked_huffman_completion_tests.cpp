...
namespace lzw_blocked_huffman_completion {
...
constexpr std::size_t maximum_blocks =
    maximum_payload_size / test_block_size;
...
marc_lzw_blocked_huffman_config config(const marc_direction direction,
                                       const std::size_t original_size) {
    marc_lzw_blocked_huffman_config result{};
    EXPECT_EQ(marc_lzw_blocked_huffman_config_init(direction, &result),
              MARC_STATUS_OK);
...
Workspace workspace_for(const marc_lzw_blocked_huffman_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lzw_blocked_huffman_workspace_requirements(
...
Transform create(const marc_lzw_blocked_huffman_config& settings, Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lzw_blocked_huffman_create(
                  &settings,
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 80
        + frames * (56 + maximum_descriptor_size + maximum_payload_size);
...
TEST(LzwBlockedHuffmanCompletion, RequiredDataClassesRoundTripDeterministically) {
    expect_round_trip({});
...
TEST(LzwBlockedHuffmanCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(LzwBlockedHuffmanCompletion, MalformedFinalFrameIsNeverCommitted) {
