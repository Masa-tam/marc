...
TEST(Lz78DynamicRangeFuzzRegression, EveryCanonicalTruncationIsAtomic) {
...
TEST(Lz78DynamicRangeFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lz78_dynamic_range_stream_prefix_size;
...
TEST(Lz78DynamicRangeFuzzRegression, InvalidDescriptorIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lz78_dynamic_range_stream_prefix_size + frame_header_size
