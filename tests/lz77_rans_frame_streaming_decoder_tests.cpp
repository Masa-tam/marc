...
TEST(Lz77RansFrameStreamingDecoder, DecodesOneByteInputAndOutput) {
    const auto encoded = encoded_stream();
    std::array<std::byte, 8192> frame_encoded{};
    std::array<marc::entropy::internal::RansBlockView, 7> views{};
    std::array<std::byte, 32> dictionary_staging{};
...
TEST(Lz77RansFrameStreamingDecoder, CommitsOnlyFramesBeforeLaterCorruption) {
    auto encoded = encoded_stream();
    const auto first_size = frame_size_at(
        encoded, lz77_rans_stream_prefix_size);
    const auto second_offset = lz77_rans_stream_prefix_size + first_size;
...
TEST(Lz77RansFrameStreamingDecoder, ReportsWorkspaceAndAggregateErrors) {
    const auto encoded = encoded_stream();
    const auto first_size = frame_size_at(
        encoded, lz77_rans_stream_prefix_size);
    std::array<std::byte, 8192> frame_encoded{};
    ...
    Lz77RansFrameStreamingDecoder short_encoded{
    ...
    Lz77RansFrameStreamingDecoder short_views{
        {}, frame_encoded,
        std::span<marc::entropy::internal::RansBlockView>{views}.first<6>(),
        dictionary_staging, frame_decoded};
    EXPECT_EQ(short_views.process(encoded, output, end).error.code,
              marc::core::ErrorCode::out_of_memory);

    Lz77RansFrameStreamingDecoder short_dictionary{
        {}, frame_encoded, views,
    ...
    Lz77RansFrameStreamingDecoder short_decoded{
    ...
    limits.max_internal_buffered_bytes =
        first_size + 32 + 2
        + 7 * sizeof(marc::entropy::internal::RansBlockView) - 1;
    Lz77RansFrameStreamingDecoder aggregate_limited{
...
TEST(Lz77RansFrameStreamingDecoder, RejectsTruncationTrailingAndReset) {
    const auto encoded = encoded_stream();
    std::array<std::byte, 8192> frame_encoded{};
    std::array<marc::entropy::internal::RansBlockView, 7> views{};
    std::array<std::byte, 32> dictionary_staging{};
    ...
    Lz77RansFrameStreamingDecoder truncated{
        {}, frame_encoded, views, dictionary_staging, frame_decoded};
    ...
    Lz77RansFrameStreamingDecoder trailing{
        {}, frame_encoded, views, dictionary_staging, frame_decoded};
    EXPECT_EQ(trailing.process(extended, output, end).error.code,
              marc::core::ErrorCode::malformed_stream);

    Lz77RansFrameStreamingDecoder reset{
        {}, frame_encoded, views, dictionary_staging, frame_decoded};
...
TEST(Lz77RansFrameStreamingDecoder, HandlesEmptyFlushAndPrematureEnd) {
    const auto empty_encoded = encoded_stream({});
    Lz77RansFrameStreamingDecoder empty{{}, {}, {}, {}, {}};
    ...
    std::array<std::byte, 32> dictionary_staging{};
    std::array<std::byte, 2> frame_decoded{};
    Lz77RansFrameStreamingDecoder starved{
        {}, frame_encoded, views, dictionary_staging, frame_decoded};
    ...
    const auto prefix_and_first_frame =
        lz77_rans_stream_prefix_size + first_size;
    Lz77RansFrameStreamingDecoder premature{
        {}, frame_encoded, views, dictionary_staging, frame_decoded};
    std::array<std::byte, 1> output{};
