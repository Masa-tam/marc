...
TEST(LzwRansFramePlanner, PlansExactIndependentVectorExtent) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, packed_code_a.size()> staging{};
    const auto result = marc::frame::plan_lzw_rans_frame(
...
TEST(LzwRansFramePlanner, PlansPackedCodesAndBlocksDeterministically) {
    ...
    const auto first_plan = marc::frame::plan_lzw_rans_frame(
        stream, {}, {}, 0, 0, raw, workspace, first);
    ASSERT_EQ(first_plan.error, LzwRansFrameValidationError::none);
    const auto second_plan = marc::frame::plan_lzw_rans_frame(
        stream, {}, {}, 0, 0, raw, workspace, second);
    ASSERT_EQ(second_plan.error, LzwRansFrameValidationError::none);
    ...
    EXPECT_EQ(first_plan.payload_size, 3U * 8U);
    EXPECT_EQ(first_plan.serialized_size, 56U + 3U * 528U + 3U * 8U);
...
TEST(LzwRansFramePlanner, RejectsCapacitiesBeforePackedMutation) {
    ...
    std::array staging{
        std::byte{0x5a}, std::byte{0x5a}, std::byte{0x5a}};
    EXPECT_EQ(marc::frame::plan_lzw_rans_frame(
...
TEST(LzwRansFramePlanner, EnforcesAggregateAndFrameExtent) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, packed_code_a.size()> staging{};
    auto limits = marc::core::DecoderLimits{};
    limits.max_block_size = packed_code_a.size();
    limits.max_internal_buffered_bytes = 537;
    EXPECT_EQ(marc::frame::plan_lzw_rans_frame(
...
TEST(LzwRansFrameEncoder, EmitsExactIndependentVector) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, packed_code_a.size()> staging{};
    std::array<std::byte, 592> output{};
    const auto result = marc::frame::encode_lzw_rans_frame(
...
TEST(LzwRansFrameEncoder, RoundTripsMultipleBlocksDeterministically) {
    constexpr std::array raw{
        std::byte{'A'}, std::byte{'B'}, std::byte{'A'}, std::byte{'B'},
        std::byte{'A'}, std::byte{'B'}, std::byte{'A'}};
    constexpr std::uint32_t block_size = 2;
    ...
    const auto plan = marc::frame::plan_lzw_rans_frame(
    ...
    ASSERT_EQ(marc::frame::encode_lzw_rans_frame(
                  stream, {}, {}, 0, 0, raw, workspace, encode_staging,
                  first).error,
...
TEST(LzwRansFrameEncoder, ShortOutputIsCompletelyUnchanged) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, packed_code_a.size()> staging{};
    std::array<std::byte, 591> output{};
    output.fill(std::byte{0xa5});
    EXPECT_EQ(marc::frame::encode_lzw_rans_frame(
                  stream_for(raw.size()), {}, {}, 0, 0, raw, {}, staging,
                  output).error,
