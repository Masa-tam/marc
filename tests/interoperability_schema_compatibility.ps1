...
function Convert-Schema37To36([string]$Source, [string]$Destination) {
    Copy-Item -LiteralPath $Source -Destination $Destination -Recurse
    $manifestPath = Join-Path $Destination 'manifest.json'
    $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
    $manifest.schema_version = 36
    $manifest.codec_set = 'marc-cli-v36'
    $entry = @($manifest.archives | Where-Object {
        $_.codec -eq 'lzss-contextual-rans' })
    if ($entry.Count -ne 1) {
        throw 'Schema 37 must contain one canonical Contextual rANS archive'
    }
    $oldFile = 'lzss-contextual-rans.marc'
    $newFile = 'lzss-contextual-rans-compact.marc'
    if ([string]$entry[0].file -ne $oldFile) {
        throw 'Schema 37 Contextual rANS leaf name is not canonical'
    }
    Move-Item -LiteralPath (Join-Path $Destination $oldFile) `
        -Destination (Join-Path $Destination $newFile)
    $entry[0].codec = 'lzss-contextual-rans-compact'
    $entry[0].file = $newFile
    Write-Manifest $manifestPath $manifest
}
...
$schema56Reordered = Join-Path $root 'schema56-reordered'
$schema55 = Join-Path $root 'schema55'
$schema54 = Join-Path $root 'schema54'
$schema53 = Join-Path $root 'schema53'
$schema52 = Join-Path $root 'schema52'
$schema51 = Join-Path $root 'schema51'
$schema50 = Join-Path $root 'schema50'
$schema49 = Join-Path $root 'schema49'
$schema48 = Join-Path $root 'schema48'
$schema47 = Join-Path $root 'schema47'
$schema46 = Join-Path $root 'schema46'
$schema45 = Join-Path $root 'schema45'
$schema44 = Join-Path $root 'schema44'
$schema43 = Join-Path $root 'schema43'
$schema42 = Join-Path $root 'schema42'
$schema41 = Join-Path $root 'schema41'
$schema40 = Join-Path $root 'schema40'
$schema39 = Join-Path $root 'schema39'
$schema38 = Join-Path $root 'schema38'
$schema37 = Join-Path $root 'schema37'
$schema36 = Join-Path $root 'schema36'
$schema35 = Join-Path $root 'schema35'
$schema34 = Join-Path $root 'schema34'
...
$schema51Profiles = $schema50Profiles + @(
    'lzss-contextual-blocked-huffman-16m')
$schema52Profiles = $schema51Profiles + @(
    'lzss-contextual-adaptive-huffman-16m')
$schema53Profiles = $schema52Profiles + @(
    'lzss-contextual-dynamic-range-64m')
$schema54Profiles = $schema53Profiles + @(
    'lzss-contextual-rans-64m')
$schema55Profiles = $schema54Profiles + @(
    'lzss-contextual-tans-64m')
$schema56Profiles = $schema55Profiles + @(
    'lzss-contextual-blocked-huffman-64m')
try {
    $null = New-Item -ItemType Directory -Path $root
    & (Join-Path $PSScriptRoot 'create_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -OutputDirectory $schema56 `
        -Platform 'local-schema-test' `
        -Compiler 'local-schema-test' `
        -SourceRevision ('0' * 40)
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema56 `
        -OutputDirectory (Join-Path $root 'verified56')

    Copy-Item -LiteralPath $schema56 -Destination $schema56Reordered -Recurse
    $reorderedManifestPath = Join-Path $schema56Reordered 'manifest.json'
    ...
    try {
        & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
            -MarcCli $resolvedCli `
            -BundleDirectory $schema56Reordered `
            -OutputDirectory (Join-Path $root 'verified56-reordered')
    } catch {
    ...
    if (-not $reorderedRejected) {
        throw 'Verifier accepted a reordered schema-56 manifest'
    }

    Convert-Bundle $schema56 $schema55 55 'marc-cli-v55' $schema55Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema55 `
        -OutputDirectory (Join-Path $root 'verified55')

    Convert-Bundle $schema55 $schema54 54 'marc-cli-v54' $schema54Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema54 `
        -OutputDirectory (Join-Path $root 'verified54')

    Convert-Bundle $schema54 $schema53 53 'marc-cli-v53' $schema53Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema53 `
        -OutputDirectory (Join-Path $root 'verified53')

    Convert-Bundle $schema53 $schema52 52 'marc-cli-v52' $schema52Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema52 `
        -OutputDirectory (Join-Path $root 'verified52')

    Convert-Bundle $schema52 $schema51 51 'marc-cli-v51' $schema51Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema51 `
        -OutputDirectory (Join-Path $root 'verified51')

    Convert-Bundle $schema51 $schema50 50 'marc-cli-v50' $schema50Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema50 `
        -OutputDirectory (Join-Path $root 'verified50')

    Convert-Bundle $schema50 $schema49 49 'marc-cli-v49' $schema49Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema49 `
        -OutputDirectory (Join-Path $root 'verified49')

    Convert-Bundle $schema49 $schema48 48 'marc-cli-v48' $schema48Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema48 `
        -OutputDirectory (Join-Path $root 'verified48')

    Convert-Bundle $schema48 $schema47 47 'marc-cli-v47' $schema47Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema47 `
        -OutputDirectory (Join-Path $root 'verified47')

    Convert-Bundle $schema47 $schema46 46 'marc-cli-v46' $schema46Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema46 `
        -OutputDirectory (Join-Path $root 'verified46')

    Convert-Bundle $schema46 $schema45 45 'marc-cli-v45' $schema45Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema45 `
        -OutputDirectory (Join-Path $root 'verified45')

    Convert-Bundle $schema45 $schema44 44 'marc-cli-v44' $schema44Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema44 `
        -OutputDirectory (Join-Path $root 'verified44')

    Convert-Bundle $schema44 $schema43 43 'marc-cli-v43' $schema43Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema43 `
        -OutputDirectory (Join-Path $root 'verified43')

    Convert-Bundle $schema43 $schema42 42 'marc-cli-v42' $schema42Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema42 `
        -OutputDirectory (Join-Path $root 'verified42')

    Convert-Bundle $schema42 $schema41 41 'marc-cli-v41' $schema41Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema41 `
        -OutputDirectory (Join-Path $root 'verified41')

    Convert-Bundle $schema41 $schema40 40 'marc-cli-v40' $schema40Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema40 `
        -OutputDirectory (Join-Path $root 'verified40')

    Convert-Bundle $schema40 $schema39 39 'marc-cli-v39' $schema39Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema39 `
        -OutputDirectory (Join-Path $root 'verified39')

    Convert-Bundle $schema39 $schema38 38 'marc-cli-v38' $schema38Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema38 `
        -OutputDirectory (Join-Path $root 'verified38')

    Convert-Bundle $schema38 $schema37 37 'marc-cli-v37' $schema37Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema37 `
        -OutputDirectory (Join-Path $root 'verified37')

    Convert-Schema37To36 $schema37 $schema36
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema36 `
        -OutputDirectory (Join-Path $root 'verified36')

    Convert-Bundle $schema36 $schema35 35 'marc-cli-v35' $schema35Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema35 `
        -OutputDirectory (Join-Path $root 'verified35')

    Convert-Bundle $schema35 $schema34 34 'marc-cli-v34' $schema34Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
        -BundleDirectory $schema34 `
        -OutputDirectory (Join-Path $root 'verified34')

    Convert-Bundle $schema34 $schema33 33 'marc-cli-v33' $schema33Profiles
    & (Join-Path $PSScriptRoot 'verify_interoperability_bundle.ps1') `
        -MarcCli $resolvedCli `
    ...
    Write-Host 'Verified interoperability schemas 1 through 56'
} finally {
