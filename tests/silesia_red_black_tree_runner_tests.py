#!/usr/bin/env python3
"""Unit tests for the fixed AVL/Red-Black Silesia runner."""
...
import run_silesia_red_black_tree_experiment as runner  # noqa: E402
...
class SilesiaRedBlackTreeRunnerTests(unittest.TestCase):
    ...
    def test_validates_complete_frame_reports(self) -> None:
        for strategy in runner.STRATEGIES:
            runner._validate_report(_report(strategy), strategy, 8,
                                    runner.WINDOWS[0])
        for key, value in (
            ("mode", "frames-limited"),
            ("binary_tree_workspace_bytes", "invalid"),
            ("literal_count", 3),
    ...
    def test_rejects_nonfinite_time_and_incomplete_histogram(self) -> None:
        ...
        report["red_black_tree_frame_seconds"] = float("nan")
        with self.assertRaises(RunnerError):
            runner._validate_report(report, runner.STRATEGIES[1], 8,
                                    runner.WINDOWS[0])
        report = _report(runner.STRATEGIES[1])
        report["red_black_tree_query_depth_histogram"] = [0, 3]
        with self.assertRaises(RunnerError):
            runner._validate_report(report, runner.STRATEGIES[1], 8,
                                    runner.WINDOWS[0])
    ...
    def test_aggregates_and_compares_each_window(self) -> None:
        ...
        self.assertTrue(all(
            item["red_black_to_avl_workspace_ratio"] == 1.0
            for item in comparisons
        ))
