...
TEST(LzwTansFuzzRegression, EveryCanonicalTruncationIsAtomic) {
...
TEST(LzwTansFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lzw_tans_stream_prefix_size;
...
TEST(LzwTansFuzzRegression, InvalidDescriptorIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lzw_tans_stream_prefix_size + frame_header_size + 16] ^=
        std::byte{1};
    expect_atomic_failure(encoded);
}
