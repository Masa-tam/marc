...
constexpr std::size_t maximum_payload_size = test_frame_size * 12 + 5;
constexpr std::size_t maximum_frame_size = 64 + 16 + maximum_payload_size;
...
[[nodiscard]] marc_lzss_contextual_dynamic_range_config config(
    ...
    result.max_block_size = test_frame_size;
...
[[nodiscard]] Workspace workspace_for(
    const marc_lzss_contextual_dynamic_range_config& settings) {
    Workspace result{};
...
[[nodiscard]] std::size_t frame_extent(
    ...
    return 64 + load_le32(stream, offset + 36)
        + load_le32(stream, offset + 40)
        + load_le32(stream, offset + 32)
...
TEST(LzssContextualDynamicRangeCompletion,
