...
TEST(LzwRansFuzzRegression, EveryCanonicalTruncationIsAtomic) {
...
TEST(LzwRansFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lzw_rans_stream_prefix_size;
...
TEST(LzwRansFuzzRegression, NonzeroDescriptorReservedByteIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lzw_rans_stream_prefix_size + frame_header_size + 10] =
        std::byte{1};
    expect_atomic_failure(encoded);
}
