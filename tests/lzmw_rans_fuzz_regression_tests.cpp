...
TEST(LzmwRansFuzzRegression, EveryCanonicalTruncationIsAtomic) {
...
TEST(LzmwRansFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lzmw_rans_stream_prefix_size;
...
TEST(LzmwRansFuzzRegression, NonzeroDescriptorReservedByteIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lzmw_rans_stream_prefix_size + frame_header_size + 10] =
        std::byte{1};
    expect_atomic_failure(encoded);
}
