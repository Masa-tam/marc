...
TEST(Lz77DynamicRangeFrameDecoder,
     PublishesHandVectorOnlyAfterPrivateDecode) {
    std::array<std::byte, 16> dictionary_staging{};
    std::array<std::byte, 3> raw_staging{};
    std::array<std::byte, 3> output{};
    raw_staging.fill(std::byte{0x5a});
    output.fill(std::byte{0x5a});
    const auto result = marc::frame::decode_lz77_dynamic_range_frame(
        stream_for_a(), {}, {}, 0, 0, single_literal_frame,
...
TEST(Lz77DynamicRangeFrameDecoder, PublishesOverlappingMatchExactly) {
    std::array<std::byte, 32> tokens{};
    const marc::dictionary::internal::Lz77Token literal{
        marc::dictionary::internal::Lz77TokenTag::literal, 0, 0, 'A'};
    const marc::dictionary::internal::Lz77Token match{
...
TEST(Lz77DynamicRangeFrameDecoder,
     RejectsShortOutputBeforePrivateMutation) {
    std::array<std::byte, 16> dictionary_staging{};
    dictionary_staging.fill(std::byte{0x5a});
    std::array<std::byte, 1> raw_staging{std::byte{0x5a}};
    const auto result = marc::frame::decode_lz77_dynamic_range_frame(
...
TEST(Lz77DynamicRangeFrameDecoder, MalformedLayersNeverPublishRawBytes) {
    std::array<std::byte, 16> dictionary_staging{};
    std::array<std::byte, 1> raw_staging{std::byte{0x5a}};
    std::array<std::byte, 1> output{std::byte{0x5a}};
    auto invalid_descriptor = single_literal_frame;
    invalid_descriptor[71] = std::byte{1};
    EXPECT_EQ(marc::frame::decode_lz77_dynamic_range_frame(
...
TEST(Lz77DynamicRangeFrameEncoder, PlansExactHandVectorExtent) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, 16> staging{};
    const auto result = marc::frame::plan_lz77_dynamic_range_frame(
    ...
    EXPECT_EQ(result.serialized_size, single_literal_frame.size());
    EXPECT_EQ(staging[0], std::byte{0});
    EXPECT_EQ(staging[12], std::byte{'A'});
}

TEST(Lz77DynamicRangeFrameEncoder, EmitsExactIndependentHandVector) {
    constexpr std::array raw{std::byte{'A'}};
    std::array<std::byte, 16> staging{};
    std::array<std::byte, single_literal_frame.size()> output{};
    const auto result = marc::frame::encode_lz77_dynamic_range_frame(
        stream_for_a(), {}, {}, 0, 0, raw, staging, output);
    ASSERT_EQ(result.error, Lz77DynamicRangeFrameValidationError::none);
    EXPECT_EQ(output, single_literal_frame);
}

TEST(Lz77DynamicRangeFrameEncoder,
     RoundTripsOverlappingMatchDeterministically) {
    constexpr std::array raw{
        std::byte{'A'}, std::byte{'A'}, std::byte{'A'}, std::byte{'A'},
        std::byte{'A'}};
    std::array<std::byte, 80> encode_staging{};
    const auto plan = marc::frame::plan_lz77_dynamic_range_frame(
        stream_for(raw.size()), {}, {}, 0, 0, raw, encode_staging);
    ASSERT_EQ(plan.error, Lz77DynamicRangeFrameValidationError::none);
    std::vector<std::byte> first(plan.serialized_size);
    std::vector<std::byte> second(plan.serialized_size);
    ASSERT_EQ(marc::frame::encode_lz77_dynamic_range_frame(
    ...
    std::array<std::byte, 80> decode_staging{};
    std::array<std::byte, raw.size()> raw_staging{};
    std::array<std::byte, raw.size()> decoded{};
    ASSERT_EQ(marc::frame::decode_lz77_dynamic_range_frame(
...
TEST(Lz77DynamicRangeFrameEncoder,
    ...
    EXPECT_EQ(marc::frame::plan_lz77_dynamic_range_frame(
...
TEST(Lz77DynamicRangeFrameEncoder, RejectsEmptyAndUnexpectedFrameExtent) {
    std::array<std::byte, 32> staging{};
    EXPECT_EQ(marc::frame::plan_lz77_dynamic_range_frame(
                  stream_for_a(), {}, {}, 0, 0,
                  std::span<const std::byte>{}, staging).error,
              Lz77DynamicRangeFrameValidationError::input_size_mismatch);
...
TEST(Lz77DynamicRangeFrameEncoder, EnforcesAggregateWorkspaceBound) {
    auto limits = marc::core::DecoderLimits{};
    limits.max_block_size = 16;
    limits.max_internal_buffered_bytes = 16 + 16 + 16 - 1;
    constexpr std::array raw{std::byte{'A'}};
