...
#include "dictionary/lzd_streaming_encoder.hpp"
...
std::vector<std::byte> reference_encode(
    ...
    std::vector<LzdEncoderEntry> dictionary(
        lzd_encoder_workspace_entries(raw.size(), parameters));
...
struct EncoderStorage {
    EncoderStorage(const std::size_t raw_size,
                   const LzdParameters parameters = {}) {
        requirements = lzd_streaming_encoder_workspace_requirements(
            raw_size, parameters);
    ...
    LzdStreamingEncoderWorkspaceRequirements requirements{};
    std::vector<std::byte> raw;
    std::vector<std::byte> encoded;
    std::vector<LzdEncoderEntry> dictionary;
};

TEST(LzdStreamingEncoder, ReportsBoundedWorkspaceRequirements) {
    const auto empty = lzd_streaming_encoder_workspace_requirements(0, {});
...
TEST(LzdStreamingEncoder, FullFrameCanDrainBeforeEndInput) {
    ...
    EXPECT_EQ(std::vector(output.begin(), output.end()),
...
TEST(LzdStreamingEncoder, PreservesEndInputWhileDraining) {
...
TEST(LzdStreamingEncoder, FlushDoesNotClosePartialFrame) {
    const auto raw = bytes("abbaababaaba");
    EncoderStorage storage{raw.size()};
    LzdStreamingEncoder encoder{{}, raw.size(), {}, storage.raw,
                                 storage.encoded, storage.dictionary};
...
TEST(LzdStreamingEncoder, PreservesDictionaryFreeze) {
    const auto raw = bytes("ABABABABAB");
    ...
    LzdStreamingEncoder encoder{parameters, raw.size(), limits, storage.raw,
                                 storage.encoded, storage.dictionary};
    ...
    EXPECT_EQ(result.output_produced, output.size());
    EXPECT_EQ(std::vector(output.begin(), output.end()),
              reference_encode(raw, parameters, limits));
}

TEST(LzdStreamingEncoder, ReportsPrematureTrailingAndWorkspaceFailures) {
    const auto raw = bytes("ABAB");
    EncoderStorage storage{raw.size()};
    LzdStreamingEncoder premature{{}, raw.size(), {}, storage.raw,
                                   storage.encoded, storage.dictionary};
    ...
    LzdStreamingEncoder trailing{{}, 3, {}, storage.raw, storage.encoded,
                                  storage.dictionary};
    result = trailing.process(raw, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::invalid_argument);
    EXPECT_EQ(result.input_consumed, 3U);

    LzdStreamingEncoder short_raw{{}, raw.size(), {}, {}, storage.encoded,
                                   storage.dictionary};
    result = short_raw.process({}, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::out_of_memory);
    LzdStreamingEncoder short_encoded{{}, raw.size(), {}, storage.raw, {},
                                       storage.dictionary};
    result = short_encoded.process({}, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::out_of_memory);
    LzdStreamingEncoder short_dictionary{{}, raw.size(), {}, storage.raw,
                                          storage.encoded, {}};
    ...
    limits.max_internal_buffered_bytes =
        ...
        + storage.requirements.dictionary_entries * sizeof(LzdEncoderEntry)
        - 1;
    limits.max_block_size = limits.max_internal_buffered_bytes;
    LzdStreamingEncoder aggregate_limit{
...
TEST(LzdStreamingEncoder, HandlesEmptyAndRejectsReset) {
    LzdStreamingEncoder empty{{}, 0, {}, {}, {}, {}};
    auto result = empty.process(
        {}, {}, marc::core::flag_value(marc::core::ProcessFlags::end_input));
    EXPECT_EQ(result.status, marc::core::StreamStatus::end_of_stream);

    EncoderStorage storage{1};
    LzdStreamingEncoder reset{{}, 1, {}, storage.raw, storage.encoded,
                               storage.dictionary};
