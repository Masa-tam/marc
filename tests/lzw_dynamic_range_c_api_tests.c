...
static void set_small_limits(marc_lzw_dynamic_range_config* config) {
...
int main(void) {
    ...
    assert(marc_lzw_dynamic_range_config_init(
    ...
    assert(marc_lzw_dynamic_range_workspace_requirements(
               &config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 2);
    assert(needed.secondary_bytes == 86);
    ...
    assert(marc_lzw_dynamic_range_create(
