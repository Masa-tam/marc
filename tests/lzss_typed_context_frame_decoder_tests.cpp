...
[[nodiscard]] constexpr TypedContextStreamHeader
sixty_four_mib_stream_config() {
    auto stream = stream_config();
...
TEST(LzssTypedContextFrameDecoder,
     SixtyFourMiBIdentityChecksWorkspaceAndCrossingBeforeWrites) {
    constexpr auto frame = frame_vector();
    auto stream = sixty_four_mib_stream_config();
    stream.frame_size = 1;
...
TEST(LzssTypedContextFrameDecoder,
    ...
    constexpr std::uint32_t distance = 16777217;
    constexpr std::size_t full_matches = 65027;
    ...
    stream.frame_size = raw_size;
    stream.original_size = raw_size;
    auto limits = sixty_four_mib_limits();
    const marc::dictionary::internal::LzssTypedFrameValidationContext
        token_context{static_cast<std::uint32_t>(source_tokens.size()),
                      raw_size, 0};
    const auto operation_plan =
