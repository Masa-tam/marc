...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lzd_adaptive_huffman_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        EXPECT_EQ(encode_lzd_adaptive_huffman_frame(
...
TEST(LzdAdaptiveHuffmanFrameStreamingEncoder,
    ...
    std::array<marc::dictionary::internal::LzdEncoderEntry, 1> entries{};
    LzdAdaptiveHuffmanFrameStreamingEncoder encoder{
...
TEST(LzdAdaptiveHuffmanFrameStreamingEncoder,
    ...
    EXPECT_EQ(first.output_produced,
              lzd_adaptive_huffman_stream_prefix_size);
...
TEST(LzdAdaptiveHuffmanFrameStreamingEncoder,
    ...
    LzdAdaptiveHuffmanFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    LzdAdaptiveHuffmanFrameStreamingEncoder short_dictionary{
    ...
    EXPECT_EQ(short_dictionary.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzdAdaptiveHuffmanFrameStreamingEncoder short_entries{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded, {}};
    EXPECT_EQ(short_entries.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);

    LzdAdaptiveHuffmanFrameStreamingEncoder short_encoded{
    ...
    limits.max_internal_buffered_bytes = 2 + plan.dictionary_size
        ...
        - 1;
    LzdAdaptiveHuffmanFrameStreamingEncoder aggregate_limited{
...
TEST(LzdAdaptiveHuffmanFrameStreamingEncoder,
    ...
    std::array<std::byte, lzd_adaptive_huffman_stream_prefix_size> output{};
    LzdAdaptiveHuffmanFrameStreamingEncoder empty{
        config(0), {}, {}, {}, {}, unused, {}};
    ...
    std::array<std::byte, 2048> frame_encoded{};
    std::array<marc::dictionary::internal::LzdEncoderEntry, 1> entries{};
    LzdAdaptiveHuffmanFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
    ...
    LzdAdaptiveHuffmanFrameStreamingEncoder excess{
        config(1), {}, {}, frame_input, staging, frame_encoded, entries};
    result = excess.process(input, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::invalid_argument);

    LzdAdaptiveHuffmanFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
    ...
    LzdAdaptiveHuffmanFrameStreamingEncoder unknown{
        config(input.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
