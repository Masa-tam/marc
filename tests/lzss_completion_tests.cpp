...
marc_lzss_config config(const marc_direction direction,
                        const std::size_t original_size) {
    marc_lzss_config result{};
    EXPECT_EQ(marc_lzss_config_init(direction, &result), MARC_STATUS_OK);
...
Workspace workspace_for(const marc_lzss_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lzss_workspace_requirements(&settings, &result.requirements),
              MARC_STATUS_OK);
...
Transform create(const marc_lzss_config& settings, Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lzss_create(
                  &settings,
...
TEST(LzssCompletion, RequiredDataClassesRoundTripDeterministically) {
    expect_round_trip({});
...
TEST(LzssCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(LzssCompletion, MalformedFinalFrameIsNeverCommitted) {
