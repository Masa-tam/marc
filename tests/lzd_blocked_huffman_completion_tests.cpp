...
namespace lzd_blocked_huffman_completion {

constexpr std::size_t test_frame_size = 64;
constexpr std::size_t test_block_size = 64;
constexpr std::size_t maximum_dictionary_size =
    8 * ((test_frame_size + 1) / 2);
...
marc_lzd_blocked_huffman_config config(
    ...
    marc_lzd_blocked_huffman_config result{};
    EXPECT_EQ(marc_lzd_blocked_huffman_config_init(direction, &result),
              MARC_STATUS_OK);
    result.original_size = original_size;
    result.frame_size = test_frame_size;
    result.entropy_block_size = test_block_size;
    ...
    result.max_compressed_payload_size = maximum_dictionary_size;
...
Workspace workspace_for(
    const marc_lzd_blocked_huffman_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lzd_blocked_huffman_workspace_requirements(
...
Transform create(const marc_lzd_blocked_huffman_config& settings,
                 Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lzd_blocked_huffman_create(
                  &settings,
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 80
        + frames * (56 + maximum_descriptor_size + maximum_dictionary_size);
...
TEST(LzdBlockedHuffmanCompletion,
...
TEST(LzdBlockedHuffmanCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(LzdBlockedHuffmanCompletion, MalformedFinalFrameIsNeverCommitted) {
