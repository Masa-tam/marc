...
marc_lzmw_config config(const marc_direction direction,
                       const std::size_t original_size) {
    marc_lzmw_config result{};
    EXPECT_EQ(marc_lzmw_config_init(direction, &result), MARC_STATUS_OK);
...
Workspace workspace_for(const marc_lzmw_config& config) {
    Workspace result{};
    EXPECT_EQ(marc_lzmw_workspace_requirements(&config, &result.requirements),
              MARC_STATUS_OK);
...
Transform create(const marc_lzmw_config& config, Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lzmw_create(
                  &config,
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 80 + input.size() * 4 + frames * 56;
...
TEST(LzmwCompletion, MultiFrameStreamIsIndependentOfChunking) {
