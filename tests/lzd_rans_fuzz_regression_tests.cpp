...
TEST(LzdRansFuzzRegression, EveryCanonicalTruncationIsAtomic) {
...
TEST(LzdRansFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lzd_rans_stream_prefix_size;
...
TEST(LzdRansFuzzRegression, NonzeroDescriptorReservedByteIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lzd_rans_stream_prefix_size + frame_header_size + 10] =
        std::byte{1};
    expect_atomic_failure(encoded);
}
