...
struct Workspace {
    ...
    std::array<marc::dictionary::internal::Lz78PhraseEntry, 8> phrases{};
};

} // namespace

TEST(Lz78RansFrameStreamingDecoder, DecodesOneByteInputAndOutput) {
    ...
    do {
        ...
        ASSERT_TRUE(
            marc::core::is_valid(result, chunk.size(), output.size()));
...
TEST(Lz78RansFrameStreamingDecoder,
     CommitsOnlyFramesBeforeLaterCorruption) {
    auto encoded = encoded_stream();
    const auto first_size =
        frame_size_at(encoded, lz78_rans_stream_prefix_size);
    const auto second_offset = lz78_rans_stream_prefix_size + first_size;
...
TEST(Lz78RansFrameStreamingDecoder, ReportsWorkspaceAndAggregateErrors) {
    const auto encoded = encoded_stream();
    const auto first_size =
        frame_size_at(encoded, lz78_rans_stream_prefix_size);
    Workspace workspace{};
    ...
    Lz78RansFrameStreamingDecoder short_encoded{
        {},
    ...
    Lz78RansFrameStreamingDecoder short_views{
    ...
    Lz78RansFrameStreamingDecoder short_dictionary{
        {}, workspace.frame_encoded, workspace.views,
        std::span<std::byte>{workspace.dictionary_staging}.first<7>(),
        workspace.frame_decoded, workspace.phrases};
    EXPECT_EQ(short_dictionary.process(encoded, output, end).error.code,
              marc::core::ErrorCode::out_of_memory);

    Lz78RansFrameStreamingDecoder short_decoded{
        {}, workspace.frame_encoded, workspace.views,
    ...
    Lz78RansFrameStreamingDecoder short_phrases{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
    ...
    Lz78RansFrameStreamingDecoder aggregate_limited{
...
TEST(Lz78RansFrameStreamingDecoder, RejectsTruncationTrailingAndReset) {
    const auto encoded = encoded_stream();
    Workspace workspace{};
    std::array<std::byte, raw.size()> output{};
    const auto end =
    ...
    Lz78RansFrameStreamingDecoder truncated{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
    EXPECT_EQ(
    ...
    Lz78RansFrameStreamingDecoder trailing{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
    ...
    Lz78RansFrameStreamingDecoder reset{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
    EXPECT_EQ(
    ...
    Lz78RansFrameStreamingDecoder unknown{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
...
TEST(Lz78RansFrameStreamingDecoder, HandlesEmptyFlushAndPrematureEnd) {
    const auto empty_encoded = encoded_stream({});
    Lz78RansFrameStreamingDecoder empty{{}, {}, {}, {}, {}, {}};
    EXPECT_EQ(
        empty
    ...
    Workspace workspace{};
    Lz78RansFrameStreamingDecoder starved{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
    EXPECT_EQ(
    ...
    const auto prefix_and_first_frame =
        lz78_rans_stream_prefix_size + first_size;
    Lz78RansFrameStreamingDecoder premature{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases};
    std::array<std::byte, 1> output{};
