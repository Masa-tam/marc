...
static void set_small_limits(marc_lzmw_rans_config* config) {
...
int main(void) {
    ...
    assert(marc_lzmw_rans_config_init(MARC_DIRECTION_ENCODE, &config)
           == MARC_STATUS_OK);
    ...
    assert(marc_lzmw_rans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 2);
    assert(needed.secondary_bytes == 2216);
    ...
    assert(marc_lzmw_rans_config_init(MARC_DIRECTION_DECODE, &config)
           == MARC_STATUS_OK);
    set_small_limits(&config);
    assert(marc_lzmw_rans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
