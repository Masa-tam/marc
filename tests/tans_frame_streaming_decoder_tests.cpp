...
marc::frame::StreamHeader config() {
  ...
  s.entropy_algorithm = marc::frame::EntropyAlgorithm::tans;
  s.entropy_variant = 1;
  s.frame_size = 4;
...
std::vector<std::byte> encoded() {
  auto s = config();
  auto p = marc::frame::plan_tans_stream(s, {}, raw);
  std::vector<std::byte> v(p.serialized_size);
  EXPECT_EQ(marc::frame::encode_tans_stream(s, {}, raw, v).error,
            marc::frame::TansStreamCodecError::none);
  return v;
}
TEST(TansFrameStreamingDecoder, OneByteInputAndOutput) {
  auto in = encoded();
  std::array<std::byte, 1200> frame{};
  std::array<std::byte, 4> decoded{};
  std::array<marc::entropy::internal::TansBlockView, 2> views{};
  marc::frame::TansFrameStreamingDecoder d{{}, frame, decoded, views};
...
TEST(TansFrameStreamingDecoder,
...
TEST(TansFrameStreamingDecoder, ReportsWorkspaceAndTruncation) {
  auto in = encoded();
  std::array<std::byte, 1> short_frame{};
  std::array<std::byte, 4> decoded{};
  std::array<marc::entropy::internal::TansBlockView, 2> views{};
  marc::frame::TansFrameStreamingDecoder d{{}, short_frame, decoded, views};
  auto r = d.process(
  ...
  marc::frame::TansFrameStreamingDecoder o{{}, frame, short_decoded, views};
  r = o.process(in, {},
                marc::core::flag_value(marc::core::ProcessFlags::end_input));
  EXPECT_EQ(r.status, marc::core::StreamStatus::error);
  EXPECT_EQ(r.error.code, marc::core::ErrorCode::out_of_memory);

  marc::frame::TansFrameStreamingDecoder t{{}, frame, decoded, views};
  ...
  marc::frame::TansFrameStreamingDecoder v{{}, frame, decoded, short_views};
