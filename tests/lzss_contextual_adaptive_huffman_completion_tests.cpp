...
constexpr std::size_t maximum_payload_size =
    (test_frame_size * 267 + 7) / 8;
constexpr std::size_t maximum_frame_size =
    64 + 16 + maximum_payload_size;
...
[[nodiscard]] marc_lzss_contextual_adaptive_huffman_config config(
...
[[nodiscard]] Workspace workspace_for(
    const marc_lzss_contextual_adaptive_huffman_config& settings) {
    Workspace result{};
    EXPECT_EQ(
...
TEST(LzssContextualAdaptiveHuffmanCompletion,
