...
int main(void) {
    ...
    assert(marc_lz77_rans_config_init(MARC_DIRECTION_ENCODE, &config)
           == MARC_STATUS_OK);
    ...
    assert(marc_lz77_rans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == sizeof(input));
    assert(needed.secondary_bytes == 4032);
    ...
    assert(marc_lz77_rans_config_init(MARC_DIRECTION_DECODE, &config)
           == MARC_STATUS_OK);
