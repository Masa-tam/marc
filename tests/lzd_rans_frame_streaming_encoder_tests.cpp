...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lzd_rans_stream_prefix_size);
...
TEST(LzdRansFrameStreamingEncoder, MatchesReferenceWithOneByteBuffers) {
...
TEST(LzdRansFrameStreamingEncoder, FlushKeepsCanonicalFramesAndStreamOpen) {
    const auto expected = reference();
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
    ...
    EXPECT_EQ(first.output_produced, lzd_rans_stream_prefix_size);
...
TEST(LzdRansFrameStreamingEncoder, RetainsEndInputWhileAllRegionsDrain) {
    const auto expected = reference();
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
...
TEST(LzdRansFrameStreamingEncoder, ReportsWorkspaceAndAggregateErrors) {
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
    std::array<marc::dictionary::internal::LzdEncoderEntry, 1> entries{};
    ...
    LzdRansFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    LzdRansFrameStreamingEncoder short_dictionary{
    ...
    EXPECT_EQ(short_dictionary.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzdRansFrameStreamingEncoder short_entries{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, {}};
    EXPECT_EQ(short_entries.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzdRansFrameStreamingEncoder short_encoded{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
    ...
    const auto plan = plan_lzd_rans_frame(
        ...
        dictionary_staging);
    ASSERT_EQ(plan.error, LzdRansFrameValidationError::none);
    ...
    limits.max_internal_buffered_bytes =
        ...
        - 1;
    LzdRansFrameStreamingEncoder aggregate_limited{
...
TEST(LzdRansFrameStreamingEncoder, HandlesEmptyAndProtocolErrors) {
    std::array<std::byte, 1> unused{};
    std::array<std::byte, lzd_rans_stream_prefix_size> output{};
    LzdRansFrameStreamingEncoder empty{
        config(0), {}, {}, {}, {}, unused, {}};
    auto result = empty.process(
        {}, output,
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
    EXPECT_EQ(result.status, marc::core::StreamStatus::end_of_stream);
    EXPECT_EQ(result.output_produced, lzd_rans_stream_prefix_size);
    ...
    std::array<std::byte, 4096> frame_encoded{};
    std::array<marc::dictionary::internal::LzdEncoderEntry, 1> entries{};
    LzdRansFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    ...
    LzdRansFrameStreamingEncoder excess{
    ...
    LzdRansFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    ...
    LzdRansFrameStreamingEncoder unknown{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
