...
[[nodiscard]] std::vector<std::byte> encoded_stream(
    ...
    std::vector<std::byte> encoded(lzmw_dynamic_range_stream_prefix_size);
...
[[nodiscard]] std::size_t first_frame_size(
    ...
    const std::span<const std::byte, frame_header_size> bytes{
        encoded.data() + lzmw_dynamic_range_stream_prefix_size,
        frame_header_size};
...
TEST(LzmwDynamicRangeFrameStreamingDecoder,
    ...
    LzmwDynamicRangeFrameStreamingDecoder decoder{
        {}, frame_encoded, staging, decoded, phrases, expansion};
...
TEST(LzmwDynamicRangeFrameStreamingDecoder,
    ...
    const auto second = lzmw_dynamic_range_stream_prefix_size
...
TEST(LzmwDynamicRangeFrameStreamingDecoder,
    ...
    std::array<marc::dictionary::internal::LzmwPhraseEntry, 1> phrases{};
    std::array<std::uint32_t, 2> expansion{};
    LzmwDynamicRangeFrameStreamingDecoder trailing_decoder{
        {}, frame_encoded, staging, decoded, phrases, expansion};
    std::array<std::byte, raw.size()> output{};
    ...
    LzmwDynamicRangeFrameStreamingDecoder empty_decoder{
        {}, frame_encoded, staging, decoded, phrases, expansion};
    const auto empty = empty_decoder.process(
        empty_encoded, {},
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
    EXPECT_EQ(empty.status, marc::core::StreamStatus::end_of_stream);
    EXPECT_EQ(empty.input_consumed, lzmw_dynamic_range_stream_prefix_size);
}
...
TEST(LzmwDynamicRangeFrameStreamingDecoder,
    ...
    LzmwDynamicRangeFrameStreamingDecoder short_frame{
    ...
    LzmwDynamicRangeFrameStreamingDecoder short_references{
        {}, frame_encoded, std::span<std::byte>{staging}.first<7>(), decoded,
        phrases, expansion};
    EXPECT_EQ(short_references.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzmwDynamicRangeFrameStreamingDecoder short_raw{
        {}, frame_encoded, staging,
        std::span<std::byte>{decoded}.first<1>(), phrases, expansion};
    EXPECT_EQ(short_raw.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzmwDynamicRangeFrameStreamingDecoder short_phrases{
        {}, frame_encoded, staging, decoded, {}, expansion};
    EXPECT_EQ(short_phrases.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzmwDynamicRangeFrameStreamingDecoder short_expansion{
        {}, frame_encoded, staging, decoded, phrases, {}};
    EXPECT_EQ(short_expansion.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    ...
    const auto expansion_count =
        marc::dictionary::internal::lzmw_expansion_workspace_entries(
            phrase_count, first.uncompressed_size != 0);
    const std::uint64_t aggregate = frame_header_size
        ...
        + expansion_count * sizeof(std::uint32_t);
    auto limits = marc::core::DecoderLimits{};
    limits.max_block_size = 16;
    limits.max_internal_buffered_bytes = aggregate - 1;
    LzmwDynamicRangeFrameStreamingDecoder limited{
        limits, frame_encoded, staging, decoded, phrases, expansion};
    EXPECT_EQ(limited.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::limit_exceeded);

    LzmwDynamicRangeFrameStreamingDecoder reset{
        {}, frame_encoded, staging, decoded, phrases, expansion};
    ...
    LzmwDynamicRangeFrameStreamingDecoder unknown{
        {}, frame_encoded, staging, decoded, phrases, expansion};
