...
#ifndef MARC_LZW_COMPLETION_MAXIMUM_FRAME
#define MARC_LZW_COMPLETION_MAXIMUM_FRAME(dictionary_size) \
    (56 + 16 + MARC_LZW_COMPLETION_MAXIMUM_PAYLOAD(dictionary_size))
#endif
#ifndef MARC_LZW_COMPLETION_CONFIGURE_PROFILE
#define MARC_LZW_COMPLETION_CONFIGURE_PROFILE(config) ((void)0)
#endif
constexpr std::size_t maximum_frame_size =
    MARC_LZW_COMPLETION_MAXIMUM_FRAME(maximum_dictionary_size);
...
[[nodiscard]] marc_lzw_adaptive_huffman_config config(
    const marc_direction direction, const std::size_t original_size) {
    marc_lzw_adaptive_huffman_config result{};
    EXPECT_EQ(marc_lzw_adaptive_huffman_config_init(direction, &result),
              MARC_STATUS_OK);
    result.original_size = original_size;
    result.frame_size = test_frame_size;
    result.maximum_code_width = 16;
    ...
    result.max_dictionary_entries = UINT32_C(1) << 16;
    MARC_LZW_COMPLETION_CONFIGURE_PROFILE(result);
    return result;
}

[[nodiscard]] Workspace workspace_for(
    const marc_lzw_adaptive_huffman_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lzw_adaptive_huffman_workspace_requirements(
...
[[nodiscard]] Transform create(
    ...
    Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lzw_adaptive_huffman_create(
                  &settings,
...
TEST(LzwAdaptiveHuffmanCompletion,
...
TEST(LzwAdaptiveHuffmanCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(LzwAdaptiveHuffmanCompletion, MalformedFinalFrameIsNeverCommitted) {
