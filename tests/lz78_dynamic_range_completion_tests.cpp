...
constexpr std::size_t maximum_dictionary_size = test_frame_size * 8;
constexpr std::size_t maximum_payload_size =
    maximum_dictionary_size * 2 + 5;
constexpr std::size_t maximum_frame_size =
    56 + 16 + maximum_payload_size;
...
[[nodiscard]] marc_lz78_dynamic_range_config config(
    const marc_direction direction, const std::size_t original_size) {
    marc_lz78_dynamic_range_config result{};
    EXPECT_EQ(marc_lz78_dynamic_range_config_init(direction, &result),
              MARC_STATUS_OK);
    result.original_size = original_size;
    result.frame_size = test_frame_size;
    result.maximum_entries = test_frame_size;
    ...
    result.max_dictionary_entries = test_frame_size;
    return result;
}

[[nodiscard]] Workspace workspace_for(
    const marc_lz78_dynamic_range_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lz78_dynamic_range_workspace_requirements(
...
[[nodiscard]] Transform create(
    ...
    Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lz78_dynamic_range_create(
                  &settings,
...
[[nodiscard]] std::vector<std::uint8_t> process_all(
    ...
    for (std::size_t call = 0;
        ...
        const auto available = std::min(
            output_chunk, output.size() - produced);
...
TEST(Lz78DynamicRangeCompletion,
...
TEST(Lz78DynamicRangeCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(Lz78DynamicRangeCompletion, MalformedFinalFrameIsNeverCommitted) {
