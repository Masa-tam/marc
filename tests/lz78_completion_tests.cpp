...
constexpr std::size_t maximum_payload_size = test_frame_size * 8;
constexpr std::size_t maximum_dictionary_entries = test_frame_size;
...
marc_lz78_config config(const marc_direction direction,
                        const std::size_t original_size) {
    marc_lz78_config result{};
    EXPECT_EQ(marc_lz78_config_init(direction, &result), MARC_STATUS_OK);
    result.original_size = original_size;
    result.frame_size = test_frame_size;
    result.maximum_entries = maximum_dictionary_entries;
...
Workspace workspace_for(const marc_lz78_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lz78_workspace_requirements(&settings, &result.requirements),
              MARC_STATUS_OK);
    EXPECT_NE(result.requirements.views_alignment, 0U);
    if (settings.direction == MARC_DIRECTION_ENCODE
        && settings.original_size == 0) {
...
Transform create(const marc_lz78_config& settings, Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lz78_create(
                  &settings,
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 80 + input.size() * 8 + frames * 56;
...
TEST(Lz78Completion, RequiredDataClassesRoundTripDeterministically) {
    expect_round_trip({});
...
TEST(Lz78Completion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(Lz78Completion, MalformedFinalFrameIsNeverCommitted) {
