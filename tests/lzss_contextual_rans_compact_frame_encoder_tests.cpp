...
TEST(LzssContextualRansCompactFrameEncoder,
    ...
    auto result = plan_lzss_contextual_rans_compact_frame(
    ...
    std::vector<std::byte> output(result.serialized_size + 1,
                                  std::byte{0xcc});
    result = encode_lzss_contextual_rans_compact_frame(
...
TEST(LzssContextualRansCompactFrameEncoder, CompleteDecoderRecoversLiteral) {
    constexpr std::array raw{std::byte{'A'}};
    const auto stream = stream_for(raw.size());
    std::array<LzssTypedToken, 1> encode_tokens{};
    std::vector<std::byte> frame(98);
    ASSERT_EQ(encode_lzss_contextual_rans_compact_frame(
...
TEST(LzssContextualRansCompactFrameEncoder,
    ...
    const auto plan = plan_lzss_contextual_rans_compact_frame(
...
TEST(LzssContextualRansCompactFrameEncoder,
    ...
    auto result = encode_lzss_contextual_rans_compact_frame(
        stream, {}, 0, 0, raw,
...
TEST(LzssContextualRansCompactFrameEncoder,
    ...
    stream = stream_for(2);
    result = plan_lzss_contextual_rans_compact_frame(
        stream, {}, 0, 0, raw, tokens);
    EXPECT_EQ(result.error,
