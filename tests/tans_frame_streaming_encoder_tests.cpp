...
[[nodiscard]] std::vector<std::byte> reference_encode() {
    ...
    const auto plan = marc::frame::plan_tans_stream(stream, {}, input);
    EXPECT_EQ(plan.error, marc::frame::TansStreamCodecError::none);
    std::vector<std::byte> output(plan.serialized_size);
    EXPECT_EQ(marc::frame::encode_tans_stream(stream, {}, input, output).error,
              marc::frame::TansStreamCodecError::none);
    return output;
}

TEST(TansFrameStreamingEncoder, MatchesOracleWithOneByteBuffers) {
    const auto expected = reference_encode();
    std::array<std::byte, 4> frame_input{};
    std::array<std::byte, 1200> frame_encoded{};
    marc::frame::TansFrameStreamingEncoder encoder{
...
TEST(TansFrameStreamingEncoder, EmitsFullFrameAndKeepsFlushOpen) {
    const auto expected = reference_encode();
    std::array<std::byte, 4> frame_input{};
    std::array<std::byte, 1200> frame_encoded{};
    marc::frame::TansFrameStreamingEncoder encoder{
...
TEST(TansFrameStreamingEncoder, ReportsWorkspaceLimitsAndPrematureEnd) {
    std::array<std::byte, 4> frame_input{};
    std::array<std::byte, 1> short_frame{};
    std::array<std::byte, 2048> output{};
    marc::frame::TansFrameStreamingEncoder short_encoder{
    ...
    std::array<std::byte, 1200> frame_encoded{};
    marc::frame::TansFrameStreamingEncoder limited{
    ...
    marc::frame::TansFrameStreamingEncoder premature{
        stream_for(input.size()), {}, frame_input, frame_encoded};
...
TEST(TansFrameStreamingEncoder, HandlesEmptyAndEndedCalls) {
