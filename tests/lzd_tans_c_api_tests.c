...
static void set_small_limits(marc_lzd_tans_config* config) {
...
int main(void) {
    ...
    assert(marc_lzd_tans_config_init(MARC_DIRECTION_ENCODE, &config)
           == MARC_STATUS_OK);
    ...
    assert(marc_lzd_tans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    ...
    assert(marc_lzd_tans_config_init(MARC_DIRECTION_DECODE, &config)
           == MARC_STATUS_OK);
    set_small_limits(&config);
    assert(marc_lzd_tans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
