...
[[nodiscard]] marc_lzss_contextual_rans_config settings(
    const marc_direction direction) {
    marc_lzss_contextual_rans_config result{};
    EXPECT_EQ(marc_lzss_contextual_rans_config_init(direction, &result),
              MARC_STATUS_OK);
...
[[nodiscard]] std::vector<std::uint8_t> canonical_stream() {
    auto config = settings(MARC_DIRECTION_ENCODE);
...
class LzssContextualRansFuzzRegression : public testing::Test {
protected:
    LzssContextualRansFuzzRegression()
        : decode_config_(settings(MARC_DIRECTION_DECODE)),
    ...
    void expect_private_atomic_failure(
        ...
        limits.max_total_output_size = decode_config_.max_total_output_size;
        limits.max_frame_size = decode_config_.max_frame_size;
        limits.max_block_size = decode_config_.max_block_size;
        limits.max_compressed_payload_size =
            decode_config_.max_compressed_payload_size;
        limits.max_internal_buffered_bytes =
            decode_config_.max_internal_buffered_bytes;
        limits.max_lz_distance = decode_config_.max_lz_distance;
        limits.max_lz_match_length = decode_config_.max_lz_match_length;
        limits.max_entropy_table_entries =
            decode_config_.max_entropy_table_entries;
...
private:
    marc_lzss_contextual_rans_config decode_config_{};
...
TEST_F(LzssContextualRansFuzzRegression,
