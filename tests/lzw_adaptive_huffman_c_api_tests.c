...
static void set_small_limits(marc_lzw_adaptive_huffman_config* config) {
    ...
    config->max_dictionary_entries = 256;
}
...
int main(void) {
    ...
    assert(marc_lzw_adaptive_huffman_config_init(
    ...
    config.frame_size = 2;
    config.maximum_code_width = 9;
    set_small_limits(&config);
    assert(marc_lzw_adaptive_huffman_workspace_requirements(
               &config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 2);
    assert(needed.secondary_bytes == 174);
    ...
    assert(marc_lzw_adaptive_huffman_create(
