...
static void set_small_limits(marc_lzw_rans_config* config) {
    config->max_total_output_size = 1024;
    config->max_frame_size = 5;
...
int main(void) {
    ...
    assert(marc_lzw_rans_config_init(MARC_DIRECTION_ENCODE, &config)
           == MARC_STATUS_OK);
    ...
    config.frame_size = 2;
    config.entropy_block_size = 2;
    set_small_limits(&config);
    assert(marc_lzw_rans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 2);
    assert(needed.secondary_bytes == 1136);
    ...
    assert(marc_lzw_rans_config_init(MARC_DIRECTION_DECODE, &config)
           == MARC_STATUS_OK);
    set_small_limits(&config);
    assert(marc_lzw_rans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 8248);
    assert(needed.secondary_bytes == 20);
    assert(needed.views_bytes != 0 && needed.views_alignment != 0);
