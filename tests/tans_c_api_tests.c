...
int main(void) {
    ...
    assert(marc_tans_config_init(MARC_DIRECTION_ENCODE, &config) == MARC_STATUS_OK);
    config.original_size = sizeof(input);
    config.frame_size = 4;
    config.block_size = 2;
    assert(marc_tans_workspace_requirements(&config, &needed) == MARC_STATUS_OK);
    ...
    assert(marc_tans_create(&config, primary, secondary, views, &transform)
           == MARC_STATUS_OK);
    ...
    assert(marc_tans_config_init(MARC_DIRECTION_DECODE, &config) == MARC_STATUS_OK);
    ...
    config.reserved = 1;
    assert(marc_tans_workspace_requirements(&config, &needed)
