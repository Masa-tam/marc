...
std::vector<std::byte> encoded(const std::string_view text,
    ...
    EXPECT_EQ(encode_lzss_token_stream(input, parameters, {}, result).error,
              LzssEncodeError::none);
    return result;
}

TEST(LzssStreamingDecoder, DecodesOneByteInputAndOutput) {
    constexpr std::string_view raw = "ABCABCABCXAAAAAA";
    const auto input = encoded(raw);
    std::array<std::byte, raw.size()> history{};
    LzssStreamingDecoder decoder{{}, raw.size(), {}, history};
...
TEST(LzssStreamingDecoder, PreservesEndInputWhileDraining) {
    const auto input = encoded("AAAAAA");
    std::array<std::byte, 6> history{};
    LzssStreamingDecoder decoder{{}, 6, {}, history};
    ...
    EXPECT_EQ(std::vector(output.begin(), output.end()), bytes("AAAAAA"));
}

TEST(LzssStreamingDecoder, WrapsCallerOwnedHistoryRing) {
    const auto raw = bytes("ABCABCABCX");
    LzssParameters parameters{};
    parameters.window_size = 3;
    const auto input = encoded("ABCABCABCX", parameters);
    std::array<std::byte, 3> history{};
    std::array<std::byte, 10> output{};
    LzssStreamingDecoder decoder{parameters, raw.size(), {}, history};
...
TEST(LzssStreamingDecoder, ReportsTruncationTrailingDataAndBadReference) {
    auto input = encoded("AAAAAA");
    std::array<std::byte, 6> output{};
    std::array<std::byte, 6> history1{};
    LzssStreamingDecoder truncated{{}, 6, {}, history1};
    ...
    std::array<std::byte, 6> history2{};
    LzssStreamingDecoder trailing{{}, 6, {}, history2};
    ...
    input = encoded("AAAAAA");
    input[3] = std::byte{2};
    std::array<std::byte, 6> history3{};
    LzssStreamingDecoder invalid{{}, 6, {}, history3};
...
TEST(LzssStreamingDecoder, RejectsUnsupportedFlagsAndInvalidConstruction) {
    std::array<std::byte, 1> history{};
    LzssStreamingDecoder decoder{{}, 1, {}, history};
    auto result = decoder.process(
    ...
    LzssStreamingDecoder invalid{{}, 1, limits, history};
    result = invalid.process({}, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::invalid_argument);

    LzssStreamingDecoder no_history{{}, 1, {}, {}};
    result = no_history.process({}, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::out_of_memory);
}

TEST(LzssStreamingDecoder, EnforcesCumulativeSerializedLimitBeforeInput) {
    const auto input = encoded("AB");
    marc::core::DecoderLimits limits{};
    limits.max_dictionary_serialized_size = lzss_literal_size;
    std::array<std::byte, 2> history{};
    std::array<std::byte, 2> output{};
    LzssStreamingDecoder decoder{{}, 2, limits, history};
    const auto result = decoder.process(
        input, output,
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
