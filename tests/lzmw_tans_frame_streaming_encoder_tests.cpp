...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lzmw_tans_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        const auto plan = plan_lzmw_tans_frame(
            stream, {}, limits, sequence, committed, raw, entries, staging);
        EXPECT_EQ(plan.error, LzmwTansFrameValidationError::none);
        const auto offset = output.size();
        output.resize(offset + plan.serialized_size);
        EXPECT_EQ(encode_lzmw_tans_frame(
...
TEST(LzmwTansFrameStreamingEncoder, MatchesReferenceWithOneByteBuffers) {
...
TEST(LzmwTansFrameStreamingEncoder, FlushKeepsCanonicalFramesAndStreamOpen) {
    const auto expected = reference();
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
    ...
    EXPECT_EQ(first.output_produced, lzmw_tans_stream_prefix_size);
...
TEST(LzmwTansFrameStreamingEncoder, RetainsEndInputWhileAllRegionsDrain) {
    const auto expected = reference();
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
...
TEST(LzmwTansFrameStreamingEncoder, ReportsWorkspaceAndAggregateErrors) {
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
    std::array<marc::dictionary::internal::LzmwEncoderEntry, 1> entries{};
    std::array<std::byte, 8192> output{};
    LzmwTansFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    LzmwTansFrameStreamingEncoder short_dictionary{
    ...
    EXPECT_EQ(short_dictionary.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzmwTansFrameStreamingEncoder short_entries{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, {}};
    EXPECT_EQ(short_entries.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzmwTansFrameStreamingEncoder short_encoded{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
    ...
    limits.max_internal_buffered_bytes =
        ...
        + plan.encoder_entries
              * sizeof(marc::dictionary::internal::LzmwEncoderEntry)
        - 1;
    LzmwTansFrameStreamingEncoder aggregate_limited{
...
TEST(LzmwTansFrameStreamingEncoder, HandlesEmptyAndProtocolErrors) {
    std::array<std::byte, 1> unused{};
    std::array<std::byte, lzmw_tans_stream_prefix_size> output{};
    LzmwTansFrameStreamingEncoder empty{
        config(0), {}, {}, {}, {}, unused, {}};
    auto result = empty.process(
        {}, output,
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
    EXPECT_EQ(result.status, marc::core::StreamStatus::end_of_stream);
    EXPECT_EQ(result.output_produced, lzmw_tans_stream_prefix_size);

    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
    std::array<marc::dictionary::internal::LzmwEncoderEntry, 1> entries{};
    LzmwTansFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    ...
    LzmwTansFrameStreamingEncoder unknown{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
