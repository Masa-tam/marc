...
constexpr std::size_t maximum_payload_size = maximum_dictionary_size * 2 + 5;
...
[[nodiscard]] marc_lz77_dynamic_range_config config(
    const marc_direction direction, const std::size_t original_size) {
    marc_lz77_dynamic_range_config result{};
    EXPECT_EQ(marc_lz77_dynamic_range_config_init(direction, &result),
              MARC_STATUS_OK);
...
[[nodiscard]] Workspace workspace_for(
    const marc_lz77_dynamic_range_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lz77_dynamic_range_workspace_requirements(
...
[[nodiscard]] Transform create(
    ...
    Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lz77_dynamic_range_create(
                  &settings,
...
TEST(Lz77DynamicRangeCompletion,
...
TEST(Lz77DynamicRangeCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(Lz77DynamicRangeCompletion, MalformedFinalFrameIsNeverCommitted) {
