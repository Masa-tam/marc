...
[[nodiscard]] marc_lzss_contextual_blocked_huffman_config config(
    ...
    result.max_frame_size = frame_size;
    result.max_block_size = frame_size * 6;
...
[[nodiscard]] Workspace workspace_for(
    const marc_lzss_contextual_blocked_huffman_config& settings) {
    Workspace result{};
...
[[nodiscard]] std::vector<std::uint8_t> encode(
    ...
    const auto frames = input.empty() ? std::size_t{0}
        : std::size_t{1} + (input.size() - 1) / frame_size;
    return process_all(
...
void expect_round_trip(const std::span<const std::uint8_t> input) {
    ...
    EXPECT_EQ(first[14], 2U);
    EXPECT_EQ(first[16], 2U);
    EXPECT_EQ(first[18], 2U);
...
[[nodiscard]] std::size_t frame_extent(
    ...
    return 64 + load_le32(stream, offset + 32)
        + load_le32(stream, offset + 36)
        + load_le32(stream, offset + 40)
        + load_le32(stream, offset + 44);
}
...
TEST(LzssContextualBlockedHuffmanCompletion,
