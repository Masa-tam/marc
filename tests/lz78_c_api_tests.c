...
int main(void) {
    ...
    assert(marc_lz78_config_init(MARC_DIRECTION_ENCODE, &config)
           == MARC_STATUS_OK);
    assert(config.struct_size == sizeof(config));
    assert(config.maximum_entries == 65536);
    config.original_size = sizeof(input);
    config.frame_size = 3;
    assert(marc_lz78_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 3 && needed.secondary_bytes == 80);
    ...
    assert(encoded_size == 224);
    marc_transform_destroy(transform);
    free(primary.data);
    free(secondary.data);
    free(views.data);

    assert(marc_lz78_config_init(MARC_DIRECTION_DECODE, &config)
           == MARC_STATUS_OK);
    ...
    assert(marc_lz78_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes >= 72 && needed.primary_bytes <= 56 + 4096);
