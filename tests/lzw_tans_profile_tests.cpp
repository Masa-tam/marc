...
TEST(LzwTansProfile, BuildsCanonicalWorstCaseEncoderWorkspace) {
    StreamHeader stream{};
    LzwTansEncoderWorkspaceRequirements workspace{};
    ASSERT_EQ(make_lzw_tans_profile(
                  {17, 10, 4, {}}, {}, stream, workspace),
              LzwTansProfileError::none);
    ...
    EXPECT_EQ(workspace.frame_encoded_bytes, 2'736U);
...
TEST(LzwTansProfile, UsesShortFrameAndCanonicalEmptyLayout) {
    StreamHeader stream{};
    LzwTansEncoderWorkspaceRequirements workspace{};
    ASSERT_EQ(make_lzw_tans_profile(
    ...
    EXPECT_EQ(workspace.frame_encoded_bytes, 1'137U);
    EXPECT_EQ(workspace.encoder_entry_count, 6U);

    workspace = {1, 1, 1, 1, 1, 8};
    ASSERT_EQ(make_lzw_tans_profile({}, {}, stream, workspace),
              LzwTansProfileError::none);
...
TEST(LzwTansProfile, EnforcesBlockPayloadAggregateAndFrameLimits) {
    StreamHeader stream{};
    LzwTansEncoderWorkspaceRequirements workspace{};
    auto limits = marc::core::DecoderLimits{};
    limits.max_blocks_per_frame = 1;
    EXPECT_EQ(make_lzw_tans_profile(
    ...
    LzwTansProfileConfig invalid{};
    invalid.parameters.maximum_code_width = 8;
    EXPECT_EQ(make_lzw_tans_profile(invalid, {}, stream, workspace),
              LzwTansProfileError::invalid_configuration);
}

TEST(LzwTansProfile, CalculatesDecoderLayoutFromLocalLimits) {
    auto limits = marc::core::DecoderLimits{};
    ...
    LzwTansDecoderWorkspaceRequirements workspace{};
    ASSERT_EQ(calculate_lzw_tans_decoder_workspace(limits, workspace),
              LzwTansProfileError::none);
    ...
    EXPECT_EQ(calculate_lzw_tans_decoder_workspace(limits, workspace),
              LzwTansProfileError::arithmetic_overflow);
    EXPECT_EQ(workspace.frame_encoded_bytes, 0U);
}

TEST(LzwTansProfile, PartitionsEncoderAndDecoderOpaqueStorage) {
    StreamHeader stream{};
    LzwTansEncoderWorkspaceRequirements encoder{};
    ASSERT_EQ(make_lzw_tans_profile(
    ...
    LzwTansEncoderViews encoder_views{};
    ASSERT_EQ(partition_lzw_tans_encoder_views(
    ...
    EXPECT_EQ(partition_lzw_tans_encoder_views(
                  encoder,
                  encoder_storage.first(encoder_storage.size() - 1),
                  encoder_views),
              LzwTansWorkspaceError::too_small);
    ...
    if (encoder.views_alignment > 1) {
        ...
        EXPECT_EQ(partition_lzw_tans_encoder_views(
                  ...
                  LzwTansWorkspaceError::misaligned);
    }
    ...
    limits.max_blocks_per_frame = 4;
    limits.max_dictionary_entries = 300;
    LzwTansDecoderWorkspaceRequirements decoder{};
    ASSERT_EQ(calculate_lzw_tans_decoder_workspace(limits, decoder),
              LzwTansProfileError::none);
    ...
    LzwTansDecoderViews decoder_views{};
    ASSERT_EQ(partition_lzw_tans_decoder_views(
    ...
    EXPECT_EQ(partition_lzw_tans_decoder_views(
...
TEST(LzwTansProfile, MapsStableCoreErrors) {
    EXPECT_EQ(lzw_tans_profile_error_code(LzwTansProfileError::none),
              marc::core::ErrorCode::none);
    EXPECT_EQ(lzw_tans_profile_error_code(
                  LzwTansProfileError::invalid_configuration),
              marc::core::ErrorCode::invalid_argument);
    EXPECT_EQ(lzw_tans_profile_error_code(LzwTansProfileError::unsupported),
...
TEST(LzwTansProfile, RequirementsConstructStreamingRoundTrip) {
    ...
    const LzwTansProfileConfig config{raw.size(), 2, 2, {}};
    StreamHeader stream{};
    LzwTansEncoderWorkspaceRequirements encoder_ws{};
    ASSERT_EQ(make_lzw_tans_profile(
    ...
    LzwTansEncoderViews encode_views{};
    ASSERT_EQ(partition_lzw_tans_encoder_views(
    ...
    LzwTansDecoderWorkspaceRequirements decoder_ws{};
    ASSERT_EQ(calculate_lzw_tans_decoder_workspace(limits, decoder_ws),
              LzwTansProfileError::none);
    ...
    LzwTansDecoderViews decode_views{};
    ASSERT_EQ(partition_lzw_tans_decoder_views(
