...
constexpr std::array rans_payload{
    std::byte{0x00}, std::byte{0xa5}, std::byte{0x22}, std::byte{0x10},
...
TEST(Lz77RansVector, EmitsIndependentSingleLiteralFrame) {
    ...
    EXPECT_EQ(descriptor.frequencies[0], 3840);
    EXPECT_EQ(descriptor.frequencies[0x41], 256);
    ...
    ASSERT_EQ(marc::entropy::internal::serialize_rans_descriptor(
                  descriptor, literal_a_token.size(), rans_payload.size(),
                  limits,
                  std::span<std::byte,
                            marc::entropy::internal::rans_descriptor_size>{
                      frame.data() + marc::frame::frame_header_size,
