...
std::vector<std::byte> encoded(const std::string_view text) {
    ...
    std::vector<LzdEncoderEntry> workspace(
        lzd_encoder_workspace_entries(raw.size(), {}));
    const auto plan = plan_lzd_token_stream(raw, {}, {}, workspace);
...
struct DecoderStorage {
    explicit DecoderStorage(const std::size_t raw_size) {
        requirements = lzd_streaming_decoder_workspace_requirements(
            raw_size, {});
    ...
    LzdStreamingDecoderWorkspaceRequirements requirements{};
...
LzdStreamingDecoder make_decoder(
...
TEST(LzdStreamingDecoder, ReportsConservativeWorkspaceRequirements) {
    const auto empty = lzd_streaming_decoder_workspace_requirements(0, {});
    ...
    const auto one = lzd_streaming_decoder_workspace_requirements(1, {});
    EXPECT_TRUE(one.supported);
    EXPECT_EQ(one.encoded_bytes, 8U);
    EXPECT_EQ(one.phrase_entries, 1U);
    EXPECT_EQ(one.expansion_entries, 2U);
    EXPECT_EQ(one.decoded_bytes, 1U);

    LzdParameters frozen{};
    frozen.maximum_entries = 1;
    const auto ten = lzd_streaming_decoder_workspace_requirements(10, frozen);
    ...
    EXPECT_FALSE(lzd_streaming_decoder_workspace_requirements(
                     UINT64_MAX, {}).supported);
}

TEST(LzdStreamingDecoder, DecodesOneByteInputAndOutput) {
...
TEST(LzdStreamingDecoder, PreservesEndInputWhileDraining) {
...
TEST(LzdStreamingDecoder, AcceptsEndInputWithZeroFinalBytesAndEmptyFrame) {
...
TEST(LzdStreamingDecoder, RejectsMalformedFrameWithoutPublishing) {
    ...
    EXPECT_EQ(result.error.byte_position, lzd_token_size);
...
TEST(LzdStreamingDecoder, RejectsExcessInputBeforeConsumption) {
    DecoderStorage storage{1};
    auto decoder = make_decoder(1, storage);
    std::array<std::byte, lzd_token_size + 1> input{};
    std::array<std::byte, 1> output{};
    const auto result = decoder.process(
        input, output,
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
...
TEST(LzdStreamingDecoder, RejectsUnsupportedFlagsAndBadConstruction) {
    DecoderStorage storage{1};
    auto decoder = make_decoder(1, storage);
    auto result = decoder.process(
        {}, {},
    ...
    LzdStreamingDecoder short_encoded{
        {}, 1, {}, {}, storage.phrases, storage.expansion, storage.decoded};
    result = short_encoded.process({}, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::out_of_memory);
    LzdStreamingDecoder short_phrases{
        {}, 1, {}, storage.encoded, {}, storage.expansion, storage.decoded};
    result = short_phrases.process({}, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::out_of_memory);
    LzdStreamingDecoder short_expansion{
        {}, 1, {}, storage.encoded, storage.phrases, {}, storage.decoded};
    result = short_expansion.process({}, {}, 0);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::out_of_memory);
    LzdStreamingDecoder short_decoded{
        {}, 1, {}, storage.encoded, storage.phrases, storage.expansion, {}};
    ...
    limits.max_internal_buffered_bytes =
        ...
        + storage.requirements.phrase_entries * sizeof(LzdPhraseEntry)
    ...
    auto limited = make_decoder(1, storage, limits);
