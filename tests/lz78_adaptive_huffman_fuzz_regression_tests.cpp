...
TEST(Lz78AdaptiveHuffmanFuzzRegression, EveryCanonicalTruncationIsAtomic) {
    const auto encoded = canonical_stream();
    for (std::size_t size = 0; size < encoded.size(); ++size)
        expect_atomic_failure(std::span<const std::byte>{encoded}.first(size));
}

TEST(Lz78AdaptiveHuffmanFuzzRegression, ExtremeFrameLengthsAreAtomic) {
...
TEST(Lz78AdaptiveHuffmanFuzzRegression, InvalidDescriptorIsAtomic) {
    auto encoded = canonical_stream();
