...
[[nodiscard]] std::vector<std::byte> encoded_stream(
    ...
    std::vector<std::byte> encoded(lzw_dynamic_range_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        const auto plan = plan_lzw_dynamic_range_frame(
            ...
            staging);
        EXPECT_EQ(plan.error, LzwDynamicRangeFrameValidationError::none);
        const auto offset = encoded.size();
        encoded.resize(offset + plan.serialized_size);
        EXPECT_EQ(encode_lzw_dynamic_range_frame(
...
[[nodiscard]] std::size_t first_frame_size(
    ...
    const std::span<const std::byte, frame_header_size> bytes{
        encoded.data() + lzw_dynamic_range_stream_prefix_size,
        frame_header_size};
...
TEST(LzwDynamicRangeFrameStreamingDecoder,
    ...
    LzwDynamicRangeFrameStreamingDecoder decoder{
...
TEST(LzwDynamicRangeFrameStreamingDecoder,
    ...
    const auto second = lzw_dynamic_range_stream_prefix_size
...
TEST(LzwDynamicRangeFrameStreamingDecoder,
    ...
    std::array<std::byte, 2> decoded{};
    std::array<marc::dictionary::internal::LzwPhraseEntry, 1> phrases{};
    LzwDynamicRangeFrameStreamingDecoder trailing_decoder{
        {}, frame_encoded, staging, decoded, phrases};
    std::array<std::byte, raw.size()> output{};
    ...
    LzwDynamicRangeFrameStreamingDecoder empty_decoder{
        {}, frame_encoded, staging, decoded, phrases};
    const auto empty = empty_decoder.process(
        empty_encoded, {},
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
    EXPECT_EQ(empty.status, marc::core::StreamStatus::end_of_stream);
    EXPECT_EQ(empty.input_consumed, lzw_dynamic_range_stream_prefix_size);
}
...
TEST(LzwDynamicRangeFrameStreamingDecoder,
    ...
    LzwDynamicRangeFrameStreamingDecoder short_frame{
    ...
    EXPECT_EQ(short_raw.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzwDynamicRangeFrameStreamingDecoder short_phrases{
    ...
    LzwDynamicRangeFrameStreamingDecoder limited{
    ...
    LzwDynamicRangeFrameStreamingDecoder reset{
        {}, frame_encoded, staging, decoded, phrases};
    ...
    LzwDynamicRangeFrameStreamingDecoder unknown{
        {}, frame_encoded, staging, decoded, phrases};
