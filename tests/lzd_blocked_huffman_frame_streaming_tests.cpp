...
[[nodiscard]] std::vector<std::byte> encoded_stream() {
    ...
    std::vector<std::byte> encoded(lzd_blocked_huffman_stream_prefix_size);
    const std::span<std::byte, stream_header_size> header{
        encoded.data(), stream_header_size};
    const std::span<std::byte, marc::dictionary::internal::lzd_parameter_size>
    ...
    for (std::size_t offset = 0; offset < raw.size(); offset += 2) {
        ...
        const auto plan = plan_lzd_blocked_huffman_frame(
            stream, {}, {}, sequence, offset, input, entries, staging);
        EXPECT_EQ(plan.error, LzdBlockedHuffmanFrameValidationError::none);
        const auto original_size = encoded.size();
        encoded.resize(original_size + plan.serialized_size);
        EXPECT_EQ(encode_lzd_blocked_huffman_frame(
...
TEST(LzdBlockedHuffmanFrameStreaming, ProfileWorkspacesAreDirectlyUsable) {
    ...
    StreamHeader stream{};
    LzdBlockedHuffmanEncoderWorkspaceRequirements encoder_requirements{};
    const LzdBlockedHuffmanProfileConfig profile_config{
        raw.size(), 2, 4, {}};
    ASSERT_EQ(make_lzd_blocked_huffman_profile(
    ...
    auto opaque = aligned_storage(
        ...
        encoder_requirements.views_alignment);
    LzdBlockedHuffmanEncoderViews encoder_views{};
    ASSERT_EQ(partition_lzd_blocked_huffman_encoder_views(
                  encoder_requirements, opaque, encoder_views),
              LzdBlockedHuffmanWorkspaceError::none);
    LzdBlockedHuffmanFrameStreamingEncoder encoder{
    ...
    LzdBlockedHuffmanDecoderWorkspaceRequirements decoder_requirements{};
    ASSERT_EQ(calculate_lzd_blocked_huffman_decoder_workspace(
    ...
    auto decoder_opaque = aligned_storage(
        ...
        decoder_requirements.views_alignment);
    LzdBlockedHuffmanDecoderViews decoder_views{};
    ASSERT_EQ(partition_lzd_blocked_huffman_decoder_views(
                  decoder_requirements, decoder_opaque, decoder_views),
              LzdBlockedHuffmanWorkspaceError::none);
    LzdBlockedHuffmanFrameStreamingDecoder decoder{
...
TEST(LzdBlockedHuffmanFrameStreaming, OneByteEncoderMatchesFrameOracle) {
...
TEST(LzdBlockedHuffmanFrameStreaming, DecoderHandlesOneByteBoundaries) {
...
TEST(LzdBlockedHuffmanFrameStreaming,
...
TEST(LzdBlockedHuffmanFrameStreaming,
    ...
    LzdBlockedHuffmanFrameStreamingDecoder short_phrases{
    ...
    LzdBlockedHuffmanFrameStreamingDecoder reset{
        {}, frame_encoded, staging, decoded, views, phrases, expansion};
    EXPECT_EQ(reset
...
TEST(LzdBlockedHuffmanFrameStreaming, EmptyStreamAndPrematureEndAreExact) {
    std::array<std::byte, lzd_blocked_huffman_stream_prefix_size> prefix{};
    std::array<std::byte, 1> unused{};
    LzdBlockedHuffmanFrameStreamingEncoder empty_encoder{
        config(0), {}, {}, {}, {}, unused, {}};
    ...
    LzdBlockedHuffmanFrameStreamingDecoder empty_decoder{
        {}, {}, {}, {}, {}, {}, {}};
    ...
    std::array<std::byte, 96> frame_encoded{};
    std::array<marc::dictionary::internal::LzdEncoderEntry, 1> entries{};
    LzdBlockedHuffmanFrameStreamingEncoder premature{
        config(raw.size()), {}, {}, frame_input, staging, frame_encoded,
        entries};
    EXPECT_EQ(premature
...
TEST(LzdBlockedHuffmanFrameStreaming, FlushPreservesPartialInput) {
    const auto expected = encoded_stream();
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> staging{};
    std::array<std::byte, 96> frame_encoded{};
...
TEST(LzdBlockedHuffmanFrameStreaming, EnforcesFrameAggregateLimits) {
    auto limits = marc::core::DecoderLimits{};
    limits.max_total_output_size = raw.size();
    limits.max_frame_size = 2;
    limits.max_block_size = 4;
