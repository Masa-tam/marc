...
static void test_apply_profile(void) {
    ...
    static const uint64_t payloads[] = {
        ...
        UINT64_C(234881032), UINT64_C(1073741832)};
    ...
    snapshot = config;
    assert(marc_lzss_contextual_rans_config_apply_profile(
               &config, UINT32_C(5)) == MARC_STATUS_INVALID_ARGUMENT);
    assert(memcmp(&config, &snapshot, sizeof(config)) == 0);
    assert(marc_lzss_contextual_rans_config_apply_profile(
...
static void test_sixteen_mib_public_boundary(void) {
    ...
    assert(marc_lzss_contextual_rans_config_apply_profile(
           ...
           == MARC_STATUS_OK);
#if SIZE_MAX > UINT32_MAX
    config.max_internal_buffered_bytes = UINT64_C(453755912);
    assert(marc_lzss_contextual_rans_workspace_requirements(
               &config, &needed) == MARC_STATUS_LIMIT_EXCEEDED);
    config.max_internal_buffered_bytes = UINT64_C(453755913);
#endif
    assert(marc_lzss_contextual_rans_workspace_requirements(
               &config, &needed) == MARC_STATUS_OK);
#if SIZE_MAX > UINT32_MAX
    assert(needed.primary_bytes == UINT64_C(234890249));
    assert(needed.secondary_bytes == UINT64_C(16777216));
    assert(needed.views_bytes == UINT64_C(202088448));
#endif
}
...
static void test_sixty_four_mib_public_boundary(void) {
    ...
    config.match_finder_strategy =
        MARC_LZSS_MATCH_FINDER_BINARY_TREE_EXACT;
    config.max_internal_buffered_bytes = UINT64_C(3892323368);
    assert(marc_lzss_contextual_rans_workspace_requirements(
               &config, &needed) == MARC_STATUS_LIMIT_EXCEEDED);
    config.max_internal_buffered_bytes = UINT64_C(3892323369);
    assert(marc_lzss_contextual_rans_workspace_requirements(
               &config, &needed) == MARC_STATUS_OK);
    assert(needed.primary_bytes == UINT64_C(67108864));
    assert(needed.secondary_bytes == UINT64_C(1073751081));
    assert(needed.views_bytes == UINT64_C(2751463424));
    ...
    assert(marc_lzss_contextual_rans_config_apply_profile(
               &config, MARC_LZSS_CONTEXTUAL_PROFILE_64M)
           == MARC_STATUS_OK);
    config.max_internal_buffered_bytes = UINT64_C(1946928168);
    assert(marc_lzss_contextual_rans_workspace_requirements(
               &config, &needed) == MARC_STATUS_LIMIT_EXCEEDED);
    config.max_internal_buffered_bytes = UINT64_C(1946928169);
    assert(marc_lzss_contextual_rans_workspace_requirements(
               &config, &needed) == MARC_STATUS_OK);
    assert(needed.primary_bytes == UINT64_C(1073751081));
    assert(needed.secondary_bytes == UINT64_C(67108864));
    assert(needed.views_bytes == UINT64_C(806068224));
#endif
}
