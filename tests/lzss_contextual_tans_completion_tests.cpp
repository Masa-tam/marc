...
[[nodiscard]] marc_lzss_contextual_tans_config config(
    ...
    result.max_block_size = test_frame_size * 6;
    result.max_compressed_payload_size = test_frame_size * 9 + 2;
...
[[nodiscard]] Workspace workspace_for(
    const marc_lzss_contextual_tans_config& settings) {
    Workspace result{};
...
TEST(LzssContextualTansCompletion,
...
TEST(LzssContextualTansCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(LzssContextualTansCompletion, MalformedFinalFrameIsNeverCommitted) {
