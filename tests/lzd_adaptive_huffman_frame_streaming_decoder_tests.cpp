...
[[nodiscard]] std::vector<std::byte> encoded_stream(
    ...
    std::vector<std::byte> encoded(lzd_adaptive_huffman_stream_prefix_size);
...
[[nodiscard]] std::size_t first_frame_size(
    ...
    const std::span<const std::byte, frame_header_size> bytes{
        encoded.data() + lzd_adaptive_huffman_stream_prefix_size,
        frame_header_size};
...
TEST(LzdAdaptiveHuffmanFrameStreamingDecoder,
...
TEST(LzdAdaptiveHuffmanFrameStreamingDecoder,
    ...
    const auto second = lzd_adaptive_huffman_stream_prefix_size
...
TEST(LzdAdaptiveHuffmanFrameStreamingDecoder,
    ...
    std::array<marc::dictionary::internal::LzdPhraseEntry, 1> phrases{};
    std::array<std::uint32_t, 2> expansion{};
    LzdAdaptiveHuffmanFrameStreamingDecoder trailing_decoder{
        {}, frame_encoded, staging, decoded, phrases, expansion};
    std::array<std::byte, raw.size()> output{};
    ...
    LzdAdaptiveHuffmanFrameStreamingDecoder empty_decoder{
        {}, frame_encoded, staging, decoded, phrases, expansion};
...
TEST(LzdAdaptiveHuffmanFrameStreamingDecoder,
    ...
    LzdAdaptiveHuffmanFrameStreamingDecoder short_frame{
    ...
    EXPECT_EQ(short_raw.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzdAdaptiveHuffmanFrameStreamingDecoder short_phrases{
        {}, frame_encoded, staging, decoded, {}, expansion};
    EXPECT_EQ(short_phrases.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzdAdaptiveHuffmanFrameStreamingDecoder short_expansion{
    ...
    LzdAdaptiveHuffmanFrameStreamingDecoder limited{
    ...
    LzdAdaptiveHuffmanFrameStreamingDecoder reset{
        {}, frame_encoded, staging, decoded, phrases, expansion};
    ...
    LzdAdaptiveHuffmanFrameStreamingDecoder unknown{
        {}, frame_encoded, staging, decoded, phrases, expansion};
