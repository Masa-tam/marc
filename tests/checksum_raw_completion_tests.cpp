...
constexpr std::size_t serialized_frame_size = 56 + test_frame_size + 4;
...
marc_checksum_raw_config config(const marc_direction direction,
                                const std::size_t original_size) {
    marc_checksum_raw_config result{};
    EXPECT_EQ(marc_checksum_raw_config_init(direction, &result),
              MARC_STATUS_OK);
    result.original_size = original_size;
    result.frame_size = test_frame_size;
    result.max_total_output_size = 4096;
    result.max_frame_size = test_frame_size;
    result.max_compressed_payload_size = test_frame_size;
    result.max_dictionary_serialized_size = test_frame_size;
    result.max_internal_buffered_bytes = serialized_frame_size;
    return result;
}

Workspace workspace_for(const marc_checksum_raw_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_checksum_raw_workspace_requirements(
                  &settings, &result.requirements),
              MARC_STATUS_OK);
    EXPECT_EQ(result.requirements.secondary_bytes, 0U);
...
Transform create(const marc_checksum_raw_config& settings,
                 Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_checksum_raw_create(
                  &settings,
                  {workspace.primary.data(), workspace.primary.size()}, &raw),
              MARC_STATUS_OK);
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 80 + input.size() + frames * 60;
...
TEST(ChecksumRawCompletion, RequiredDataClassesRoundTripDeterministically) {
    expect_round_trip({});
...
TEST(ChecksumRawCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(ChecksumRawCompletion, MalformedStreamsFailStablyAtFrameBoundaries) {
