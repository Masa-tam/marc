...
constexpr std::array rans_payload{
    std::byte{0x82}, std::byte{0x27}, std::byte{0xa1}, std::byte{0xbd},
...
expected_complete_frame() {
    ...
    expected[descriptor + 4] = std::byte{0x09};
...
TEST(LzdRansVector, EmitsIndependentTerminalTokenFrame) {
    ...
    EXPECT_EQ(descriptor.frequencies[0x00], 1536);
    EXPECT_EQ(descriptor.frequencies[0x41], 512);
    EXPECT_EQ(descriptor.frequencies[0xff], 2048);
    ...
    ASSERT_EQ(marc::entropy::internal::serialize_rans_descriptor(
                  descriptor, terminal_token_a.size(), rans_payload.size(),
                  limits,
                  std::span<std::byte,
                            marc::entropy::internal::rans_descriptor_size>{
                      frame.data() + marc::frame::frame_header_size,
