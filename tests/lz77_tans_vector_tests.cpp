...
constexpr std::array tans_payload{
    std::byte{0x0a}, std::byte{0x05}, std::byte{0x03}};
...
expected_complete_frame() {
    ...
    expected[descriptor] = std::byte{0x10};
    expected[descriptor + 4] = std::byte{0x03};
    expected[descriptor + 8] = std::byte{0x0c};
    expected[descriptor + 9] = std::byte{0x05};
    expected[descriptor + 17] = std::byte{0x0f};
    expected[descriptor + 16 + 2 * 0x41 + 1] = std::byte{0x01};
...
TEST(Lz77TansVector, EmitsIndependentSingleLiteralFrame) {
    ...
    EXPECT_EQ(descriptor.frequencies[0], 3840U);
    EXPECT_EQ(descriptor.frequencies[0x41], 256U);
    ...
    ASSERT_EQ(marc::entropy::internal::serialize_tans_descriptor(
                  descriptor, literal_a_token.size(), tans_payload.size(),
                  limits,
                  std::span<std::byte,
                            marc::entropy::internal::tans_descriptor_size>{
                      frame.data() + marc::frame::frame_header_size,
