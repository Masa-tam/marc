...
constexpr std::size_t state_size = 8;
constexpr std::size_t maximum_payload_size =
    test_frame_size + maximum_blocks * state_size;
...
marc_rans_config config(
    ...
    marc_rans_config result{};
    EXPECT_EQ(marc_rans_config_init(direction, &result), MARC_STATUS_OK);
...
Workspace workspace_for(const marc_rans_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_rans_workspace_requirements(
...
Transform create(const marc_rans_config& settings, Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_rans_create(
                  &settings,
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 64 + input.size()
...
TEST(RansCompletion, RequiredDataClassesRoundTripDeterministically) {
    expect_round_trip({});
...
TEST(RansCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(RansCompletion, MalformedFinalFrameIsNeverCommitted) {
