...
static void set_small_limits(marc_lz78_rans_config* config) {
...
int main(void) {
    ...
    assert(marc_lz78_rans_config_init(MARC_DIRECTION_ENCODE, &config)
           == MARC_STATUS_OK);
    ...
    config.frame_size = 2;
    config.entropy_block_size = 5;
    config.maximum_entries = 2;
    set_small_limits(&config);
    assert(marc_lz78_rans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 2);
    assert(needed.secondary_bytes == 2232);
    ...
    assert(marc_lz78_rans_config_init(MARC_DIRECTION_DECODE, &config)
           == MARC_STATUS_OK);
    set_small_limits(&config);
    assert(marc_lz78_rans_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 8248);
    assert(needed.secondary_bytes == 21);
    assert(needed.views_bytes != 0 && needed.views_alignment != 0);
