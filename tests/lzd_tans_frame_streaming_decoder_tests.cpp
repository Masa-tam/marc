...
TEST(LzdTansFrameStreamingDecoder, DecodesOneByteInputAndOutput) {
...
TEST(LzdTansFrameStreamingDecoder,
     CommitsOnlyFramesBeforeLaterCorruption) {
    auto encoded = encoded_stream();
    const auto first_size =
        frame_size_at(encoded, lzd_tans_stream_prefix_size);
    const auto second_offset = lzd_tans_stream_prefix_size + first_size;
...
TEST(LzdTansFrameStreamingDecoder, ReportsWorkspaceAndAggregateErrors) {
    const auto encoded = encoded_stream();
    const auto first_offset = lzd_tans_stream_prefix_size;
    ...
    LzdTansFrameStreamingDecoder short_encoded{
        {},
    ...
    LzdTansFrameStreamingDecoder short_views{
    ...
    LzdTansFrameStreamingDecoder short_dictionary{
        {}, workspace.frame_encoded, workspace.views,
    ...
    LzdTansFrameStreamingDecoder short_decoded{
        {}, workspace.frame_encoded, workspace.views,
    ...
    LzdTansFrameStreamingDecoder short_phrases{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
    ...
    LzdTansFrameStreamingDecoder short_expansion{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases,
    ...
    LzdTansFrameStreamingDecoder aggregate_limited{
...
TEST(LzdTansFrameStreamingDecoder, RejectsTruncationTrailingAndReset) {
    const auto encoded = encoded_stream();
    Workspace workspace{};
    std::array<std::byte, raw.size()> output{};
    const auto end =
    ...
    LzdTansFrameStreamingDecoder truncated{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
    ...
    LzdTansFrameStreamingDecoder trailing{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
    ...
    LzdTansFrameStreamingDecoder reset{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
    ...
    LzdTansFrameStreamingDecoder unknown{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
...
TEST(LzdTansFrameStreamingDecoder, HandlesEmptyFlushAndPrematureEnd) {
    const auto empty_encoded = encoded_stream({});
    LzdTansFrameStreamingDecoder empty{{}, {}, {}, {}, {}, {}, {}};
    ...
    Workspace workspace{};
    LzdTansFrameStreamingDecoder starved{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
    ...
    const auto prefix_and_first_frame =
        lzd_tans_stream_prefix_size + first_size;
    LzdTansFrameStreamingDecoder premature{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
    std::array<std::byte, 1> output{};
