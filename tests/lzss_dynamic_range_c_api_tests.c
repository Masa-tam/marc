...
int main(void) {
    ...
    assert(marc_lzss_dynamic_range_config_init(
    ...
    assert(marc_lzss_dynamic_range_workspace_requirements(
               &config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == sizeof(input));
    assert(needed.secondary_bytes > 149);
    ...
    assert(marc_lzss_dynamic_range_create(
