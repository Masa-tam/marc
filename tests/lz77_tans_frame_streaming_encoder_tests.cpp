...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lz77_tans_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        const auto plan = plan_lz77_tans_frame(
            stream, {}, limits, sequence, committed, raw, staging);
        EXPECT_EQ(plan.error, Lz77TansFrameValidationError::none);
        const auto offset = output.size();
        output.resize(offset + plan.serialized_size);
        EXPECT_EQ(encode_lz77_tans_frame(
                      stream, {}, limits, sequence, committed, raw, staging,
...
TEST(Lz77TansFrameStreamingEncoder, MatchesReferenceWithOneByteBuffers) {
...
TEST(Lz77TansFrameStreamingEncoder, EmitsFullFramesAndKeepsFlushOpen) {
    const auto expected = reference();
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 32> dictionary_staging{};
    std::array<std::byte, 8192> frame_encoded{};
    ...
    EXPECT_EQ(first.output_produced, lz77_tans_stream_prefix_size);
...
TEST(Lz77TansFrameStreamingEncoder, ReportsWorkspaceAndAggregateErrors) {
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 32> dictionary_staging{};
    std::array<std::byte, 8192> frame_encoded{};
    std::array<std::byte, 16384> output{};
    Lz77TansFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    Lz77TansFrameStreamingEncoder short_encoded{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
    ...
    Lz77TansFrameStreamingEncoder aggregate_limited{
...
TEST(Lz77TansFrameStreamingEncoder, HandlesEmptyAndProtocolErrors) {
    std::array<std::byte, 1> unused{};
    std::array<std::byte, lz77_tans_stream_prefix_size> output{};
    Lz77TansFrameStreamingEncoder empty{
    ...
    EXPECT_EQ(result.output_produced, lz77_tans_stream_prefix_size);

    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 32> dictionary_staging{};
    std::array<std::byte, 8192> frame_encoded{};
    Lz77TansFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded};
    ...
    Lz77TansFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded};
