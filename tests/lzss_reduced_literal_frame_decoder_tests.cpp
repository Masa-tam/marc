...
[[nodiscard]] TypedContextStreamHeader stream_for(const std::uint32_t raw) {
    ...
    stream.context_count = 24;
    stream.dictionary_variant = 8;
    stream.context_variant = 8;
    return stream;
}
...
stream_header_bytes() {
    ...
    EXPECT_TRUE(marc::core::store_le(output, 84, std::uint16_t{24}));
    EXPECT_TRUE(marc::core::store_le(output, 96, std::uint16_t{1}));
    EXPECT_TRUE(marc::core::store_le(output, 98, std::uint16_t{8}));
    return bytes;
}
...
[[nodiscard]] std::vector<std::byte> frame_for(const std::uint32_t length) {
    ...
    const auto plan = marc::entropy::internal::
        plan_lzss_reduced_literal_range_operations(
    ...
    EXPECT_TRUE(marc::core::store_le(output, 72, std::uint16_t{24}));
...
TEST(LzssReducedLiteralFrameIntegrationHeader, ParsesOnlyItsPrivateIdentity) {
    const auto limits = marc::core::DecoderLimits{};
    ...
    ASSERT_EQ(parse_lzss_reduced_literal_stream_header(
    ...
    EXPECT_EQ(parsed.context_variant, 8U);
    EXPECT_EQ(validate_lzss_reduced_literal_stream_semantics(
...
TEST(LzssReducedLiteralFrameDecoder, ReconstructsLengthBoundaries) {
    const auto limits = marc::core::DecoderLimits{};
    for (const auto length : {3U, 4U, 5U, 258U}) {
        ...
        const auto result = decode_lzss_reduced_literal_frame(
...
TEST(LzssReducedLiteralFrameDecoder, RejectsMalformedWithoutRawOutput) {
    ...
    auto result = decode_lzss_reduced_literal_frame(
    ...
    frame[48] = std::byte{1};
    result = decode_lzss_reduced_literal_frame(
        frame, context, tokens, raw);
    EXPECT_EQ(result.preflight_error,
...
TEST(LzssReducedLiteralFrameDecoder, ChecksCapacitiesOverlapAndSequence) {
    auto frame = frame_for(3);
    const auto stream = stream_for(4);
    const auto limits = marc::core::DecoderLimits{};
    const TypedContextFrameValidationContext context{stream, limits, 0, 0};
    std::array<LzssTypedToken, 2> tokens{};
    std::array<std::byte, 4> raw{};
    EXPECT_EQ(decode_lzss_reduced_literal_frame(
...
TEST(LzssReducedLiteralFrameDecoder, RejectsNoncanonicalTerminalBytesBeforeOutput) {
    auto frame = frame_for(3);
    const auto stream = stream_for(4);
    const auto limits = marc::core::DecoderLimits{};
    const TypedContextFrameValidationContext context{stream, limits, 0, 0};
    ...
    std::array<std::byte, 4> raw{};
    raw.fill(std::byte{0xcc});
    // A nearby terminal code remains in the same decoded interval, but is
    // not the encoder's canonical lower-bound representative.
    frame.back() ^= std::byte{1};
    const auto result = decode_lzss_reduced_literal_frame(frame, context, tokens, raw);
    EXPECT_EQ(result.error, LzssShortMatchFrameDecodeError::token_decode_error);
    EXPECT_EQ(result.token_decode.token_count, 2U);
    EXPECT_EQ(result.token_decode.raw_size, 4U);
    EXPECT_EQ(result.token_decode.entropy.error,
        marc::entropy::internal::ContextualDynamicRangeDecodeError::invalid_interval);
    EXPECT_EQ(result.serialized_consumed, 0U);
    EXPECT_EQ(tokens[0].literal, 0xa5);
    for (const auto byte : raw) EXPECT_EQ(byte, std::byte{0xcc});
}
