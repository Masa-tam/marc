...
constexpr std::array tans_payload{
    std::byte{0x08}, std::byte{0x03}, std::byte{0x9b}, std::byte{0x00}};
...
expected_complete_frame() {
    ...
    expected[descriptor + 4] = std::byte{0x04};
    expected[descriptor + 8] = std::byte{0x0c};
    expected[descriptor + 9] = std::byte{0x03};
    expected[descriptor + 17] = std::byte{0x06};
    expected[descriptor + 16 + 2 * 0x41 + 1] = std::byte{0x02};
    expected[descriptor + 16 + 2 * 0xff + 1] = std::byte{0x08};
...
TEST(LzdTansVector, EmitsIndependentTerminalTokenFrame) {
    ...
    EXPECT_EQ(descriptor.frequencies[0x00], 1536U);
    EXPECT_EQ(descriptor.frequencies[0x41], 512U);
    EXPECT_EQ(descriptor.frequencies[0xff], 2048U);
    ...
    ASSERT_EQ(marc::entropy::internal::serialize_tans_descriptor(
                  descriptor, terminal_token_a.size(), tans_payload.size(),
                  limits,
                  std::span<std::byte,
                            marc::entropy::internal::tans_descriptor_size>{
                      frame.data() + marc::frame::frame_header_size,
