...
static void set_small_limits(marc_lzmw_adaptive_huffman_config* config) {
    config->max_total_output_size = 1024;
    config->max_frame_size = 2;
    config->max_compressed_payload_size = 264;
...
int main(void) {
    ...
    assert(marc_lzmw_adaptive_huffman_config_init(
    ...
    assert(marc_lzmw_adaptive_huffman_workspace_requirements(
               &config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 2);
    assert(needed.secondary_bytes == 344);
    ...
    assert(marc_lzmw_adaptive_huffman_create(
