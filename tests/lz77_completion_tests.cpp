...
constexpr std::size_t maximum_payload_size = test_frame_size * 16;
...
marc_lz77_config config(const marc_direction direction,
                        const std::size_t original_size) {
    marc_lz77_config result{};
    EXPECT_EQ(marc_lz77_config_init(direction, &result), MARC_STATUS_OK);
...
Workspace workspace_for(const marc_lz77_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lz77_workspace_requirements(&settings, &result.requirements),
              MARC_STATUS_OK);
...
Transform create(const marc_lz77_config& settings, Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lz77_create(
                  &settings,
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 80 + input.size() * 16 + frames * 56;
...
TEST(Lz77Completion, RequiredDataClassesRoundTripDeterministically) {
    expect_round_trip({});
...
TEST(Lz77Completion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(Lz77Completion, MalformedFinalFrameIsNeverCommitted) {
