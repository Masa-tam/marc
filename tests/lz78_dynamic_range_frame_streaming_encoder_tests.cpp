...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lz78_dynamic_range_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        EXPECT_EQ(plan.error, Lz78DynamicRangeFrameValidationError::none);
        const auto offset = output.size();
        output.resize(offset + plan.serialized_size);
        EXPECT_EQ(encode_lz78_dynamic_range_frame(
...
TEST(Lz78DynamicRangeFrameStreamingEncoder,
...
TEST(Lz78DynamicRangeFrameStreamingEncoder,
    ...
    EXPECT_EQ(first.output_produced, lz78_dynamic_range_stream_prefix_size);
...
TEST(Lz78DynamicRangeFrameStreamingEncoder,
    ...
    Lz78DynamicRangeFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    Lz78DynamicRangeFrameStreamingEncoder short_dictionary{
    ...
    Lz78DynamicRangeFrameStreamingEncoder short_entries{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded,
    ...
    Lz78DynamicRangeFrameStreamingEncoder short_encoded{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
    ...
    limits.max_internal_buffered_bytes =
        ...
        - 1;
    Lz78DynamicRangeFrameStreamingEncoder aggregate_limited{
...
TEST(Lz78DynamicRangeFrameStreamingEncoder, HandlesEmptyAndProtocolErrors) {
    std::array<std::byte, 1> unused{};
    std::array<std::byte, lz78_dynamic_range_stream_prefix_size> output{};
    Lz78DynamicRangeFrameStreamingEncoder empty{
    ...
    EXPECT_EQ(result.output_produced, lz78_dynamic_range_stream_prefix_size);

    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 16> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
    std::array<marc::dictionary::internal::Lz78EncoderEntry, 2> entries{};
    Lz78DynamicRangeFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    ...
    Lz78DynamicRangeFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    ...
    Lz78DynamicRangeFrameStreamingEncoder unknown{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    result = unknown.process({}, {}, UINT32_C(1) << 31);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::unsupported);

    Lz78DynamicRangeFrameStreamingEncoder excess{
