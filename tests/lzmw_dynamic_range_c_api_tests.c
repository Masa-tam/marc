...
static void set_small_limits(marc_lzmw_dynamic_range_config* config) {
    config->max_total_output_size = 1024;
    config->max_frame_size = 2;
    config->max_compressed_payload_size = 21;
...
int main(void) {
    ...
    assert(marc_lzmw_dynamic_range_config_init(
    ...
    assert(marc_lzmw_dynamic_range_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    assert(needed.primary_bytes == 2);
    assert(needed.secondary_bytes == 101);
    ...
    assert(marc_lzmw_dynamic_range_create(
