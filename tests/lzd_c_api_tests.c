...
int main(void) {
    ...
    assert(marc_lzd_config_init(MARC_DIRECTION_ENCODE, &config)
           == MARC_STATUS_OK);
    assert(config.struct_size == sizeof(config));
    assert(config.maximum_entries == 65536);
    config.original_size = sizeof(input);
    config.frame_size = 2;
    assert(marc_lzd_workspace_requirements(&config, &needed)
           == MARC_STATUS_OK);
    ...
    if (needed.views_alignment > 1) {
        ...
        assert(marc_lzd_create(
                   &config, primary, secondary, misaligned, &transform)
               == MARC_STATUS_INVALID_ARGUMENT);
        free(storage.data);
    }

    assert(marc_lzd_create(&config, primary, secondary, views, &transform)
           == MARC_STATUS_OK);
    ...
    assert(marc_lzd_config_init(MARC_DIRECTION_DECODE, &config)
           == MARC_STATUS_OK);
