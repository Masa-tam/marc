...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lz77_rans_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        EXPECT_EQ(plan.error, Lz77RansFrameValidationError::none);
        const auto offset = output.size();
        output.resize(offset + plan.serialized_size);
        EXPECT_EQ(encode_lz77_rans_frame(
                      stream, {}, limits, sequence, committed, raw, staging,
...
TEST(Lz77RansFrameStreamingEncoder, MatchesReferenceWithOneByteBuffers) {
...
TEST(Lz77RansFrameStreamingEncoder, EmitsFullFramesAndKeepsFlushOpen) {
    const auto expected = reference();
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 32> dictionary_staging{};
    std::array<std::byte, 8192> frame_encoded{};
    ...
    EXPECT_EQ(first.output_produced, lz77_rans_stream_prefix_size);
...
TEST(Lz77RansFrameStreamingEncoder, ReportsWorkspaceAndAggregateErrors) {
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 32> dictionary_staging{};
    std::array<std::byte, 8192> frame_encoded{};
    std::array<std::byte, 16384> output{};
    Lz77RansFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    Lz77RansFrameStreamingEncoder short_dictionary{
    ...
    Lz77RansFrameStreamingEncoder short_encoded{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
    ...
    Lz77RansFrameStreamingEncoder aggregate_limited{
...
TEST(Lz77RansFrameStreamingEncoder, HandlesEmptyAndProtocolErrors) {
    std::array<std::byte, 1> unused{};
    std::array<std::byte, lz77_rans_stream_prefix_size> output{};
    Lz77RansFrameStreamingEncoder empty{
    ...
    EXPECT_EQ(result.output_produced, lz77_rans_stream_prefix_size);

    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 32> dictionary_staging{};
    std::array<std::byte, 8192> frame_encoded{};
    Lz77RansFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded};
    ...
    Lz77RansFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded};
