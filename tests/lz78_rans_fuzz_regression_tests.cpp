...
TEST(Lz78RansFuzzRegression, EveryCanonicalTruncationIsAtomic) {
...
TEST(Lz78RansFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lz78_rans_stream_prefix_size;
...
TEST(Lz78RansFuzzRegression, NonzeroDescriptorReservedByteIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lz78_rans_stream_prefix_size + frame_header_size + 10] =
        std::byte{1};
    expect_atomic_failure(encoded);
}
