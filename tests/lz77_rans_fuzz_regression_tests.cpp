...
[[nodiscard]] std::vector<std::byte> canonical_stream() {
    ...
    Lz77RansFrameStreamingEncoder encoder{
...
void expect_atomic_failure(const std::span<const std::byte> input) {
    ...
    std::array<marc::entropy::internal::RansBlockView, 8> views{};
    std::array<std::byte, raw.size() * 16> dictionary{};
    std::array<std::byte, raw.size()> decoded_frame{};
    std::array<std::byte, raw.size()> output{};
    output.fill(std::byte{0xa5});
    Lz77RansFrameStreamingDecoder decoder{
        {}, encoded_frame, views, dictionary, decoded_frame};
...
TEST(Lz77RansFuzzRegression, EveryCanonicalTruncationIsAtomic) {
    const auto encoded = canonical_stream();
    for (std::size_t size = 0; size < encoded.size(); ++size)
        expect_atomic_failure(std::span<const std::byte>{encoded}.first(size));
}

TEST(Lz77RansFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lz77_rans_stream_prefix_size;
...
TEST(Lz77RansFuzzRegression, InvalidDescriptorIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lz77_rans_stream_prefix_size + frame_header_size + 16] ^=
        std::byte{1};
    expect_atomic_failure(encoded);
}
