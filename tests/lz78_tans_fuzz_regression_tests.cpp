...
using EncoderEntry = marc::dictionary::internal::Lz78EncoderEntry;
...
[[nodiscard]] std::vector<std::byte> canonical_stream() {
    ...
    std::array<std::byte, raw.size() * 8> dictionary{};
    std::array<std::byte, 8192> frame_encoded{};
    std::array<EncoderEntry, raw.size()> entries{};
...
void expect_atomic_failure(const std::span<const std::byte> input) {
    ...
    Lz78TansFrameStreamingDecoder decoder{
        {}, encoded_frame, views, dictionary, decoded_frame, phrases};
...
TEST(Lz78TansFuzzRegression, EveryCanonicalTruncationIsAtomic) {
...
TEST(Lz78TansFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lz78_tans_stream_prefix_size;
...
TEST(Lz78TansFuzzRegression, InvalidDescriptorIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lz78_tans_stream_prefix_size + frame_header_size + 16] ^=
        std::byte{1};
    expect_atomic_failure(encoded);
}
