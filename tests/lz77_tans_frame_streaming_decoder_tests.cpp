...
[[nodiscard]] std::vector<std::byte> encoded_stream(
    ...
    Lz77TansFrameStreamingEncoder encoder{
...
TEST(Lz77TansFrameStreamingDecoder, DecodesOneByteInputAndOutput) {
    const auto encoded = encoded_stream();
    std::array<std::byte, 8192> frame_encoded{};
    std::array<marc::entropy::internal::TansBlockView, 7> views{};
    std::array<std::byte, 32> dictionary_staging{};
    std::array<std::byte, 2> frame_decoded{};
    Lz77TansFrameStreamingDecoder decoder{
...
TEST(Lz77TansFrameStreamingDecoder, CommitsOnlyFramesBeforeLaterCorruption) {
    auto encoded = encoded_stream();
    const auto first_size = frame_size_at(
        encoded, lz77_tans_stream_prefix_size);
    const auto second_offset = lz77_tans_stream_prefix_size + first_size;
...
TEST(Lz77TansFrameStreamingDecoder, ReportsWorkspaceAndAggregateErrors) {
    const auto encoded = encoded_stream();
    const auto first_size = frame_size_at(
        encoded, lz77_tans_stream_prefix_size);
    std::array<std::byte, 8192> frame_encoded{};
    ...
    Lz77TansFrameStreamingDecoder short_encoded{
    ...
    Lz77TansFrameStreamingDecoder short_views{
        {}, frame_encoded,
        std::span<marc::entropy::internal::TansBlockView>{views}.first<6>(),
        dictionary_staging, frame_decoded};
    EXPECT_EQ(short_views.process(encoded, output, end).error.code,
              marc::core::ErrorCode::out_of_memory);

    Lz77TansFrameStreamingDecoder short_dictionary{
        {}, frame_encoded, views,
        std::span<std::byte>{dictionary_staging}.first<31>(),
        frame_decoded};
    EXPECT_EQ(short_dictionary.process(encoded, output, end).error.code,
              marc::core::ErrorCode::out_of_memory);

    Lz77TansFrameStreamingDecoder short_decoded{
    ...
    limits.max_internal_buffered_bytes =
        first_size + 32 + 2
        + 7 * sizeof(marc::entropy::internal::TansBlockView) - 1;
    Lz77TansFrameStreamingDecoder aggregate_limited{
...
TEST(Lz77TansFrameStreamingDecoder, RejectsTruncationTrailingAndReset) {
    const auto encoded = encoded_stream();
    std::array<std::byte, 8192> frame_encoded{};
    std::array<marc::entropy::internal::TansBlockView, 7> views{};
    std::array<std::byte, 32> dictionary_staging{};
    ...
    Lz77TansFrameStreamingDecoder truncated{
        {}, frame_encoded, views, dictionary_staging, frame_decoded};
    ...
    Lz77TansFrameStreamingDecoder trailing{
        {}, frame_encoded, views, dictionary_staging, frame_decoded};
    EXPECT_EQ(trailing.process(extended, output, end).error.code,
              marc::core::ErrorCode::malformed_stream);

    Lz77TansFrameStreamingDecoder reset{
        {}, frame_encoded, views, dictionary_staging, frame_decoded};
...
TEST(Lz77TansFrameStreamingDecoder, HandlesEmptyFlushAndPrematureEnd) {
    const auto empty_encoded = encoded_stream({});
    Lz77TansFrameStreamingDecoder empty{{}, {}, {}, {}, {}};
    ...
    std::array<std::byte, 32> dictionary_staging{};
    std::array<std::byte, 2> frame_decoded{};
    Lz77TansFrameStreamingDecoder starved{
        {}, frame_encoded, views, dictionary_staging, frame_decoded};
    ...
    const auto prefix_and_first_frame =
        lz77_tans_stream_prefix_size + first_size;
    Lz77TansFrameStreamingDecoder premature{
        {}, frame_encoded, views, dictionary_staging, frame_decoded};
    std::array<std::byte, 1> output{};
