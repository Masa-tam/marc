...
TEST(LzdRansFrameStreamingDecoder, DecodesOneByteInputAndOutput) {
...
TEST(LzdRansFrameStreamingDecoder, CommitsOnlyFramesBeforeLaterCorruption) {
    auto encoded = encoded_stream();
    const auto first_size =
        frame_size_at(encoded, lzd_rans_stream_prefix_size);
    const auto second_offset = lzd_rans_stream_prefix_size + first_size;
...
TEST(LzdRansFrameStreamingDecoder, ReportsWorkspaceAndAggregateErrors) {
    const auto encoded = encoded_stream();
    const auto first_offset = lzd_rans_stream_prefix_size;
    ...
    const auto phrase_count =
        marc::dictionary::internal::lzd_validation_workspace_entries(
            dictionary_size, raw_size, {});
    const auto expansion_count =
        marc::dictionary::internal::lzd_expansion_workspace_entries(
    ...
    LzdRansFrameStreamingDecoder short_encoded{
        {},
    ...
    LzdRansFrameStreamingDecoder short_views{
    ...
    LzdRansFrameStreamingDecoder short_dictionary{
        {}, workspace.frame_encoded, workspace.views,
    ...
    LzdRansFrameStreamingDecoder short_decoded{
        {}, workspace.frame_encoded, workspace.views,
    ...
    LzdRansFrameStreamingDecoder short_phrases{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        std::span<marc::dictionary::internal::LzdPhraseEntry>{
            workspace.phrases}.first(phrase_count - 1),
        workspace.expansion};
    EXPECT_EQ(short_phrases.process(encoded, output, end).error.code,
              marc::core::ErrorCode::out_of_memory);

    LzdRansFrameStreamingDecoder short_expansion{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases,
    ...
    limits.max_internal_buffered_bytes =
        ...
        + phrase_count * sizeof(marc::dictionary::internal::LzdPhraseEntry)
        + expansion_count * sizeof(std::uint32_t) - 1;
    LzdRansFrameStreamingDecoder aggregate_limited{
...
TEST(LzdRansFrameStreamingDecoder, RejectsTruncationTrailingAndReset) {
    const auto encoded = encoded_stream();
    Workspace workspace{};
    std::array<std::byte, raw.size()> output{};
    const auto end =
    ...
    LzdRansFrameStreamingDecoder truncated{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
    ...
    LzdRansFrameStreamingDecoder trailing{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
    ...
    LzdRansFrameStreamingDecoder reset{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
    ...
    LzdRansFrameStreamingDecoder unknown{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
...
TEST(LzdRansFrameStreamingDecoder, HandlesEmptyFlushAndPrematureEnd) {
    const auto empty_encoded = encoded_stream({});
    LzdRansFrameStreamingDecoder empty{{}, {}, {}, {}, {}, {}, {}};
    ...
    Workspace workspace{};
    LzdRansFrameStreamingDecoder starved{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
    ...
    const auto prefix_and_first_frame =
        lzd_rans_stream_prefix_size + first_size;
    LzdRansFrameStreamingDecoder premature{
        {}, workspace.frame_encoded, workspace.views,
        workspace.dictionary_staging, workspace.frame_decoded,
        workspace.phrases, workspace.expansion};
    std::array<std::byte, 1> output{};
