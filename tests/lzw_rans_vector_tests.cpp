...
TEST(LzwRansVector, EmitsIndependentSingleCodeFrame) {
    ...
    EXPECT_EQ(descriptor.frequencies[0], 2048);
    EXPECT_EQ(descriptor.frequencies[0x41], 2048);
    ...
    ASSERT_EQ(marc::entropy::internal::serialize_rans_descriptor(
                  descriptor, packed_code_a.size(), rans_payload.size(),
                  limits,
                  std::span<std::byte,
                            marc::entropy::internal::rans_descriptor_size>{
                      frame.data() + marc::frame::frame_header_size,
