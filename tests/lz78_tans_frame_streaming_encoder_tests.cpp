...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lz78_tans_stream_prefix_size);
    ...
    while (committed < input.size()) {
        ...
        const auto plan = plan_lz78_tans_frame(
            stream, {}, limits, sequence, committed, raw, entries, staging);
        EXPECT_EQ(plan.error, Lz78TansFrameValidationError::none);
        const auto offset = output.size();
        output.resize(offset + plan.serialized_size);
        EXPECT_EQ(encode_lz78_tans_frame(
...
TEST(Lz78TansFrameStreamingEncoder, MatchesReferenceWithOneByteBuffers) {
...
TEST(Lz78TansFrameStreamingEncoder, EmitsFramesAndKeepsFlushOpen) {
    const auto expected = reference();
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 16> dictionary_staging{};
    std::array<std::byte, 8192> frame_encoded{};
    ...
    EXPECT_EQ(first.output_produced, lz78_tans_stream_prefix_size);
...
TEST(Lz78TansFrameStreamingEncoder,
...
TEST(Lz78TansFrameStreamingEncoder, ReportsWorkspaceAndAggregateErrors) {
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 16> dictionary_staging{};
    std::array<std::byte, 8192> frame_encoded{};
    std::array<marc::dictionary::internal::Lz78EncoderEntry, 2> entries{};
    ...
    Lz78TansFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    Lz78TansFrameStreamingEncoder short_dictionary{
    ...
    Lz78TansFrameStreamingEncoder short_entries{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded,
    ...
    Lz78TansFrameStreamingEncoder short_encoded{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
    ...
    const auto plan = plan_lz78_tans_frame(
        ...
        dictionary_staging);
    ASSERT_EQ(plan.error, Lz78TansFrameValidationError::none);
    ...
    limits.max_internal_buffered_bytes =
        ...
        - 1;
    Lz78TansFrameStreamingEncoder aggregate_limited{
...
TEST(Lz78TansFrameStreamingEncoder, HandlesEmptyAndProtocolErrors) {
    std::array<std::byte, 1> unused{};
    std::array<std::byte, lz78_tans_stream_prefix_size> output{};
    Lz78TansFrameStreamingEncoder empty{
        config(0), {}, {}, std::span<std::byte>{}, std::span<std::byte>{},
        unused, std::span<marc::dictionary::internal::Lz78EncoderEntry>{}};
    auto result = empty.process(
        {}, output,
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
    EXPECT_EQ(result.status, marc::core::StreamStatus::end_of_stream);
    EXPECT_EQ(result.output_produced, lz78_tans_stream_prefix_size);

    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 16> dictionary_staging{};
    std::array<std::byte, 8192> frame_encoded{};
    std::array<marc::dictionary::internal::Lz78EncoderEntry, 2> entries{};
    Lz78TansFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    ...
    Lz78TansFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    ...
    Lz78TansFrameStreamingEncoder unknown{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    result = unknown.process({}, {}, UINT32_C(1) << 31);
    EXPECT_EQ(result.error.code, marc::core::ErrorCode::unsupported);

    Lz78TansFrameStreamingEncoder excess{
