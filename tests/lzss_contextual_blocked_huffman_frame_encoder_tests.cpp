...
[[nodiscard]] LzssContextualBlockedHuffmanStreamHeader stream_for_4m(
    const std::uint64_t original_size) {
    auto stream = stream_for(original_size);
    stream.frame_size = static_cast<std::uint32_t>(original_size);
...
[[nodiscard]] LzssContextualBlockedHuffmanStreamHeader stream_for_16m(
    const std::uint64_t original_size) {
    auto stream = stream_for(original_size);
    stream.frame_size = static_cast<std::uint32_t>(original_size);
    ...
    for (std::size_t profile = 0; profile < windows.size(); ++profile) {
        for (const unsigned mode : {0U, 1U, 2U}) {
            ...
            for (std::size_t offset = 0; offset < input.size(); offset += stream.frame_size, ++sequence) {
                ...
                const auto plan = plan_lzss_contextual_blocked_huffman_frame_hash_chain(
                    stream, limits, sequence, offset, raw, tokens, workspace);
                ASSERT_EQ(plan.error, LzssContextualBlockedHuffmanFrameEncodeError::none);
                ...
                for (const bool probe : {false, true}) {
                    ...
                    const auto payload_offset =
                        lzss_contextual_blocked_huffman_frame_header_size + plan.descriptor_size;
                    std::vector<std::byte> encoded(plan.serialized_size + 1, std::byte{0xcc});
                    const auto entropy = encode_lzss_contextual_blocked_huffman_tokens(
                        std::span<const LzssTypedToken>{tokens}.first(token_result.token_count),
                        stream.dictionary, context, limits,
                        std::span{encoded}.subspan(payload_offset, plan.payload_size),
                        descriptor, selected.layout.context_variant);
                    ASSERT_EQ(entropy.error, LzssContextualBlockedHuffmanEncodeError::none);
                    ASSERT_EQ(entropy.payload_size, plan.payload_size);
                    ASSERT_EQ(entropy.descriptor_size, plan.descriptor_size);
                    ...
                    ASSERT_EQ(serialize_contextual_blocked_huffman_descriptor(
                                  ...
                                  std::span{encoded}.subspan(
                                      ...
                                      entropy.descriptor_size), written,
                                  selected.layout.context_variant),
                              ContextualBlockedHuffmanFormatError::none);
                    ASSERT_EQ(written, entropy.descriptor_size);
                    ...
                    header.descriptor_size = static_cast<std::uint32_t>(entropy.descriptor_size);
                    ASSERT_EQ(serialize_lzss_contextual_blocked_huffman_frame_header(
                                  header, {stream, limits, sequence, offset},
                                  std::span<std::byte, lzss_contextual_blocked_huffman_frame_header_size>{
                                      encoded.data(), lzss_contextual_blocked_huffman_frame_header_size}),
                              LzssContextualBlockedHuffmanFrameHeaderError::none);
                    EXPECT_EQ(encoded.back(), std::byte{0xcc});
                    encoded.pop_back();
                    EXPECT_EQ(encoded, baseline);
                    std::vector<HuffmanDecodeTable> tables(35);
                    ...
                    const auto decoded = decode_lzss_contextual_blocked_huffman_frame(
                        encoded, {stream, limits, sequence, offset}, tables,
                        restored_tokens, restored);
                    ASSERT_EQ(decoded.error, LzssContextualBlockedHuffmanFrameDecodeError::none);
...
TEST(LzssContextualBlockedHuffmanFrameEncoder,
    ...
    ASSERT_EQ(plan.error,
              LzssContextualBlockedHuffmanFrameEncodeError::none);
    EXPECT_EQ(plan.serialized_size, 88U);
    EXPECT_EQ(plan.descriptor_size, 24U);
    EXPECT_EQ(plan.decision_count, 2U);

    std::array<std::byte, 88> frame{};
    ASSERT_EQ(encode_lzss_contextual_blocked_huffman_frame(
                  stream, {}, 0, 0, raw, tokens, frame).error,
              LzssContextualBlockedHuffmanFrameEncodeError::none);
    std::array<LzssTypedToken, 1> decoded_tokens{};
    std::array<std::byte, 1> decoded{};
    const auto result = decode_lzss_contextual_blocked_huffman_frame(
        frame, {stream, {}, 0, 0}, {}, decoded_tokens, decoded);
    ASSERT_EQ(result.error,
              LzssContextualBlockedHuffmanFrameDecodeError::none);
    EXPECT_EQ(result.serialized_consumed, frame.size());
    EXPECT_EQ(decoded, raw);
