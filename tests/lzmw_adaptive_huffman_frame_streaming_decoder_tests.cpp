...
[[nodiscard]] std::vector<std::byte> encoded_stream(
    ...
    std::vector<std::byte> encoded(lzmw_adaptive_huffman_stream_prefix_size);
...
[[nodiscard]] std::size_t first_frame_size(
    ...
    const std::span<const std::byte, frame_header_size> bytes{
        encoded.data() + lzmw_adaptive_huffman_stream_prefix_size,
        frame_header_size};
...
TEST(LzmwAdaptiveHuffmanFrameStreamingDecoder,
...
TEST(LzmwAdaptiveHuffmanFrameStreamingDecoder,
    ...
    const auto second = lzmw_adaptive_huffman_stream_prefix_size
...
TEST(LzmwAdaptiveHuffmanFrameStreamingDecoder,
    ...
    std::array<marc::dictionary::internal::LzmwPhraseEntry, 1> phrases{};
    std::array<std::uint32_t, 2> expansion{};
    LzmwAdaptiveHuffmanFrameStreamingDecoder trailing_decoder{
        {}, frame_encoded, staging, decoded, phrases, expansion};
    std::array<std::byte, raw.size()> output{};
    ...
    LzmwAdaptiveHuffmanFrameStreamingDecoder empty_decoder{
        {}, frame_encoded, staging, decoded, phrases, expansion};
...
TEST(LzmwAdaptiveHuffmanFrameStreamingDecoder,
    ...
    LzmwAdaptiveHuffmanFrameStreamingDecoder short_frame{
    ...
    LzmwAdaptiveHuffmanFrameStreamingDecoder short_references{
    ...
    EXPECT_EQ(short_references.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzmwAdaptiveHuffmanFrameStreamingDecoder short_raw{
    ...
    EXPECT_EQ(short_raw.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzmwAdaptiveHuffmanFrameStreamingDecoder short_phrases{
        {}, frame_encoded, staging, decoded, {}, expansion};
    EXPECT_EQ(short_phrases.process(encoded, output, flags).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzmwAdaptiveHuffmanFrameStreamingDecoder short_expansion{
    ...
    LzmwAdaptiveHuffmanFrameStreamingDecoder limited{
    ...
    LzmwAdaptiveHuffmanFrameStreamingDecoder reset{
        {}, frame_encoded, staging, decoded, phrases, expansion};
    ...
    LzmwAdaptiveHuffmanFrameStreamingDecoder unknown{
        {}, frame_encoded, staging, decoded, phrases, expansion};
