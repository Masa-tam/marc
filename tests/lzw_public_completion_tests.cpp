...
namespace lzw_public_completion {

constexpr std::size_t test_frame_size = 64;
constexpr std::size_t maximum_payload_size = test_frame_size * 2;
constexpr std::size_t maximum_dictionary_entries = 256;
...
marc_lzw_config config(const marc_direction direction,
                       const std::size_t original_size) {
    marc_lzw_config result{};
    EXPECT_EQ(marc_lzw_config_init(direction, &result), MARC_STATUS_OK);
    result.original_size = original_size;
    result.frame_size = test_frame_size;
    result.maximum_code_width = 9;
    ...
    result.max_dictionary_serialized_size = maximum_payload_size;
    result.max_internal_buffered_bytes = 16384;
    result.max_dictionary_entries = maximum_dictionary_entries;
    return result;
}

Workspace workspace_for(const marc_lzw_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lzw_workspace_requirements(&settings, &result.requirements),
              MARC_STATUS_OK);
    EXPECT_NE(result.requirements.views_alignment, 0U);
    if (settings.direction == MARC_DIRECTION_ENCODE
        && settings.original_size <= 1) {
        EXPECT_EQ(result.requirements.views_bytes, 0U);
    } else {
        EXPECT_NE(result.requirements.views_bytes, 0U);
    }
    result.primary.resize(result.requirements.primary_bytes);
    result.secondary.resize(result.requirements.secondary_bytes);
    if (result.requirements.views_bytes == 0) return result;
    const auto alignment = result.requirements.views_alignment;
    result.views_storage.resize(result.requirements.views_bytes + alignment - 1);
...
Transform create(const marc_lzw_config& settings, Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lzw_create(
                  &settings,
...
std::vector<std::uint8_t> encode(
    ...
    const auto capacity = 80 + input.size() * 2 + frames * 56;
...
TEST(LzwPublicCompletion, RequiredDataClassesRoundTripDeterministically) {
    expect_round_trip({});
...
TEST(LzwPublicCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(LzwPublicCompletion, MalformedFinalFrameIsNeverCommitted) {
