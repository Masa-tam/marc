...
constexpr std::size_t maximum_dictionary_size = 4 * test_frame_size;
#ifndef MARC_LZMW_COMPLETION_MAXIMUM_PAYLOAD
#define MARC_LZMW_COMPLETION_MAXIMUM_PAYLOAD(dictionary_size) \
    ((dictionary_size) * 33)
#endif
constexpr std::size_t maximum_payload_size =
    MARC_LZMW_COMPLETION_MAXIMUM_PAYLOAD(maximum_dictionary_size);
constexpr std::size_t maximum_frame_size = 56 + 16 + maximum_payload_size;
constexpr std::size_t maximum_dictionary_entries = test_frame_size - 1;
...
[[nodiscard]] marc_lzmw_adaptive_huffman_config config(
    const marc_direction direction, const std::size_t original_size) {
    marc_lzmw_adaptive_huffman_config result{};
    EXPECT_EQ(marc_lzmw_adaptive_huffman_config_init(direction, &result),
              MARC_STATUS_OK);
...
[[nodiscard]] Workspace workspace_for(
    const marc_lzmw_adaptive_huffman_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lzmw_adaptive_huffman_workspace_requirements(
...
TEST(LzmwAdaptiveHuffmanCompletion,
...
TEST(LzmwAdaptiveHuffmanCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(LzmwAdaptiveHuffmanCompletion, MalformedFinalFrameIsNeverCommitted) {
