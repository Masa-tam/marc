...
[[nodiscard]] std::vector<std::byte> reference() {
    ...
    std::vector<std::byte> output(lzd_tans_stream_prefix_size);
    ...
    EXPECT_EQ(marc::dictionary::internal::serialize_lzd_parameters(
                  {}, limits,
                  std::span<std::byte,
                            marc::dictionary::internal::lzd_parameter_size>{
                      output.data() + stream_header_size,
                      marc::dictionary::internal::lzd_parameter_size}),
              marc::dictionary::internal::LzdFormatError::none);

    std::array<marc::dictionary::internal::LzdEncoderEntry, 1> entries{};
...
TEST(LzdTansFrameStreamingEncoder, MatchesReferenceWithOneByteBuffers) {
...
TEST(LzdTansFrameStreamingEncoder, FlushKeepsCanonicalFramesAndStreamOpen) {
    const auto expected = reference();
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
    ...
    EXPECT_EQ(first.output_produced, lzd_tans_stream_prefix_size);
...
TEST(LzdTansFrameStreamingEncoder, RetainsEndInputWhileAllRegionsDrain) {
    const auto expected = reference();
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
...
TEST(LzdTansFrameStreamingEncoder, ReportsWorkspaceAndAggregateErrors) {
    std::array<std::byte, 2> frame_input{};
    std::array<std::byte, 8> dictionary_staging{};
    std::array<std::byte, 4096> frame_encoded{};
    std::array<marc::dictionary::internal::LzdEncoderEntry, 1> entries{};
    ...
    LzdTansFrameStreamingEncoder short_input{
        config(input.size()), {}, {},
    ...
    LzdTansFrameStreamingEncoder short_dictionary{
        config(input.size()), {}, {}, frame_input,
        std::span<std::byte>{dictionary_staging}.first<7>(), frame_encoded,
        entries};
    EXPECT_EQ(short_dictionary.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzdTansFrameStreamingEncoder short_entries{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, {}};
    EXPECT_EQ(short_entries.process({}, {}, 0).error.code,
              marc::core::ErrorCode::out_of_memory);
    LzdTansFrameStreamingEncoder short_encoded{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
    ...
    const auto plan = plan_lzd_tans_frame(
        ...
        dictionary_staging);
    ASSERT_EQ(plan.error, LzdTansFrameValidationError::none);
    auto limits = marc::core::DecoderLimits{};
    limits.max_block_size = 2;
    limits.max_internal_buffered_bytes =
        2 + plan.dictionary_size + plan.serialized_size
        + plan.encoder_entries
              * sizeof(marc::dictionary::internal::LzdEncoderEntry)
        - 1;
    LzdTansFrameStreamingEncoder aggregate_limited{
...
TEST(LzdTansFrameStreamingEncoder, HandlesEmptyAndProtocolErrors) {
    std::array<std::byte, 1> unused{};
    std::array<std::byte, lzd_tans_stream_prefix_size> output{};
    LzdTansFrameStreamingEncoder empty{
        config(0), {}, {}, {}, {}, unused, {}};
    auto result = empty.process(
        {}, output,
        marc::core::flag_value(marc::core::ProcessFlags::end_input));
    EXPECT_EQ(result.status, marc::core::StreamStatus::end_of_stream);
    EXPECT_EQ(result.output_produced, lzd_tans_stream_prefix_size);
    ...
    std::array<std::byte, 4096> frame_encoded{};
    std::array<marc::dictionary::internal::LzdEncoderEntry, 1> entries{};
    LzdTansFrameStreamingEncoder premature{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    ...
    LzdTansFrameStreamingEncoder excess{
    ...
    LzdTansFrameStreamingEncoder reset{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
    ...
    LzdTansFrameStreamingEncoder unknown{
        config(input.size()), {}, {}, frame_input, dictionary_staging,
        frame_encoded, entries};
