...
TEST(LzssTansFuzzRegression, EveryCanonicalTruncationIsAtomic) {
    const auto encoded = canonical_stream();
    for (std::size_t size = 0; size < encoded.size(); ++size)
        expect_atomic_failure(std::span<const std::byte>{encoded}.first(size));
}

TEST(LzssTansFuzzRegression, ExtremeFrameLengthsAreAtomic) {
    auto encoded = canonical_stream();
    const auto frame_offset = lzss_tans_stream_prefix_size;
...
TEST(LzssTansFuzzRegression, InvalidDescriptorIsAtomic) {
    auto encoded = canonical_stream();
    encoded[lzss_tans_stream_prefix_size + frame_header_size + 16] ^=
        std::byte{1};
    expect_atomic_failure(encoded);
}
