...
constexpr std::size_t maximum_payload_size =
    maximum_dictionary_size + maximum_blocks * 8;
...
[[nodiscard]] marc_lz77_rans_config config(
    const marc_direction direction, const std::size_t original_size) {
    marc_lz77_rans_config result{};
    EXPECT_EQ(marc_lz77_rans_config_init(direction, &result),
              MARC_STATUS_OK);
...
[[nodiscard]] Workspace workspace_for(
    const marc_lz77_rans_config& settings) {
    Workspace result{};
    EXPECT_EQ(marc_lz77_rans_workspace_requirements(
...
[[nodiscard]] Transform create(
    const marc_lz77_rans_config& settings, Workspace& workspace) {
    marc_transform* raw{};
    EXPECT_EQ(marc_lz77_rans_create(
                  &settings,
...
TEST(Lz77RansCompletion, RequiredDataClassesRoundTripDeterministically) {
    expect_round_trip({});
...
TEST(Lz77RansCompletion, MultiFrameStreamIsIndependentOfChunking) {
...
TEST(Lz77RansCompletion, MalformedFinalFrameIsNeverCommitted) {
