...
[[nodiscard]] LzssShortLengthEscapeStreamEncodeError check_regions(
    const std::span<const LzssShortLengthEscapeFrameTokens> frames,
    ...
    if (!core::checked_multiply(frames.size(),
                                sizeof(LzssShortLengthEscapeFrameTokens),
                                frame_bytes)
        || !core::checked_multiply(operations.size(),
                                   sizeof(context::internal::ModeledOperation),
                                   operation_bytes)) {
        return LzssShortLengthEscapeStreamEncodeError::arithmetic_overflow;
    }
    ...
    for (std::size_t first = 0; first < regions.size(); ++first) {
        for (std::size_t second = first + 1; second < regions.size();
            ...
            if (overlap == core::BufferOverlap::overlap) {
                return LzssShortLengthEscapeStreamEncodeError::overlapping_workspaces;
            }
        }
    }
    ...
    return LzssShortLengthEscapeStreamEncodeError::none;
}
...
[[nodiscard]] LzssShortLengthEscapeStreamEncodeResult plan_impl(
    ...
    if (!std::in_range<std::size_t>(required_count)
        ...
        result.error = LzssShortLengthEscapeStreamEncodeError::frame_count_mismatch;
        return result;
    }
    result.error = check_regions(frames, operations, {});
    if (result.error != LzssShortLengthEscapeStreamEncodeError::none) {
        return result;
    }
    ...
    for (std::size_t index = 0; index < frames.size(); ++index) {
        ...
        result.frame = plan_lzss_short_length_escape_frame(
        ...
        if (result.frame.error != LzssShortMatchFrameEncodeError::none) {
            result.error = LzssShortLengthEscapeStreamEncodeError::frame_error;
            return result;
        }
        if (!core::checked_add(result.serialized_size,
            ...
            || !core::checked_add(raw_committed,
                                  ...
                                  raw_committed)) {
            result.error = LzssShortLengthEscapeStreamEncodeError::arithmetic_overflow;
            return result;
        }
    }
    if (raw_committed != stream.original_size) {
        result.error = LzssShortLengthEscapeStreamEncodeError::internal_error;
        return result;
    }
...
encode_lzss_short_length_escape_stream(
    ...
    if (serialized_output.size() < result.serialized_size) {
        result.error = LzssShortLengthEscapeStreamEncodeError::output_too_small;
        return result;
    }
    ...
    for (std::size_t index = 0; index < frames.size(); ++index) {
        result.frame_index = index;
        result.frame = encode_lzss_short_length_escape_frame(
            stream, limits, static_cast<std::uint64_t>(index), raw_committed,
            frames[index].tokens, operations,
