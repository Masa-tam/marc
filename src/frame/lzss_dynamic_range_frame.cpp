...
inline constexpr std::uint64_t max_raw_frame_size = UINT64_C(1) << 23;
...
[[nodiscard]] LzssDynamicRangeFrameValidationResult validate_frame(
    ...
    if (input.size() < frame_header_size) {
        result.error =
            LzssDynamicRangeFrameValidationError::truncated_frame;
        return result;
    }
    ...
    if (result.header_error != FrameHeaderError::none) {
        result.error = LzssDynamicRangeFrameValidationError::header_error;
        return result;
    }
    ...
    if (result.payload_size > maximum_payload_size) {
        result.error =
            LzssDynamicRangeFrameValidationError::invalid_entropy_extent;
        return result;
    }
    ...
    if (workspace_bytes > limits.max_internal_buffered_bytes) {
        result.error = LzssDynamicRangeFrameValidationError::workspace_limit;
        return result;
    }
    ...
    if (entropy_decoded.error
        ...
        result.error =
            LzssDynamicRangeFrameValidationError::entropy_decode_error;
        return result;
    }
...
LzssDynamicRangeFrameValidationResult plan_lzss_dynamic_range_frame(
    ...
    const auto entropy_limits = entropy_limits_for(
        limits, result.dictionary_size);
    const auto entropy_plan = entropy::internal::plan_dynamic_range_frame(
    ...
    if (entropy_plan.error
        ...
        result.error =
            LzssDynamicRangeFrameValidationError::entropy_encode_error;
        return result;
    }
...
LzssDynamicRangeFrameValidationResult decode_lzss_dynamic_range_frame(
    const StreamHeader& stream,
    const dictionary::internal::LzssParameters& parameters,
    const core::DecoderLimits& limits,
    const std::uint64_t expected_sequence,
