...
[[nodiscard]] ScanResult scan_frames(
    ...
    while (committed < stream.original_size) {
        if (input.size() - scan.cursor < frame_header_size) {
            scan.error = AdaptiveHuffmanStreamCodecError::truncated_stream;
            return scan;
        }
        ...
        if (parse_frame_header(header_bytes, context, header)
            ...
            scan.error = AdaptiveHuffmanStreamCodecError::frame_error;
            scan.frame_error = AdaptiveHuffmanFrameCodecError::header_error;
            return scan;
        }
        ...
        if (!core::checked_add(
            ...
            || !core::checked_add(
                ...
                frame_size)) {
            scan.error = AdaptiveHuffmanStreamCodecError::arithmetic_overflow;
            return scan;
        }
        ...
        const auto frame_result = write_output
            ? decode_adaptive_huffman_frame(
                stream, limits, scan.frame_count, committed, frame_input,
                output.subspan(static_cast<std::size_t>(committed),
                               header.dictionary_serialized_size))
            : validate_adaptive_huffman_frame(
                stream, limits, scan.frame_count, committed, frame_input);
        if (frame_result.error != AdaptiveHuffmanFrameCodecError::none) {
            scan.error = AdaptiveHuffmanStreamCodecError::frame_error;
    ...
    if (scan.cursor != input.size()) {
        scan.error = AdaptiveHuffmanStreamCodecError::trailing_stream_bytes;
    }
    return scan;
}
...
AdaptiveHuffmanStreamCodecResult plan_adaptive_huffman_stream(
    const StreamHeader& stream, const core::DecoderLimits& limits,
    const std::span<const std::byte> input) noexcept {
    AdaptiveHuffmanStreamCodecResult result{};
    result.stream_header_error = validate_stream_header(stream, limits);
    if (result.stream_header_error != StreamHeaderError::none) {
        result.error = AdaptiveHuffmanStreamCodecError::invalid_stream_header;
        return result;
    }
    if (!supported_pipeline(stream)) {
        result.error = AdaptiveHuffmanStreamCodecError::unsupported_pipeline;
        return result;
    }
    result.output_size = input.size();
    if (input.size() != stream.original_size) {
        result.error = AdaptiveHuffmanStreamCodecError::input_size_mismatch;
        return result;
    }
    ...
    while (offset < input.size()) {
        ...
        const auto frame = plan_adaptive_huffman_frame(
            stream, limits, result.frame_count, offset,
            input.subspan(offset, size));
        if (frame.error != AdaptiveHuffmanFrameCodecError::none) {
            result.frame_index = result.frame_count;
            result.frame_error = frame.error;
            result.error = AdaptiveHuffmanStreamCodecError::frame_error;
            return result;
        }
        if (!core::checked_add(result.serialized_size, frame.serialized_size,
                               result.serialized_size)) {
            result.error = AdaptiveHuffmanStreamCodecError::arithmetic_overflow;
            return result;
        }
        offset += size;
        ++result.frame_count;
...
AdaptiveHuffmanStreamCodecResult encode_adaptive_huffman_stream(
    const StreamHeader& stream, const core::DecoderLimits& limits,
    const std::span<const std::byte> input,
    const std::span<std::byte> output) noexcept {
    auto result = plan_adaptive_huffman_stream(stream, limits, input);
    if (result.error != AdaptiveHuffmanStreamCodecError::none) return result;
    if (output.size() < result.serialized_size) {
        result.error = AdaptiveHuffmanStreamCodecError::output_too_small;
        return result;
    }
    ...
    if (serialize_stream_header(stream, limits, header_output)
        ...
        result.error = AdaptiveHuffmanStreamCodecError::internal_error;
        return result;
    }
    ...
    while (input_offset < input.size()) {
        ...
        const auto frame_plan = plan_adaptive_huffman_frame(
        ...
        if (encoded.error != AdaptiveHuffmanFrameCodecError::none) {
...
AdaptiveHuffmanStreamCodecResult decode_adaptive_huffman_stream(
    const std::span<const std::byte> input,
    ...
    if (input.size() < stream_header_size) {
        result.error = AdaptiveHuffmanStreamCodecError::truncated_stream;
        return result;
    }
    ...
    if (parsed.original_size == 0) {
        if (input.size() != stream_header_size) {
            result.error = AdaptiveHuffmanStreamCodecError::trailing_stream_bytes;
            return result;
        }
        stream = parsed;
        return result;
    ...
    if (validation.error != AdaptiveHuffmanStreamCodecError::none) {
    ...
    if (decoded.error != AdaptiveHuffmanStreamCodecError::none) {
