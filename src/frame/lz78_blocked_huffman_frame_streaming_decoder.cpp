...
Lz78BlockedHuffmanFrameStreamingDecoder::
Lz78BlockedHuffmanFrameStreamingDecoder(
...
core::ProcessResult Lz78BlockedHuffmanFrameStreamingDecoder::fail(
...
bool Lz78BlockedHuffmanFrameStreamingDecoder::parse_collected_prefix()
    noexcept {
...
bool Lz78BlockedHuffmanFrameStreamingDecoder::
...
bool Lz78BlockedHuffmanFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const auto decoded = decode_lz78_blocked_huffman_frame(
    ...
    if (decoded.error != Lz78BlockedHuffmanFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult Lz78BlockedHuffmanFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lz78_blocked_huffman_stream_prefix_size
                - header_collected_;
            ...
            if (header_collected_
                != lz78_blocked_huffman_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream, consumed,
                                produced);
