...
core::ProcessResult LzmwDynamicRangeFrameStreamingDecoder::fail(
...
bool LzmwDynamicRangeFrameStreamingDecoder::parse_collected_prefix()
    noexcept {
...
bool LzmwDynamicRangeFrameStreamingDecoder::
...
bool LzmwDynamicRangeFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const auto decoded = decode_lzmw_dynamic_range_frame_to_staging(
    ...
    if (decoded.error != LzmwDynamicRangeFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzmwDynamicRangeFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzmw_dynamic_range_stream_prefix_size
                - header_collected_;
            ...
            if (header_collected_ != lzmw_dynamic_range_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream,
                                consumed, produced);
