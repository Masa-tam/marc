...
LzssRansFrameStreamingDecoder::LzssRansFrameStreamingDecoder(
...
core::ProcessResult LzssRansFrameStreamingDecoder::fail(
...
bool LzssRansFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool LzssRansFrameStreamingDecoder::parse_collected_frame_header() noexcept {
...
bool LzssRansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzss_rans_frame_to_staging(
    ...
    if (decoded.error != LzssRansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzssRansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lzss_rans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzss_rans_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream,
                                consumed, produced);
