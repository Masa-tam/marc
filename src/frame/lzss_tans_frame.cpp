...
[[nodiscard]] LzssTansFrameValidationResult plan_frame(
    const StreamHeader& stream,
    const dictionary::internal::LzssParameters& parameters,
    const core::DecoderLimits& limits,
    ...
    if (input.empty()
        ...
        result.error = LzssTansFrameValidationError::input_size_mismatch;
        return result;
    }
    ...
    if (input_staging_overlap != core::BufferOverlap::disjoint) {
        ...
        result.error = LzssTansFrameValidationError::dictionary_encode_error;
        return result;
    }
    ...
    if (block_count_u64 > limits.max_blocks_per_frame
        ...
        result.error = LzssTansFrameValidationError::entropy_encode_error;
        return result;
    }
    ...
    for (std::size_t block = 0; block < result.block_count; ++block) {
        ...
        if (!core::checked_add(
                ...
                result.payload_size)) {
            result.error =
                LzssTansFrameValidationError::arithmetic_overflow;
            return result;
        }
    ...
    if (result.header_error != FrameHeaderError::none) {
        result.error =
            ...
            : LzssTansFrameValidationError::header_error;
        return result;
    }
    if (!core::checked_add(
            frame_header_size, result.descriptor_size,
...
template <bool UseHashChain>
[[nodiscard]] LzssTansFrameValidationResult encode_frame(
    const StreamHeader& stream,
    const dictionary::internal::LzssParameters& parameters,
    const core::DecoderLimits& limits,
    const std::uint64_t sequence,
    ...
    if (output.size() < result.serialized_size) {
        result.error =
            LzssTansFrameValidationError::serialized_output_too_small;
        return result;
    }
    ...
    for (std::size_t block = 0; block < result.block_count; ++block) {
        ...
        const auto entropy_encoded =
            entropy::internal::encode_tans_block(
                dictionary_staging.subspan(dictionary_offset, block_size),
                limits,
                output.subspan(
...
LzssTansFrameValidationResult plan_lzss_tans_frame(
    const StreamHeader& stream,
    const dictionary::internal::LzssParameters& parameters,
    const core::DecoderLimits& limits,
    const std::uint64_t sequence,
...
LzssTansFrameValidationResult encode_lzss_tans_frame(
    const StreamHeader& stream,
    const dictionary::internal::LzssParameters& parameters,
    const core::DecoderLimits& limits,
    const std::uint64_t sequence,
...
LzssTansFrameValidationResult plan_lzss_tans_frame_hash_chain(
    const StreamHeader& stream,
    const dictionary::internal::LzssParameters& parameters,
    const core::DecoderLimits& limits,
    const std::uint64_t sequence,
...
LzssTansFrameValidationResult encode_lzss_tans_frame_hash_chain(
    const StreamHeader& stream,
    const dictionary::internal::LzssParameters& parameters,
    const core::DecoderLimits& limits,
    const std::uint64_t sequence,
