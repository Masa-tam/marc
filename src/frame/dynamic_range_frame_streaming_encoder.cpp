...
DynamicRangeFrameStreamingEncoder::DynamicRangeFrameStreamingEncoder(
...
core::ProcessResult DynamicRangeFrameStreamingEncoder::fail(
...
bool DynamicRangeFrameStreamingEncoder::prepare_frame() noexcept {
    preparation_error_ = core::ErrorCode::internal_error;
    const auto plan = plan_dynamic_range_frame(
        stream_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_));
    if (plan.error != DynamicRangeFrameCodecError::none) {
        if (plan.encode_error
            == entropy::internal::DynamicRangeEncodeError::limit_exceeded) {
    ...
    if (encoded.error != DynamicRangeFrameCodecError::none) return false;
...
core::ProcessResult DynamicRangeFrameStreamingEncoder::process(
