...
BlockedHuffmanFrameStreamingEncoder::BlockedHuffmanFrameStreamingEncoder(
...
core::ProcessResult BlockedHuffmanFrameStreamingEncoder::fail(
...
bool BlockedHuffmanFrameStreamingEncoder::prepare_frame() noexcept {
    const auto plan = plan_blocked_huffman_frame(
    ...
    if (plan.error != BlockedHuffmanFrameCodecError::none
        || frame_encoded_storage_.size() < plan.serialized_size) {
        return false;
    }
    const auto encoded = encode_blocked_huffman_frame(
        stream_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_),
...
core::ProcessResult BlockedHuffmanFrameStreamingEncoder::process(
    ...
    while (true) {
        if (state_ == State::draining_header
            ...
            const auto writable = output.size() - produced;
            const auto count = std::min(remaining, writable);
            if (count != 0) {
        ...
        const auto frame_remaining = expected_frame - frame_input_size_;
        const auto available = input.size() - consumed;
        const auto count = std::min(frame_remaining, available);
        if (count != 0) {
