#ifndef MARC_FRAME_LZW_RANS_FRAME_STREAMING_ENCODER_HPP
#define MARC_FRAME_LZW_RANS_FRAME_STREAMING_ENCODER_HPP
...
inline constexpr std::size_t lzw_rans_stream_prefix_size =
    stream_header_size + dictionary::internal::lzw_parameter_size;

class LzwRansFrameStreamingEncoder final : public core::Transform {
public:
    LzwRansFrameStreamingEncoder(
        StreamHeader stream,
...
private:
    ...
    std::array<std::byte, lzw_rans_stream_prefix_size> prefix_{};
    std::size_t frame_input_size_{};
    std::size_t pending_size_{lzw_rans_stream_prefix_size};
