...
LzssTansFrameStreamingEncoder::LzssTansFrameStreamingEncoder(
...
core::ProcessResult LzssTansFrameStreamingEncoder::fail(
...
bool LzssTansFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    const auto plan = use_hash_chain_
        ...
        : plan_lzss_tans_frame(
            stream_, parameters_, limits_, frame_sequence_, input_committed_,
            frame_input_storage_.first(frame_input_size_), dictionary_staging_);
    if (plan.error != LzssTansFrameValidationError::none) {
        switch (plan.error) {
        case LzssTansFrameValidationError::input_size_mismatch:
        case LzssTansFrameValidationError::unsupported_pipeline:
            preparation_error_ = core::ErrorCode::invalid_argument;
            break;
        case LzssTansFrameValidationError::dictionary_staging_too_small:
        case LzssTansFrameValidationError::serialized_output_too_small:
            preparation_error_ = core::ErrorCode::out_of_memory;
            break;
        case LzssTansFrameValidationError::workspace_limit:
        case LzssTansFrameValidationError::invalid_dictionary_extent:
        case LzssTansFrameValidationError::invalid_entropy_extent:
        case LzssTansFrameValidationError::entropy_encode_error:
        case LzssTansFrameValidationError::header_error:
        case LzssTansFrameValidationError::arithmetic_overflow:
            preparation_error_ = core::ErrorCode::limit_exceeded;
            break;
        case LzssTansFrameValidationError::dictionary_encode_error:
    ...
    const auto encoded = use_hash_chain_
        ...
        : encode_lzss_tans_frame(
            stream_, parameters_, limits_, frame_sequence_, input_committed_,
            frame_input_storage_.first(frame_input_size_), dictionary_staging_,
            frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzssTansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzssTansFrameStreamingEncoder::process(
