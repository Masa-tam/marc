...
LzmwBlockedHuffmanFrameStreamingDecoder::
LzmwBlockedHuffmanFrameStreamingDecoder(
    ...
    : limits_(limits), frame_encoded_storage_(frame_encoded_storage),
      ...
      frame_views_(frame_views), phrase_workspace_(phrase_workspace),
...
core::ProcessResult LzmwBlockedHuffmanFrameStreamingDecoder::fail(
...
bool LzmwBlockedHuffmanFrameStreamingDecoder::parse_collected_prefix()
    noexcept {
    ...
    if (stream_.dictionary_algorithm != DictionaryAlgorithm::lzmw
    ...
    const std::span<const std::byte,
        ...
        parameter_bytes{prefix_bytes_.data() + stream_header_size,
                        dictionary::internal::lzmw_parameter_size};
    return dictionary::internal::parse_lzmw_parameters(
               parameter_bytes, limits_, parameters_)
        == dictionary::internal::LzmwFormatError::none;
}

bool LzmwBlockedHuffmanFrameStreamingDecoder::
...
bool LzmwBlockedHuffmanFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const auto decoded = decode_lzmw_blocked_huffman_frame(
        stream_, parameters_, limits_, frame_sequence_, output_committed_,
        frame_encoded_storage_.first(frame_serialized_size_), frame_views_,
    ...
    if (decoded.error != LzmwBlockedHuffmanFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzmwBlockedHuffmanFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzmw_blocked_huffman_stream_prefix_size
                - header_collected_;
            ...
            if (header_collected_
                != lzmw_blocked_huffman_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream, consumed,
                                produced);
