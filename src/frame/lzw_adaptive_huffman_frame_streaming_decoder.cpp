...
LzwAdaptiveHuffmanFrameStreamingDecoder::
LzwAdaptiveHuffmanFrameStreamingDecoder(
...
core::ProcessResult LzwAdaptiveHuffmanFrameStreamingDecoder::fail(
...
bool LzwAdaptiveHuffmanFrameStreamingDecoder::parse_collected_prefix()
    noexcept {
...
bool LzwAdaptiveHuffmanFrameStreamingDecoder::
parse_collected_frame_header() noexcept {
    ...
    if (!core::checked_multiply(
            ...
            packed_bits)
        || !core::checked_add(
            packed_bits, UINT64_C(7), rounded_packed_bits)
        ...
        || frame_.dictionary_serialized_size > rounded_packed_bits / 8
...
bool LzwAdaptiveHuffmanFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const auto decoded = decode_lzw_adaptive_huffman_frame_to_staging(
    ...
    if (decoded.error != LzwAdaptiveHuffmanFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzwAdaptiveHuffmanFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzw_adaptive_huffman_stream_prefix_size
                - header_collected_;
