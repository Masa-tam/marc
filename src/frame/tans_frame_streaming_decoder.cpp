...
TansFrameStreamingDecoder::TansFrameStreamingDecoder(
...
core::ProcessResult TansFrameStreamingDecoder::fail(
...
bool TansFrameStreamingDecoder::parse_collected_stream_header()
    noexcept {
...
bool TansFrameStreamingDecoder::parse_collected_frame_header()
    noexcept {
...
bool TansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_tans_frame(
        stream_, limits_, frame_sequence_, output_committed_,
        frame_encoded_storage_.first(frame_serialized_size_),
        frame_decoded_storage_.first(frame_.dictionary_serialized_size),
        frame_views_);
    if (decoded.error != TansFrameCodecError::none) {
        return false;
    }
...
core::ProcessResult TansFrameStreamingDecoder::process(
