...
[[nodiscard]] RansFrameCodecResult parse_frame(
    ...
    RansFrameCodecResult result{};
    if (validate_stream_header(stream, limits) != StreamHeaderError::none
        || !supported_pipeline(stream)) {
        result.error = RansFrameCodecError::unsupported_pipeline;
        return result;
    }
    if (input.size() < frame_header_size) {
        result.error = RansFrameCodecError::truncated_frame;
        return result;
    }
    ...
    if (result.header_error != FrameHeaderError::none) {
        result.error = RansFrameCodecError::header_error;
        return result;
    }
    ...
    if (!core::checked_add(
        ...
        || !core::checked_add(
            ...
            result.serialized_size)) {
        result.error = RansFrameCodecError::arithmetic_overflow;
        return result;
    }
    ...
    if (input.size() != result.serialized_size) {
        result.error = RansFrameCodecError::trailing_frame_bytes;
        return result;
    }
    ...
    if (controlled.error
        == entropy::internal::RansControllerError::output_views_too_small) {
        result.error = RansFrameCodecError::views_too_small;
    } else if (controlled.error
               != entropy::internal::RansControllerError::none) {
        result.error = RansFrameCodecError::controller_error;
    }
...
RansFrameCodecResult plan_rans_frame(
    const StreamHeader& stream, const core::DecoderLimits& limits,
    ...
    if (input.empty() || input.size() > std::numeric_limits<std::uint32_t>::max()) {
        result.error = RansFrameCodecError::input_size_mismatch;
        return result;
    }
    ...
    if (block_count_u64 > limits.max_blocks_per_frame
        ...
        result.error = RansFrameCodecError::body_encode_error;
        result.encode_error = entropy::internal::RansEncodeError::limit_exceeded;
        return result;
    }
    ...
    for (std::size_t block = 0; block < result.block_count; ++block) {
        ...
        const auto planned = entropy::internal::plan_rans_block(
            input.subspan(input_offset, size), limits, descriptor);
        if (planned.error != entropy::internal::RansEncodeError::none) {
    ...
    if (!core::checked_multiply(
            result.block_count, entropy::internal::rans_descriptor_size,
            descriptor_size)
    ...
    if (result.header_error != FrameHeaderError::none) {
        result.error = result.header_error == FrameHeaderError::unexpected_frame_size
            ? RansFrameCodecError::input_size_mismatch
...
RansFrameCodecResult encode_rans_frame(
    const StreamHeader& stream, const core::DecoderLimits& limits,
    const std::uint64_t sequence,
    const std::uint64_t output_already_committed,
    const std::span<const std::byte> input,
    const std::span<std::byte> output) noexcept {
    auto result = plan_rans_frame(
        stream, limits, sequence, output_already_committed, input);
    if (result.error != RansFrameCodecError::none) return result;
    if (output.size() < result.serialized_size) {
        result.error = RansFrameCodecError::output_too_small;
        return result;
    }
    ...
    if (serialize_frame_header(header, context, header_output)
        ...
        result.error = RansFrameCodecError::internal_error;
        return result;
    }
    std::size_t input_offset{};
    ...
    for (std::size_t block = 0; block < result.block_count; ++block) {
        ...
        const auto block_plan = entropy::internal::plan_rans_block(
            input.subspan(input_offset, size), limits, descriptor);
        const std::span<std::byte, entropy::internal::rans_descriptor_size>
            descriptor_output{
                output.data() + frame_header_size
                    + block * entropy::internal::rans_descriptor_size,
...
RansFrameCodecResult validate_rans_frame(
    const StreamHeader& stream, const core::DecoderLimits& limits,
    const std::uint64_t expected_sequence,
    const std::uint64_t output_already_committed,
    const std::span<const std::byte> input,
    ...
    for (std::size_t block = 0; block < result.block_count; ++block) {
        ...
        if (validated.error != entropy::internal::RansDecodeError::none) {
            ...
            result.error = RansFrameCodecError::body_decode_error;
            return result;
        }
    }
    result.block_index = result.block_count;
...
RansFrameCodecResult decode_rans_frame(
    const StreamHeader& stream, const core::DecoderLimits& limits,
    const std::uint64_t expected_sequence,
    const std::uint64_t output_already_committed,
    const std::span<const std::byte> input,
