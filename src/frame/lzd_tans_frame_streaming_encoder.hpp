#ifndef MARC_FRAME_LZD_TANS_FRAME_STREAMING_ENCODER_HPP
#define MARC_FRAME_LZD_TANS_FRAME_STREAMING_ENCODER_HPP
...
inline constexpr std::size_t lzd_tans_stream_prefix_size =
    stream_header_size + dictionary::internal::lzd_parameter_size;

class LzdTansFrameStreamingEncoder final : public core::Transform {
public:
    LzdTansFrameStreamingEncoder(
        StreamHeader stream,
...
private:
    ...
    std::array<std::byte, lzd_tans_stream_prefix_size> prefix_{};
    std::size_t frame_input_size_{};
    std::size_t pending_size_{lzd_tans_stream_prefix_size};
