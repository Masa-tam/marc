...
LzwRansFrameStreamingDecoder::LzwRansFrameStreamingDecoder(
...
core::ProcessResult LzwRansFrameStreamingDecoder::fail(
...
bool LzwRansFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool LzwRansFrameStreamingDecoder::parse_collected_frame_header() noexcept {
...
bool LzwRansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzw_rans_frame_to_staging(
    ...
    if (decoded.error != LzwRansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzwRansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lzw_rans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzw_rans_stream_prefix_size) {
