...
LzssContextualTansFrameDecodeResult decode_lzss_contextual_tans_frame(
    ...
    LzssContextualTansFrameDecodeResult result{};
    LzssContextualTansFrameLayout layout{};
    result.preflight = preflight_lzss_contextual_tans_frame(
    ...
    if (!core::checked_multiply(
            tables.size(), sizeof(entropy::internal::TansDecodeEntry),
            table_bytes)
        || !core::checked_multiply(
            tokens.size(), sizeof(dictionary::internal::LzssTypedToken),
            token_bytes)) {
        result.error =
            LzssContextualTansFrameDecodeError::arithmetic_overflow;
        return result;
    }
    ...
    const auto payload = serialized_frame.subspan(
        lzss_contextual_tans_frame_header_size + descriptor_size,
