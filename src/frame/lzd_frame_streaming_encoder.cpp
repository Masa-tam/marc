...
LzdFrameStreamingEncoder::LzdFrameStreamingEncoder(
...
core::ProcessResult LzdFrameStreamingEncoder::fail(
...
bool LzdFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    if (plan.error != LzdFrameCodecError::none) {
        if (plan.error == LzdFrameCodecError::input_size_mismatch) {
            preparation_error_ = core::ErrorCode::invalid_argument;
        } else if (plan.error == LzdFrameCodecError::limit_exceeded
                   || plan.error == LzdFrameCodecError::arithmetic_overflow
                   || plan.encode_error
                       == dictionary::internal::LzdEncodeError::input_limit_exceeded
                   || plan.encode_error
                       == dictionary::internal::LzdEncodeError::serialized_limit_exceeded
                   || plan.encode_error
                       == dictionary::internal::LzdEncodeError::workspace_limit_exceeded
                   || plan.encode_error
                       == dictionary::internal::LzdEncodeError::arithmetic_overflow) {
            preparation_error_ = core::ErrorCode::limit_exceeded;
        } else if (plan.encode_error
                   == dictionary::internal::LzdEncodeError::workspace_too_small) {
    ...
    if (encoded.error != LzdFrameCodecError::none) return false;
...
core::ProcessResult LzdFrameStreamingEncoder::process(
