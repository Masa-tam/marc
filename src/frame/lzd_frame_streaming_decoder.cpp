...
LzdFrameStreamingDecoder::LzdFrameStreamingDecoder(
...
core::ProcessResult LzdFrameStreamingDecoder::fail(
...
bool LzdFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool LzdFrameStreamingDecoder::parse_collected_frame_header() noexcept {
...
bool LzdFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzd_frame(
    ...
    if (decoded.error != LzdFrameCodecError::none) return false;
...
core::ProcessResult LzdFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzd_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzd_stream_prefix_size) {
