#ifndef MARC_FRAME_LZSS_RANS_FRAME_STREAMING_DECODER_HPP
#define MARC_FRAME_LZSS_RANS_FRAME_STREAMING_DECODER_HPP
...
class LzssRansFrameStreamingDecoder final : public core::Transform {
public:
    LzssRansFrameStreamingDecoder(
        core::DecoderLimits limits,
        std::span<std::byte> frame_encoded_storage,
        std::span<entropy::internal::RansBlockView> frame_views,
        std::span<std::byte> dictionary_staging,
        std::span<std::byte> frame_decoded_storage) noexcept;
...
private:
    ...
    std::array<std::byte, lzss_rans_stream_prefix_size> prefix_bytes_{};
