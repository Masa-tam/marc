...
Lz77DynamicRangeFrameStreamingDecoder::
Lz77DynamicRangeFrameStreamingDecoder(
...
core::ProcessResult Lz77DynamicRangeFrameStreamingDecoder::fail(
...
bool Lz77DynamicRangeFrameStreamingDecoder::parse_collected_prefix()
    noexcept {
...
bool Lz77DynamicRangeFrameStreamingDecoder::
...
bool Lz77DynamicRangeFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const auto decoded = decode_lz77_dynamic_range_frame_to_staging(
    ...
    if (decoded.error != Lz77DynamicRangeFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult Lz77DynamicRangeFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lz77_dynamic_range_stream_prefix_size
                - header_collected_;
            ...
            if (header_collected_
                != lz77_dynamic_range_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream,
                                consumed, produced);
