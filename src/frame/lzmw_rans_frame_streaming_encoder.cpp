...
LzmwRansFrameStreamingEncoder::LzmwRansFrameStreamingEncoder(
    ...
    if (validate_stream_header(stream_, limits_) != StreamHeaderError::none
        ...
        || dictionary::internal::validate_lzmw_parameters(parameters_, limits_)
...
core::ProcessResult LzmwRansFrameStreamingEncoder::fail(
...
bool LzmwRansFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    const auto plan = plan_lzmw_rans_frame(
        ...
        dictionary_staging_);
    if (plan.error != LzmwRansFrameValidationError::none) {
        switch (plan.error) {
        case LzmwRansFrameValidationError::input_size_mismatch:
        case LzmwRansFrameValidationError::unsupported_pipeline:
            preparation_error_ = core::ErrorCode::invalid_argument;
            break;
        case LzmwRansFrameValidationError::encoder_workspace_too_small:
        case LzmwRansFrameValidationError::dictionary_staging_too_small:
        case LzmwRansFrameValidationError::serialized_output_too_small:
            preparation_error_ = core::ErrorCode::out_of_memory;
            break;
        case LzmwRansFrameValidationError::workspace_limit:
        case LzmwRansFrameValidationError::invalid_dictionary_extent:
        case LzmwRansFrameValidationError::invalid_entropy_extent:
        case LzmwRansFrameValidationError::dictionary_encode_error:
        case LzmwRansFrameValidationError::entropy_encode_error:
        case LzmwRansFrameValidationError::header_error:
        case LzmwRansFrameValidationError::arithmetic_overflow:
    ...
    const auto encoded = encode_lzmw_rans_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_), encoder_workspace_,
        dictionary_staging_,
        frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzmwRansFrameValidationError::none) return false;
...
core::ProcessResult LzmwRansFrameStreamingEncoder::process(
    ...
    while (true) {
        ...
        const auto count = std::min(
            expected_frame - frame_input_size_, input.size() - consumed);
        if (count != 0) {
