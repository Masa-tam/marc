#ifndef MARC_FRAME_LZW_BLOCKED_HUFFMAN_FRAME_STREAMING_ENCODER_HPP
#define MARC_FRAME_LZW_BLOCKED_HUFFMAN_FRAME_STREAMING_ENCODER_HPP
...
inline constexpr std::size_t lzw_blocked_huffman_stream_prefix_size =
    stream_header_size + dictionary::internal::lzw_parameter_size;

class LzwBlockedHuffmanFrameStreamingEncoder final : public core::Transform {
public:
  LzwBlockedHuffmanFrameStreamingEncoder(
      StreamHeader stream, dictionary::internal::LzwParameters parameters,
  ...
  process(std::span<const std::byte> input, std::span<std::byte> output,
...
private:
  ...
  dictionary::internal::LzwParameters parameters_{};
  ...
  std::span<dictionary::internal::LzwEncoderEntry> encoder_workspace_{};
  std::array<std::byte, lzw_blocked_huffman_stream_prefix_size> prefix_{};
  std::size_t frame_input_size_{};
  std::size_t pending_size_{lzw_blocked_huffman_stream_prefix_size};
