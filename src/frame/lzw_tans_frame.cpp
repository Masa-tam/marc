...
LzwTansFrameValidationResult plan_lzw_tans_frame(
    ...
    if (input.empty()
        ...
        result.error = LzwTansFrameValidationError::input_size_mismatch;
        return result;
    }
    ...
    if (dictionary_plan.error
        ...
        result.error = LzwTansFrameValidationError::dictionary_encode_error;
        return result;
    }
    ...
    if (dictionary_encoded.error
        ...
        result.error = dictionary_encoded.error
                == dictionary::internal::LzwEncodeError::none
            ? LzwTansFrameValidationError::internal_error
            : LzwTansFrameValidationError::dictionary_encode_error;
        return result;
    }
    ...
    if (block_count_u64 > limits.max_blocks_per_frame
        ...
        result.error = LzwTansFrameValidationError::entropy_encode_error;
        return result;
    }
    ...
    if (result.header_error != FrameHeaderError::none) {
        result.error = result.header_error
            ...
            : LzwTansFrameValidationError::header_error;
        return result;
    }
    if (!core::checked_add(
...
LzwTansFrameValidationResult encode_lzw_tans_frame(
    const StreamHeader& stream,
    const dictionary::internal::LzwParameters& parameters,
    const core::DecoderLimits& limits,
    const std::uint64_t sequence,
    ...
    auto result = plan_lzw_tans_frame(
...
[[nodiscard]] LzwTansFrameValidationResult validate_frame(
    ...
    if (require_output && output.size() < result.raw_size) {
        result.error = LzwTansFrameValidationError::raw_output_too_small;
        return result;
    }
...
LzwTansFrameValidationResult decode_lzw_tans_frame(
    const StreamHeader& stream,
    const dictionary::internal::LzwParameters& parameters,
    const core::DecoderLimits& limits,
    const std::uint64_t expected_sequence,
