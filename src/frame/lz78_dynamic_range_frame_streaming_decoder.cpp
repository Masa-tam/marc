...
Lz78DynamicRangeFrameStreamingDecoder::
Lz78DynamicRangeFrameStreamingDecoder(
...
core::ProcessResult Lz78DynamicRangeFrameStreamingDecoder::fail(
...
bool Lz78DynamicRangeFrameStreamingDecoder::parse_collected_prefix()
    noexcept {
    ...
    if (stream_.dictionary_algorithm != DictionaryAlgorithm::lz78
        ...
        || stream_.frame_size > (UINT32_C(1) << 21)
...
bool Lz78DynamicRangeFrameStreamingDecoder::
parse_collected_frame_header() noexcept {
    ...
    if (!core::checked_multiply(
        ...
        || frame_.compressed_payload_size < 5
...
bool Lz78DynamicRangeFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const auto decoded = decode_lz78_dynamic_range_frame_to_staging(
    ...
    if (decoded.error != Lz78DynamicRangeFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult Lz78DynamicRangeFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lz78_dynamic_range_stream_prefix_size
                - header_collected_;
            ...
            if (header_collected_
                != lz78_dynamic_range_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream,
                                consumed, produced);
