...
core::ProcessResult LzmwAdaptiveHuffmanFrameStreamingDecoder::fail(
...
bool LzmwAdaptiveHuffmanFrameStreamingDecoder::parse_collected_prefix()
    noexcept {
...
bool LzmwAdaptiveHuffmanFrameStreamingDecoder::
...
bool LzmwAdaptiveHuffmanFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const auto decoded = decode_lzmw_adaptive_huffman_frame_to_staging(
    ...
    if (decoded.error != LzmwAdaptiveHuffmanFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzmwAdaptiveHuffmanFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzmw_adaptive_huffman_stream_prefix_size
                - header_collected_;
