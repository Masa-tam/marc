...
[[nodiscard]] bool is_limit_error(
    const LzssContextualAdaptiveHuffmanFramePreflightResult& result) noexcept {
    return result.error
...
[[nodiscard]] constexpr bool valid_admission(
    ...
    case LzssContextualAdaptiveHuffmanStreamAdmission::field_context_64m:
        return true;
    }
    return false;
}
...
LzssContextualAdaptiveHuffmanFrameStreamingDecoder(
    ...
    const bool valid_extents = core::checked_multiply(
        ...
        && core::checked_multiply(
            symbol_workspace_.size(), sizeof(std::uint16_t), symbol_bytes)
        && core::checked_multiply(
    ...
    const std::array overlaps{
        valid_extents
            ? regions_overlap(
                  ...
                  serialized_frame_workspace_.size(), node_workspace_.data(),
                  node_bytes)
        ...
        valid_extents
            ? regions_overlap(
                  ...
                  symbol_workspace_.data(), symbol_bytes)
            : OverlapCheck::arithmetic_overflow,
        valid_extents
            ? regions_overlap(
                  serialized_frame_workspace_.data(),
        ...
        valid_extents
            ? regions_overlap(
                  node_workspace_.data(), node_bytes,
                  symbol_workspace_.data(), symbol_bytes)
            : OverlapCheck::arithmetic_overflow,
        valid_extents
            ? regions_overlap(
    ...
    case LzssContextualAdaptiveHuffmanStreamAdmission::field_context_64m:
        minimum_node_count =
            entropy::internal::contextual_adaptive_huffman_node_entries_v5;
        minimum_symbol_count =
            entropy::internal::contextual_adaptive_huffman_symbol_entries_v5;
        break;
    }
...
core::ProcessResult
LzssContextualAdaptiveHuffmanFrameStreamingDecoder::fail(
...
parse_collected_stream_header() noexcept {
    std::size_t consumed{};
    const auto error = parse_lzss_contextual_adaptive_huffman_stream_header(
    ...
    case LzssContextualAdaptiveHuffmanStreamAdmission::field_context_16m:
        ...
        break;
    case LzssContextualAdaptiveHuffmanStreamAdmission::field_context_64m:
        if (stream_.dictionary_variant != 6
            || stream_.context_algorithm != 1
...
bool LzssContextualAdaptiveHuffmanFrameStreamingDecoder::
prepare_collected_frame() noexcept {
    preparation_error_ = core::ErrorCode::malformed_stream;
    const LzssContextualAdaptiveHuffmanFrameValidationContext validation_context{
    ...
    const auto header_error =
        parse_lzss_contextual_adaptive_huffman_frame_header(
            frame_header_bytes_, validation_context, frame_, consumed);
    if (header_error
    ...
    if (!core::checked_add(
            ...
            static_cast<std::uint64_t>(node_bytes), buffered_bytes)
        || !core::checked_add(
            buffered_bytes, static_cast<std::uint64_t>(symbol_bytes),
            buffered_bytes)
        || !core::checked_add(
    ...
    std::memmove(
        ...
        lzss_contextual_adaptive_huffman_frame_header_size);
    frame_collected_ = lzss_contextual_adaptive_huffman_frame_header_size;
    return true;
}

bool LzssContextualAdaptiveHuffmanFrameStreamingDecoder::
decode_collected_frame() noexcept {
    const LzssContextualAdaptiveHuffmanFrameValidationContext validation_context{
        stream_, limits_, frame_sequence_, output_committed_};
    ...
    if (decoded.error
        ...
        if (decoded.error
            ...
            preparation_error_ = core::ErrorCode::internal_error;
                       ...
                       == LzssContextualAdaptiveHuffmanFrameDecodeError::
                           token_decode_error
                   && decoded.token_decode.error
                       == context::internal::
                           LzssContextualAdaptiveHuffmanDecodeError::
                               limit_exceeded) {
            preparation_error_ = core::ErrorCode::limit_exceeded;
        } else if (is_limit_error(decoded.preflight)) {
            preparation_error_ = core::ErrorCode::limit_exceeded;
        } else {
            preparation_error_ = core::ErrorCode::malformed_stream;
        }
...
core::ProcessResult
LzssContextualAdaptiveHuffmanFrameStreamingDecoder::process(
    ...
    const std::array output_overlaps{
        regions_overlap(
            ...
            serialized_frame_workspace_.size()),
        regions_overlap(
            output.data(), output.size(), node_workspace_.data(), node_bytes),
        regions_overlap(
            output.data(), output.size(), symbol_workspace_.data(),
            symbol_bytes),
        regions_overlap(
            output.data(), output.size(), token_workspace_.data(), token_bytes),
        regions_overlap(
