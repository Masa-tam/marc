...
[[nodiscard]] bool is_limit_failure(
    ...
    return result.error == LzssTypedContextFrameEncodeError::workspace_limit
        || (result.error
        ...
        || (result.error
            ...
            && result.context_encode.error
                == context::internal::LzssFieldContextError::limit_exceeded)
        || (result.error
                == LzssTypedContextFrameEncodeError::entropy_encode_error
        ...
        || (result.error == LzssTypedContextFrameEncodeError::header_error
            && result.header_error
                == TypedContextFrameHeaderError::limit_exceeded)
        || (result.error
                == LzssTypedContextFrameEncodeError::descriptor_error
...
LzssTypedContextFrameStreamingEncoder(
    ...
    const bool valid_extents = native_extent(
        ...
        && native_extent(
            operation_workspace_.size(),
            sizeof(context::internal::ModeledOperation), operation_bytes);
    const auto raw_tokens = valid_extents
        ? regions_overlap(raw_frame_workspace_.data(),
                          raw_frame_workspace_.size(), token_workspace_.data(),
                          token_bytes)
        : OverlapCheck::arithmetic_overflow;
    const auto raw_operations = valid_extents
        ? regions_overlap(raw_frame_workspace_.data(),
    ...
    const auto tokens_operations = valid_extents
        ? regions_overlap(token_workspace_.data(), token_bytes,
                          operation_workspace_.data(), operation_bytes)
        : OverlapCheck::arithmetic_overflow;
    const auto tokens_serialized = valid_extents
        ? regions_overlap(token_workspace_.data(), token_bytes,
                          serialized_frame_workspace_.data(),
                          serialized_frame_workspace_.size())
        : OverlapCheck::arithmetic_overflow;
...
core::ProcessResult LzssTypedContextFrameStreamingEncoder::fail(
...
bool LzssTypedContextFrameStreamingEncoder::output_is_disjoint(
    const std::span<std::byte> output) const noexcept {
    std::size_t token_bytes{};
    std::size_t operation_bytes{};
    if (!native_extent(token_workspace_.size(),
                       sizeof(dictionary::internal::LzssTypedToken),
                       token_bytes)
        || !native_extent(operation_workspace_.size(),
                          sizeof(context::internal::ModeledOperation),
                          operation_bytes)) {
        return false;
    }
    return regions_overlap(output.data(), output.size(),
...
bool LzssTypedContextFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    if (plan.error != LzssTypedContextFrameEncodeError::none) {
...
core::ProcessResult LzssTypedContextFrameStreamingEncoder::process(
    ...
    while (true) {
        ...
        const auto count = std::min(expected_frame - raw_frame_size_,
                                    input.size() - consumed);
        if (count != 0) {
            std::memmove(raw_frame_workspace_.data() + raw_frame_size_,
