...
LzwRansFrameStreamingEncoder::LzwRansFrameStreamingEncoder(
...
core::ProcessResult LzwRansFrameStreamingEncoder::fail(
...
bool LzwRansFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    const auto plan = plan_lzw_rans_frame(
        ...
        dictionary_staging_);
    if (plan.error != LzwRansFrameValidationError::none) {
        switch (plan.error) {
        case LzwRansFrameValidationError::input_size_mismatch:
        case LzwRansFrameValidationError::unsupported_pipeline:
            preparation_error_ = core::ErrorCode::invalid_argument;
            break;
        case LzwRansFrameValidationError::encoder_workspace_too_small:
        case LzwRansFrameValidationError::dictionary_staging_too_small:
        case LzwRansFrameValidationError::serialized_output_too_small:
            preparation_error_ = core::ErrorCode::out_of_memory;
            break;
        case LzwRansFrameValidationError::workspace_limit:
        case LzwRansFrameValidationError::invalid_dictionary_extent:
        case LzwRansFrameValidationError::invalid_entropy_extent:
        case LzwRansFrameValidationError::dictionary_encode_error:
        case LzwRansFrameValidationError::entropy_encode_error:
        case LzwRansFrameValidationError::header_error:
        case LzwRansFrameValidationError::arithmetic_overflow:
    ...
    const auto encoded = encode_lzw_rans_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_), encoder_workspace_,
        dictionary_staging_,
        frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzwRansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzwRansFrameStreamingEncoder::process(
