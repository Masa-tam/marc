...
Lz78RansFrameStreamingDecoder::Lz78RansFrameStreamingDecoder(
...
core::ProcessResult Lz78RansFrameStreamingDecoder::fail(
...
bool Lz78RansFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool Lz78RansFrameStreamingDecoder::parse_collected_frame_header()
    noexcept {
...
bool Lz78RansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lz78_rans_frame_to_staging(
    ...
    if (decoded.error != Lz78RansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult Lz78RansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lz78_rans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lz78_rans_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
