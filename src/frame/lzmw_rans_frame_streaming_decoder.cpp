...
LzmwRansFrameStreamingDecoder::LzmwRansFrameStreamingDecoder(
...
core::ProcessResult LzmwRansFrameStreamingDecoder::fail(
...
bool LzmwRansFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool LzmwRansFrameStreamingDecoder::
...
bool LzmwRansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzmw_rans_frame_to_staging(
    ...
    if (decoded.error != LzmwRansFrameValidationError::none) return false;
...
core::ProcessResult LzmwRansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lzmw_rans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzmw_rans_stream_prefix_size) {
