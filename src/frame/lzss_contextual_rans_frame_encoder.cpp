...
LzssContextualRansFrameEncodeResult plan_lzss_contextual_rans_frame(
    ...
    result.header_error =
        validate_lzss_contextual_rans_frame_header(header, frame_context);
    ...
    result.descriptor_error =
        entropy::internal::validate_contextual_rans_descriptor(
            descriptor, header.decision_count, header.payload_size, limits);
...
LzssContextualRansFrameEncodeResult encode_lzss_contextual_rans_frame(
    ...
    result.descriptor_error =
        entropy::internal::serialize_contextual_rans_descriptor(
            descriptor, header.decision_count, header.payload_size, limits,
            std::span<
                std::byte, entropy::internal::contextual_rans_descriptor_size>{
                output.data() + lzss_contextual_rans_frame_header_size,
                entropy::internal::contextual_rans_descriptor_size});
    if (result.descriptor_error
        != entropy::internal::ContextualRansFormatError::none) {
