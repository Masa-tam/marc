...
Lz77FrameStreamingEncoder::Lz77FrameStreamingEncoder(
...
core::ProcessResult Lz77FrameStreamingEncoder::fail(
...
bool Lz77FrameStreamingEncoder::prepare_frame() noexcept {
    preparation_error_ = core::ErrorCode::internal_error;
    const auto plan = plan_lz77_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_));
    if (plan.error != Lz77FrameCodecError::none) {
        if (plan.error == Lz77FrameCodecError::input_size_mismatch)
            preparation_error_ = core::ErrorCode::invalid_argument;
        else if (plan.encode_error
                 == dictionary::internal::Lz77EncodeError::input_limit_exceeded
                 || plan.encode_error
                    == dictionary::internal::Lz77EncodeError::serialized_limit_exceeded)
    ...
    if (encoded.error != Lz77FrameCodecError::none) return false;
...
core::ProcessResult Lz77FrameStreamingEncoder::process(
