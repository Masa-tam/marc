...
LzssContextualBlockedHuffmanFrameDecodeResult
decode_lzss_contextual_blocked_huffman_frame(
    ...
    using E = LzssContextualBlockedHuffmanFrameDecodeError;
    LzssContextualBlockedHuffmanFrameDecodeResult result{};
    LzssContextualBlockedHuffmanFrameLayout layout{};
    result.preflight = preflight_lzss_contextual_blocked_huffman_frame(
    ...
    result.required_table_entries = entropy::internal::
    ...
    if (private_tables.size() < result.required_table_entries) {
        result.error = E::table_output_too_small;
        return result;
    }
    ...
    if (!core::checked_multiply(
            tables.size(), sizeof(entropy::internal::HuffmanDecodeTable),
            table_bytes)
    ...
    const context::internal::LzssFieldContextValidationContext token_context{
        layout.header.token_count, layout.header.event_count,
        layout.header.decision_count, layout.header.uncompressed_size,
