...
Lz78TansFrameStreamingDecoder::Lz78TansFrameStreamingDecoder(
...
core::ProcessResult Lz78TansFrameStreamingDecoder::fail(
...
bool Lz78TansFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool Lz78TansFrameStreamingDecoder::parse_collected_frame_header()
    noexcept {
    ...
    if (!core::checked_multiply(
            ...
            UINT64_C(8), maximum_dictionary_size)
...
bool Lz78TansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lz78_tans_frame_to_staging(
    ...
    if (decoded.error != Lz78TansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult Lz78TansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lz78_tans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lz78_tans_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
