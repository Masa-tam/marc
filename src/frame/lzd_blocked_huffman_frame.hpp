...
// Fixes the complete canonical LZD token stream in dictionary_staging before
// planning Blocked Huffman over those exact bytes. Raw input and staging must
// not overlap.
[[nodiscard]] LzdBlockedHuffmanFrameValidationResult
plan_lzd_blocked_huffman_frame(
...
[[nodiscard]] LzdBlockedHuffmanFrameValidationResult
encode_lzd_blocked_huffman_frame(
    const StreamHeader& stream,
    const dictionary::internal::LzdParameters& parameters,
    const core::DecoderLimits& limits, std::uint64_t sequence,
    std::uint64_t output_already_committed,
