...
[[nodiscard]] bool decoder_views_layout(
    ...
    return core::checked_multiply(block_count, sizeof(Block), block_bytes)
        ...
        && core::checked_multiply(
            phrase_count, sizeof(Phrase), phrase_bytes)
        && core::checked_add(phrase_offset, phrase_bytes, total_bytes);
}

} // namespace

Lz78RansProfileError make_lz78_rans_profile(
    ...
    if (core::validate_limits(limits) != core::LimitError::none
        ...
        return Lz78RansProfileError::invalid_configuration;
    }
    ...
    if (config.original_size > limits.max_total_output_size
        ...
        return Lz78RansProfileError::limit_exceeded;
    }
    ...
    if (!core::checked_multiply(
        ...
        || !core::checked_multiply(
            ...
            entry_bytes)) {
        return Lz78RansProfileError::arithmetic_overflow;
    }
    ...
    if (!to_size(largest_frame, workspace.frame_input_bytes)
        ...
        || !to_size(
            frame_encoded_bytes, workspace.frame_encoded_bytes)
...
Lz78RansProfileError calculate_lz78_rans_decoder_workspace(
    const core::DecoderLimits& limits,
...
Lz78RansWorkspaceError partition_lz78_rans_encoder_views(
    ...
    if (!core::checked_multiply(
            ...
            sizeof(dictionary::internal::Lz78EncoderEntry),
            expected_bytes)) {
        return Lz78RansWorkspaceError::arithmetic_overflow;
    }
    if (expected_bytes == 0) {
    ...
    if (expected_bytes != requirements.views_bytes
        || requirements.views_alignment
               != alignof(dictionary::internal::Lz78EncoderEntry)) {
        return Lz78RansWorkspaceError::invalid_requirements;
    }
    if (storage.size() < expected_bytes) {
        return Lz78RansWorkspaceError::too_small;
    }
    if (!aligned(storage.data(), requirements.views_alignment)) {
        return Lz78RansWorkspaceError::misaligned;
    }
    views.entries = {
        reinterpret_cast<dictionary::internal::Lz78EncoderEntry*>(
            storage.data()),
        requirements.encoder_entry_count};
    return Lz78RansWorkspaceError::none;
}

Lz78RansWorkspaceError partition_lz78_rans_decoder_views(
...
core::ErrorCode lz78_rans_profile_error_code(
    const Lz78RansProfileError error) noexcept {
    switch (error) {
    case Lz78RansProfileError::none:
        return core::ErrorCode::none;
    case Lz78RansProfileError::invalid_configuration:
        return core::ErrorCode::invalid_argument;
    case Lz78RansProfileError::unsupported:
        return core::ErrorCode::unsupported;
    case Lz78RansProfileError::limit_exceeded:
    case Lz78RansProfileError::arithmetic_overflow:
