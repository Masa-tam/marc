...
LzdRansFrameStreamingDecoder::LzdRansFrameStreamingDecoder(
...
core::ProcessResult LzdRansFrameStreamingDecoder::fail(
...
bool LzdRansFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool LzdRansFrameStreamingDecoder::
parse_collected_frame_header() noexcept {
    ...
    if (!core::checked_multiply(
        ...
        || !core::checked_add(
            ...
            state_bytes, maximum_payload_size)
...
bool LzdRansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzd_rans_frame_to_staging(
    ...
    if (decoded.error != LzdRansFrameValidationError::none) return false;
...
core::ProcessResult LzdRansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lzd_rans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzd_rans_stream_prefix_size) {
