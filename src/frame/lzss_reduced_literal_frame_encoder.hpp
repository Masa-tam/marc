#ifndef MARC_FRAME_LZSS_REDUCED_LITERAL_FRAME_ENCODER_HPP
#define MARC_FRAME_LZSS_REDUCED_LITERAL_FRAME_ENCODER_HPP

#include "frame/lzss_short_match_frame_encoder.hpp"

namespace marc::frame::internal {

// Private complete-frame encoder for the exact 2/8 + 1/8 + 3/2 identity.
// No published stream encoder or public selector calls this entry point.
[[nodiscard]] LzssShortMatchFrameEncodeResult
plan_lzss_reduced_literal_frame(
...
[[nodiscard]] LzssShortMatchFrameEncodeResult
encode_lzss_reduced_literal_frame(
    const TypedContextStreamHeader& stream,
    const core::DecoderLimits& limits,
    std::uint64_t sequence,
    std::uint64_t raw_already_committed,
