...
core::ProcessResult LzdBlockedHuffmanFrameStreamingEncoder::fail(
...
bool LzdBlockedHuffmanFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    const auto plan = plan_lzd_blocked_huffman_frame(
        ...
        dictionary_staging_);
    if (plan.error != LzdBlockedHuffmanFrameValidationError::none) {
        if (plan.error
    ...
    const auto encoded = encode_lzd_blocked_huffman_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_), encoder_workspace_,
        dictionary_staging_,
        frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzdBlockedHuffmanFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzdBlockedHuffmanFrameStreamingEncoder::process(
