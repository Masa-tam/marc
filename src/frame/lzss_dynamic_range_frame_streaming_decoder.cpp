...
core::ProcessResult LzssDynamicRangeFrameStreamingDecoder::fail(
...
bool LzssDynamicRangeFrameStreamingDecoder::parse_collected_prefix()
    noexcept {
    ...
    if (stream_.dictionary_algorithm != DictionaryAlgorithm::lzss
        ...
        || stream_.frame_size > (UINT32_C(1) << 23)
...
bool LzssDynamicRangeFrameStreamingDecoder::
parse_collected_frame_header() noexcept {
    ...
    if (!core::checked_multiply(
            static_cast<std::uint64_t>(frame_.uncompressed_size), UINT64_C(2),
...
bool LzssDynamicRangeFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const auto decoded = decode_lzss_dynamic_range_frame_to_staging(
    ...
    if (decoded.error != LzssDynamicRangeFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzssDynamicRangeFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzss_dynamic_range_stream_prefix_size
                - header_collected_;
            ...
            if (header_collected_
                != lzss_dynamic_range_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream,
                                consumed, produced);
