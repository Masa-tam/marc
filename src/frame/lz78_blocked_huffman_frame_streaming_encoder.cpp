...
core::ProcessResult Lz78BlockedHuffmanFrameStreamingEncoder::fail(
...
bool Lz78BlockedHuffmanFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    const auto encoded = encode_lz78_blocked_huffman_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_), encoder_workspace_,
        dictionary_staging_,
        frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != Lz78BlockedHuffmanFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult Lz78BlockedHuffmanFrameStreamingEncoder::process(
