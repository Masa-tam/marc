...
[[nodiscard]] bool block_payload_ceiling(
    const std::uint64_t symbols, std::uint64_t& size) noexcept {
    std::uint64_t bits{};
    return core::checked_multiply(symbols, UINT64_C(12), bits)
...
[[nodiscard]] bool payload_ceiling(
    const std::uint64_t symbols, const std::uint64_t block_size,
    std::uint64_t& size) noexcept {
    const auto full_blocks = symbols / block_size;
    const auto tail = symbols % block_size;
    std::uint64_t per_full{};
    std::uint64_t full{};
    std::uint64_t final{};
    return block_payload_ceiling(block_size, per_full)
        && core::checked_multiply(full_blocks, per_full, full)
        && (tail == 0 || block_payload_ceiling(tail, final))
        && core::checked_add(full, final, size);
}

} // namespace

LzmwTansFrameStreamingDecoder::LzmwTansFrameStreamingDecoder(
...
core::ProcessResult LzmwTansFrameStreamingDecoder::fail(
...
bool LzmwTansFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool LzmwTansFrameStreamingDecoder::parse_collected_frame_header() noexcept {
    ...
    if (!core::checked_multiply(
        ...
        || !core::checked_multiply(
            ...
            static_cast<std::uint64_t>(
                entropy::internal::tans_min_payload_size),
            state_bytes)
        || !payload_ceiling(frame_.dictionary_serialized_size,
                            stream_.entropy_block_size, maximum_payload)
        || frame_.entropy_block_count != expected_blocks
        || frame_.block_descriptors_size != expected_descriptors
        || frame_.compressed_payload_size < state_bytes
        || frame_.compressed_payload_size > maximum_payload) {
        return false;
    }
    ...
    if (!core::checked_multiply(
            ...
            static_cast<std::uint64_t>(
                sizeof(entropy::internal::TansBlockView)), view_bytes)
        || !core::checked_multiply(
            static_cast<std::uint64_t>(required_phrases),
            static_cast<std::uint64_t>(
                sizeof(dictionary::internal::LzmwPhraseEntry)), phrase_bytes)
        || !core::checked_multiply(
            static_cast<std::uint64_t>(required_expansion),
            static_cast<std::uint64_t>(sizeof(std::uint32_t)), expansion_bytes)
        || !core::checked_add(
            static_cast<std::uint64_t>(serialized_size),
            static_cast<std::uint64_t>(frame_.dictionary_serialized_size),
            buffered_bytes)
        || !core::checked_add(
            buffered_bytes, static_cast<std::uint64_t>(frame_.uncompressed_size),
            buffered_bytes)
...
bool LzmwTansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzmw_tans_frame_to_staging(
    ...
    if (decoded.error != LzmwTansFrameValidationError::none) return false;
...
core::ProcessResult LzmwTansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzmw_tans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzmw_tans_stream_prefix_size) {
