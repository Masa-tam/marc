...
LzdTansFrameStreamingEncoder::LzdTansFrameStreamingEncoder(
...
core::ProcessResult LzdTansFrameStreamingEncoder::fail(
...
bool LzdTansFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    const auto plan = plan_lzd_tans_frame(
        ...
        dictionary_staging_);
    if (plan.error != LzdTansFrameValidationError::none) {
        switch (plan.error) {
        case LzdTansFrameValidationError::input_size_mismatch:
        case LzdTansFrameValidationError::unsupported_pipeline:
            preparation_error_ = core::ErrorCode::invalid_argument;
            break;
        case LzdTansFrameValidationError::encoder_workspace_too_small:
        case LzdTansFrameValidationError::dictionary_staging_too_small:
        case LzdTansFrameValidationError::serialized_output_too_small:
            preparation_error_ = core::ErrorCode::out_of_memory;
            break;
        case LzdTansFrameValidationError::workspace_limit:
        case LzdTansFrameValidationError::invalid_dictionary_extent:
        case LzdTansFrameValidationError::invalid_entropy_extent:
        case LzdTansFrameValidationError::dictionary_encode_error:
        case LzdTansFrameValidationError::entropy_encode_error:
        case LzdTansFrameValidationError::header_error:
        case LzdTansFrameValidationError::arithmetic_overflow:
    ...
    const auto encoded = encode_lzd_tans_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_), encoder_workspace_,
        dictionary_staging_,
        frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzdTansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzdTansFrameStreamingEncoder::process(
