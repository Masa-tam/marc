...
core::ProcessResult LzwDynamicRangeFrameStreamingDecoder::fail(
...
bool LzwDynamicRangeFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool LzwDynamicRangeFrameStreamingDecoder::
...
bool LzwDynamicRangeFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzw_dynamic_range_frame_to_staging(
    ...
    if (decoded.error != LzwDynamicRangeFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzwDynamicRangeFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzw_dynamic_range_stream_prefix_size
                - header_collected_;
            ...
            if (header_collected_ != lzw_dynamic_range_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream,
                                consumed, produced);
