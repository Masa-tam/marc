...
LzdRansFrameStreamingEncoder::LzdRansFrameStreamingEncoder(
    ...
    const bool workspace_valid =
        dictionary::internal::lzd_maximum_token_stream_size(
...
core::ProcessResult LzdRansFrameStreamingEncoder::fail(
...
bool LzdRansFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    const auto plan = plan_lzd_rans_frame(
        ...
        dictionary_staging_);
    if (plan.error != LzdRansFrameValidationError::none) {
        switch (plan.error) {
        case LzdRansFrameValidationError::input_size_mismatch:
        case LzdRansFrameValidationError::unsupported_pipeline:
            preparation_error_ = core::ErrorCode::invalid_argument;
            break;
        case LzdRansFrameValidationError::encoder_workspace_too_small:
        case LzdRansFrameValidationError::dictionary_staging_too_small:
        case LzdRansFrameValidationError::serialized_output_too_small:
            preparation_error_ = core::ErrorCode::out_of_memory;
            break;
        case LzdRansFrameValidationError::workspace_limit:
        case LzdRansFrameValidationError::invalid_dictionary_extent:
        case LzdRansFrameValidationError::invalid_entropy_extent:
        case LzdRansFrameValidationError::dictionary_encode_error:
        case LzdRansFrameValidationError::entropy_encode_error:
        case LzdRansFrameValidationError::header_error:
        case LzdRansFrameValidationError::arithmetic_overflow:
    ...
    const auto encoded = encode_lzd_rans_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_), encoder_workspace_,
        dictionary_staging_,
        frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzdRansFrameValidationError::none) return false;
...
core::ProcessResult LzdRansFrameStreamingEncoder::process(
