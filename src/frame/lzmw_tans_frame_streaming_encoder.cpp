...
LzmwTansFrameStreamingEncoder::LzmwTansFrameStreamingEncoder(
...
core::ProcessResult LzmwTansFrameStreamingEncoder::fail(
...
bool LzmwTansFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    const auto plan = plan_lzmw_tans_frame(
        ...
        dictionary_staging_);
    if (plan.error != LzmwTansFrameValidationError::none) {
        switch (plan.error) {
        case LzmwTansFrameValidationError::input_size_mismatch:
        case LzmwTansFrameValidationError::unsupported_pipeline:
            preparation_error_ = core::ErrorCode::invalid_argument;
            break;
        case LzmwTansFrameValidationError::encoder_workspace_too_small:
        case LzmwTansFrameValidationError::dictionary_staging_too_small:
        case LzmwTansFrameValidationError::serialized_output_too_small:
            preparation_error_ = core::ErrorCode::out_of_memory;
            break;
        case LzmwTansFrameValidationError::workspace_limit:
        case LzmwTansFrameValidationError::invalid_dictionary_extent:
        case LzmwTansFrameValidationError::invalid_entropy_extent:
        case LzmwTansFrameValidationError::dictionary_encode_error:
        case LzmwTansFrameValidationError::entropy_encode_error:
        case LzmwTansFrameValidationError::header_error:
        case LzmwTansFrameValidationError::arithmetic_overflow:
    ...
    const auto encoded = encode_lzmw_tans_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_), encoder_workspace_,
        dictionary_staging_,
        frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzmwTansFrameValidationError::none) return false;
...
core::ProcessResult LzmwTansFrameStreamingEncoder::process(
