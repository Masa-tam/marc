...
LzssDynamicRangeFrameStreamingEncoder::
LzssDynamicRangeFrameStreamingEncoder(
...
core::ProcessResult LzssDynamicRangeFrameStreamingEncoder::fail(
...
bool LzssDynamicRangeFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    if (plan.error != LzssDynamicRangeFrameValidationError::none) {
        switch (plan.error) {
        case LzssDynamicRangeFrameValidationError::input_size_mismatch:
        case LzssDynamicRangeFrameValidationError::unsupported_pipeline:
            preparation_error_ = core::ErrorCode::invalid_argument;
            break;
        case LzssDynamicRangeFrameValidationError::
            dictionary_staging_too_small:
        case LzssDynamicRangeFrameValidationError::
            serialized_output_too_small:
            preparation_error_ = core::ErrorCode::out_of_memory;
            break;
        case LzssDynamicRangeFrameValidationError::workspace_limit:
        case LzssDynamicRangeFrameValidationError::
            invalid_dictionary_extent:
        case LzssDynamicRangeFrameValidationError::invalid_entropy_extent:
        case LzssDynamicRangeFrameValidationError::dictionary_encode_error:
        case LzssDynamicRangeFrameValidationError::entropy_encode_error:
        case LzssDynamicRangeFrameValidationError::header_error:
        case LzssDynamicRangeFrameValidationError::arithmetic_overflow:
    ...
    const auto encoded = encode_lzss_dynamic_range_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_), dictionary_staging_,
        frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzssDynamicRangeFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzssDynamicRangeFrameStreamingEncoder::process(
