...
Lz77TansFrameStreamingDecoder::Lz77TansFrameStreamingDecoder(
...
core::ProcessResult Lz77TansFrameStreamingDecoder::fail(
...
bool Lz77TansFrameStreamingDecoder::parse_collected_prefix() noexcept {
    ...
    if (stream_.dictionary_algorithm != DictionaryAlgorithm::lz77
    ...
    const std::span<const std::byte,
        ...
        parameter_bytes{prefix_bytes_.data() + stream_header_size,
                        dictionary::internal::lz77_parameter_size};
    return dictionary::internal::parse_lz77_parameters(
               parameter_bytes, limits_, parameters_)
        == dictionary::internal::Lz77FormatError::none;
}

bool Lz77TansFrameStreamingDecoder::parse_collected_frame_header() noexcept {
...
bool Lz77TansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lz77_tans_frame_to_staging(
    ...
    if (decoded.error != Lz77TansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult Lz77TansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lz77_tans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lz77_tans_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream,
                                consumed, produced);
