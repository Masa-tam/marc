...
LzwFrameStreamingDecoder::LzwFrameStreamingDecoder(
...
core::ProcessResult LzwFrameStreamingDecoder::fail(
...
bool LzwFrameStreamingDecoder::parse_collected_prefix() noexcept {
    ...
    if (stream_.dictionary_algorithm != DictionaryAlgorithm::lzw
    ...
    const std::span<const std::byte,
        ...
        parameters{prefix_bytes_.data() + stream_header_size,
                   dictionary::internal::lzw_parameter_size};
    return dictionary::internal::parse_lzw_parameters(
               parameters, limits_, parameters_)
        == dictionary::internal::LzwFormatError::none;
}

bool LzwFrameStreamingDecoder::parse_collected_frame_header() noexcept {
    ...
    const auto required_entries =
        dictionary::internal::lzw_validation_workspace_entries(
            frame_.compressed_payload_size, parameters_);
    if (required_entries > dictionary_workspace_.size()) {
        preparation_error_ = core::ErrorCode::out_of_memory;
        return false;
    }
    ...
    if (!core::checked_add(static_cast<std::uint64_t>(serialized_size),
...
bool LzwFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzw_frame(
        stream_, parameters_, limits_, frame_sequence_, output_committed_,
        frame_encoded_storage_.first(frame_serialized_size_),
        dictionary_workspace_,
        frame_decoded_storage_.first(frame_.uncompressed_size));
    if (decoded.error != LzwFrameCodecError::none) return false;
...
core::ProcessResult LzwFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzw_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzw_stream_prefix_size) {
