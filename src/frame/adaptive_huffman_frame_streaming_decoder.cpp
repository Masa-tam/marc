...
AdaptiveHuffmanFrameStreamingDecoder::AdaptiveHuffmanFrameStreamingDecoder(
...
core::ProcessResult
AdaptiveHuffmanFrameStreamingDecoder::fail(core::ErrorCode c, std::size_t i,
                                           std::size_t o) noexcept {
  state_ = State::error;
  terminal_ = {c, input_pos_, 0};
  return {i, o, core::StreamStatus::error, terminal_};
}
bool AdaptiveHuffmanFrameStreamingDecoder::parse_stream() noexcept {
...
bool AdaptiveHuffmanFrameStreamingDecoder::prepare_frame() noexcept {
...
bool AdaptiveHuffmanFrameStreamingDecoder::decode_frame() noexcept {
  auto r = decode_adaptive_huffman_frame(
      stream_, limits_, sequence_, output_committed_,
      encoded_.first(frame_size_),
      decoded_.first(frame_.dictionary_serialized_size));
  if (r.error != AdaptiveHuffmanFrameCodecError::none)
    return false;
...
core::ProcessResult
AdaptiveHuffmanFrameStreamingDecoder::process(std::span<const std::byte> in,
