...
[[nodiscard]] bool align_up(
    const std::size_t value, const std::size_t alignment,
...
[[nodiscard]] bool payload_ceiling(
    const std::uint64_t symbol_count, const std::uint64_t block_size,
    std::uint64_t& payload_size) noexcept {
...
LzmwTansProfileError make_lzmw_tans_profile(
    ...
    if (core::validate_limits(limits) != core::LimitError::none
        ...
        return LzmwTansProfileError::invalid_configuration;
    }
    ...
    if (parameter_error != dictionary::internal::LzmwFormatError::none) {
        return parameter_error
            ...
            : LzmwTansProfileError::invalid_configuration;
    }
    if (config.original_size > limits.max_total_output_size
        ...
        return LzmwTansProfileError::limit_exceeded;
    }
    ...
    return LzmwTansProfileError::none;
}

LzmwTansProfileError calculate_lzmw_tans_decoder_workspace(
    const core::DecoderLimits& limits,
...
LzmwTansWorkspaceError partition_lzmw_tans_encoder_views(
    ...
    if (!core::checked_multiply(
        ...
        return LzmwTansWorkspaceError::arithmetic_overflow;
    }
    if (expected_bytes == 0) {
    ...
    if (expected_bytes != requirements.views_bytes
        ...
        return LzmwTansWorkspaceError::invalid_requirements;
    }
    if (storage.size() < expected_bytes) {
        return LzmwTansWorkspaceError::too_small;
    }
    if (!aligned(storage.data(), requirements.views_alignment)) {
        return LzmwTansWorkspaceError::misaligned;
    }
    views.entries = {
        reinterpret_cast<dictionary::internal::LzmwEncoderEntry*>(
            storage.data()),
        requirements.encoder_entry_count};
    return LzmwTansWorkspaceError::none;
}

LzmwTansWorkspaceError partition_lzmw_tans_decoder_views(
...
core::ErrorCode lzmw_tans_profile_error_code(
    const LzmwTansProfileError error) noexcept {
    switch (error) {
    case LzmwTansProfileError::none:
        return core::ErrorCode::none;
    case LzmwTansProfileError::invalid_configuration:
        return core::ErrorCode::invalid_argument;
    case LzmwTansProfileError::unsupported:
        return core::ErrorCode::unsupported;
    case LzmwTansProfileError::limit_exceeded:
    case LzmwTansProfileError::arithmetic_overflow:
