...
LzwTansFrameStreamingDecoder::LzwTansFrameStreamingDecoder(
...
core::ProcessResult LzwTansFrameStreamingDecoder::fail(
...
bool LzwTansFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool LzwTansFrameStreamingDecoder::parse_collected_frame_header() noexcept {
    ...
    if (!core::checked_multiply(
            ...
            packed_bits)
        || !core::checked_add(packed_bits, UINT64_C(7), rounded_packed_bits)
...
bool LzwTansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzw_tans_frame_to_staging(
    ...
    if (decoded.error != LzwTansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzwTansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lzw_tans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzw_tans_stream_prefix_size) {
