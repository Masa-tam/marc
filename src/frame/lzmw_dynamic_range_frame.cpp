...
[[nodiscard]] LzmwDynamicRangeFrameValidationResult validate_frame(
    ...
    if (input.size() < frame_header_size) {
        result.error =
            LzmwDynamicRangeFrameValidationError::truncated_frame;
        return result;
    }
    ...
    if (result.header_error != FrameHeaderError::none) {
        result.error = LzmwDynamicRangeFrameValidationError::header_error;
        return result;
    }
    ...
    if (result.descriptor_error
        ...
        result.error = LzmwDynamicRangeFrameValidationError::descriptor_error;
        return result;
    }
...
[[nodiscard]] bool reconstruct_validated_references(
    ...
    result.error =
        LzmwDynamicRangeFrameValidationError::dictionary_decode_error;
    return false;
}
...
LzmwDynamicRangeFrameValidationResult encode_lzmw_dynamic_range_frame(
    const StreamHeader& stream,
    const dictionary::internal::LzmwParameters& parameters,
    const core::DecoderLimits& limits,
    const std::uint64_t sequence,
    ...
    const std::span<std::byte> dictionary_staging,
    const std::span<std::byte> output) noexcept {
    auto result = plan_lzmw_dynamic_range_frame(
    ...
    if (entropy_plan.error
        ...
        result.entropy_encode_error = entropy_plan.error;
        result.error = LzmwDynamicRangeFrameValidationError::internal_error;
        return result;
    }
