...
LzdBlockedHuffmanFrameStreamingDecoder::
LzdBlockedHuffmanFrameStreamingDecoder(
...
core::ProcessResult LzdBlockedHuffmanFrameStreamingDecoder::fail(
...
bool LzdBlockedHuffmanFrameStreamingDecoder::parse_collected_prefix()
    noexcept {
...
bool LzdBlockedHuffmanFrameStreamingDecoder::
...
bool LzdBlockedHuffmanFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const auto decoded = decode_lzd_blocked_huffman_frame(
    ...
    if (decoded.error != LzdBlockedHuffmanFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzdBlockedHuffmanFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzd_blocked_huffman_stream_prefix_size
                - header_collected_;
            ...
            if (header_collected_
                != lzd_blocked_huffman_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream, consumed,
                                produced);
