...
LzmwFrameStreamingEncoder::LzmwFrameStreamingEncoder(
    ...
    if (largest_frame > std::numeric_limits<std::size_t>::max()) {
        state_ = State::error;
        terminal_error_ = {core::ErrorCode::limit_exceeded, 0, 0};
        return;
    }
    const auto frame_workspace = static_cast<std::size_t>(largest_frame);
    const auto dictionary_entries =
        dictionary::internal::lzmw_encoder_workspace_entries(
            frame_workspace, parameters_);
    if (frame_input_storage_.size() < frame_workspace
...
core::ProcessResult LzmwFrameStreamingEncoder::fail(
...
bool LzmwFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    if (plan.error != LzmwFrameCodecError::none) {
        if (plan.error == LzmwFrameCodecError::input_size_mismatch) {
            preparation_error_ = core::ErrorCode::invalid_argument;
        } else if (plan.error == LzmwFrameCodecError::limit_exceeded
                   || plan.error == LzmwFrameCodecError::arithmetic_overflow
                   || plan.encode_error
                       == dictionary::internal::LzmwEncodeError::input_limit_exceeded
                   || plan.encode_error
                       == dictionary::internal::LzmwEncodeError::serialized_limit_exceeded
                   || plan.encode_error
                       == dictionary::internal::LzmwEncodeError::workspace_limit_exceeded
                   || plan.encode_error
                       == dictionary::internal::LzmwEncodeError::arithmetic_overflow) {
            preparation_error_ = core::ErrorCode::limit_exceeded;
        } else if (plan.encode_error
                   == dictionary::internal::LzmwEncodeError::workspace_too_small) {
    ...
    if (encoded.error != LzmwFrameCodecError::none) return false;
...
core::ProcessResult LzmwFrameStreamingEncoder::process(
