...
LzdAdaptiveHuffmanFrameStreamingDecoder::
LzdAdaptiveHuffmanFrameStreamingDecoder(
...
core::ProcessResult LzdAdaptiveHuffmanFrameStreamingDecoder::fail(
...
bool LzdAdaptiveHuffmanFrameStreamingDecoder::parse_collected_prefix()
    noexcept {
...
bool LzdAdaptiveHuffmanFrameStreamingDecoder::
...
bool LzdAdaptiveHuffmanFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const auto decoded = decode_lzd_adaptive_huffman_frame_to_staging(
    ...
    if (decoded.error != LzdAdaptiveHuffmanFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzdAdaptiveHuffmanFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzd_adaptive_huffman_stream_prefix_size
                - header_collected_;
