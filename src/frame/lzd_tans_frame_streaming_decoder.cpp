...
LzdTansFrameStreamingDecoder::LzdTansFrameStreamingDecoder(
    const core::DecoderLimits limits,
    const std::span<std::byte> frame_encoded_storage,
    const std::span<entropy::internal::TansBlockView> frame_views,
    const std::span<std::byte> dictionary_staging,
    const std::span<std::byte> frame_decoded_storage,
    const std::span<dictionary::internal::LzdPhraseEntry> phrase_workspace,
    const std::span<std::uint32_t> expansion_workspace) noexcept
    : limits_(limits), frame_encoded_storage_(frame_encoded_storage),
      frame_views_(frame_views), dictionary_staging_(dictionary_staging),
...
core::ProcessResult LzdTansFrameStreamingDecoder::fail(
...
bool LzdTansFrameStreamingDecoder::parse_collected_prefix() noexcept {
    ...
    if (stream_.dictionary_algorithm != DictionaryAlgorithm::lzd
        || stream_.dictionary_variant != 1
        || stream_.entropy_algorithm != EntropyAlgorithm::tans
        || stream_.entropy_variant != 1
        || stream_.frame_size > (UINT32_C(1) << 20)
        || stream_.entropy_block_size == 0
        || stream_.entropy_block_size
    ...
    const std::span<const std::byte,
        ...
        parameter_bytes{prefix_bytes_.data() + stream_header_size,
                        dictionary::internal::lzd_parameter_size};
    return dictionary::internal::parse_lzd_parameters(
               parameter_bytes, limits_, parameters_)
        == dictionary::internal::LzdFormatError::none;
}

bool LzdTansFrameStreamingDecoder::
parse_collected_frame_header() noexcept {
    ...
    std::uint64_t rounded_raw_size{};
    std::uint64_t maximum_dictionary_size{};
    if (!core::checked_add(
            static_cast<std::uint64_t>(frame_.uncompressed_size), UINT64_C(1),
            rounded_raw_size)
        || !core::checked_multiply(
            rounded_raw_size / 2,
        ...
        || frame_.dictionary_serialized_size
               % dictionary::internal::lzd_token_size != 0
    ...
    const auto expected_blocks =
        UINT64_C(1)
        + (static_cast<std::uint64_t>(frame_.dictionary_serialized_size) - 1)
              / stream_.entropy_block_size;
    ...
    if (!core::checked_multiply(
        ...
        || !payload_ceiling(
            ...
            stream_.entropy_block_size, maximum_payload_size)
        || frame_.entropy_block_count != expected_blocks
        || frame_.block_descriptors_size != expected_descriptor_size
        || frame_.compressed_payload_size < state_bytes
    ...
    const auto required_phrases =
        dictionary::internal::lzd_validation_workspace_entries(
            frame_.dictionary_serialized_size, frame_.uncompressed_size,
            parameters_);
    const auto required_expansion =
        dictionary::internal::lzd_expansion_workspace_entries(
            required_phrases, frame_.uncompressed_size != 0);
    if (frame_.entropy_block_count > frame_views_.size()
        || frame_.dictionary_serialized_size > dictionary_staging_.size()
        || frame_.uncompressed_size > frame_decoded_storage_.size()
        || required_phrases > phrase_workspace_.size()
        || required_expansion > expansion_workspace_.size()) {
    ...
    if (!core::checked_multiply(
            static_cast<std::uint64_t>(frame_.entropy_block_count),
            static_cast<std::uint64_t>(
        ...
        || !core::checked_add(
            ...
            buffered_bytes)
        || !core::checked_add(buffered_bytes, view_bytes, buffered_bytes)
        || !core::checked_add(buffered_bytes, phrase_bytes, buffered_bytes)
        || !core::checked_add(
            buffered_bytes, expansion_bytes, buffered_bytes)
...
bool LzdTansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzd_tans_frame_to_staging(
        stream_, parameters_, limits_, frame_sequence_, output_committed_,
        frame_encoded_storage_.first(frame_serialized_size_),
        frame_views_.first(frame_.entropy_block_count),
        dictionary_staging_.first(frame_.dictionary_serialized_size),
        phrase_workspace_, expansion_workspace_,
        frame_decoded_storage_.first(frame_.uncompressed_size));
    if (decoded.error != LzdTansFrameValidationError::none) return false;
...
core::ProcessResult LzdTansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lzd_tans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzd_tans_stream_prefix_size) {
