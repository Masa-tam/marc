...
LzdDynamicRangeProfileError make_lzd_dynamic_range_profile(
    ...
    if (core::validate_limits(limits) != core::LimitError::none
        || config.frame_size == 0) {
        return LzdDynamicRangeProfileError::invalid_configuration;
    }
    ...
    if (config.original_size > limits.max_total_output_size
        ...
        return LzdDynamicRangeProfileError::limit_exceeded;
    }
    ...
    if (validate_stream_header(stream, limits) != StreamHeaderError::none) {
        return LzdDynamicRangeProfileError::unsupported;
    }

    const auto largest_frame = std::min<std::uint64_t>(
        config.original_size, config.frame_size);
    if (largest_frame == 0) {
        return LzdDynamicRangeProfileError::none;
    }
    ...
    if (!core::checked_add(largest_frame, UINT64_C(1), rounded_frame)
        || !core::checked_multiply(
            ...
            dictionary_bytes)) {
        return LzdDynamicRangeProfileError::arithmetic_overflow;
    }
...
LzdDynamicRangeWorkspaceError partition_lzd_dynamic_range_encoder_views(
    ...
    if (!core::checked_multiply(
        ...
        return LzdDynamicRangeWorkspaceError::arithmetic_overflow;
    }
    if (expected_bytes == 0) {
    ...
    if (expected_bytes != requirements.views_bytes
        ...
        return LzdDynamicRangeWorkspaceError::invalid_requirements;
    }
    if (storage.size() < expected_bytes) {
        return LzdDynamicRangeWorkspaceError::too_small;
    }
    if (!aligned(storage.data(), requirements.views_alignment)) {
        return LzdDynamicRangeWorkspaceError::misaligned;
    }
    views.entries = {
        reinterpret_cast<dictionary::internal::LzdEncoderEntry*>(
            storage.data()),
        requirements.encoder_entry_count};
    return LzdDynamicRangeWorkspaceError::none;
}

LzdDynamicRangeWorkspaceError partition_lzd_dynamic_range_decoder_views(
...
core::ErrorCode lzd_dynamic_range_profile_error_code(
    const LzdDynamicRangeProfileError error) noexcept {
    switch (error) {
    case LzdDynamicRangeProfileError::none:
        return core::ErrorCode::none;
    case LzdDynamicRangeProfileError::invalid_configuration:
        return core::ErrorCode::invalid_argument;
    case LzdDynamicRangeProfileError::unsupported:
        return core::ErrorCode::unsupported;
    case LzdDynamicRangeProfileError::limit_exceeded:
    case LzdDynamicRangeProfileError::arithmetic_overflow:
