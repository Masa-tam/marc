...
LzssFrameStreamingDecoder::LzssFrameStreamingDecoder(
...
core::ProcessResult LzssFrameStreamingDecoder::fail(
...
bool LzssFrameStreamingDecoder::parse_collected_prefix() noexcept {
    ...
    if (stream_.dictionary_algorithm != DictionaryAlgorithm::lzss
        ...
        || stream_.entropy_algorithm != EntropyAlgorithm::none
        || stream_.entropy_variant != 0
    ...
    const std::span<const std::byte,
                    dictionary::internal::lzss_parameter_size>
        parameters{prefix_bytes_.data() + stream_header_size,
                   dictionary::internal::lzss_parameter_size};
    return dictionary::internal::parse_lzss_parameters(
               parameters, limits_, parameters_)
        == dictionary::internal::LzssFormatError::none;
}

bool LzssFrameStreamingDecoder::parse_collected_frame_header() noexcept {
    ...
    if (frame_.uncompressed_size > frame_decoded_storage_.size()) {
    ...
    if (!core::checked_add(
            frame_header_size,
            static_cast<std::size_t>(frame_.compressed_payload_size), size)
        || size > frame_encoded_storage_.size()) {
        preparation_error_ = core::ErrorCode::out_of_memory;
        return false;
    }
    std::uint64_t buffered_bytes{};
    if (!core::checked_add(static_cast<std::uint64_t>(size),
...
bool LzssFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzss_frame(
    ...
    if (decoded.error != LzssFrameCodecError::none) return false;
...
core::ProcessResult LzssFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzss_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzss_stream_prefix_size) {
