...
LzssContextualTansFrameStreamingDecoder(
    ...
    const std::span<entropy::internal::TansDecodeEntry> table_workspace,
    const std::span<dictionary::internal::LzssTypedToken> token_workspace,
    const std::span<std::byte> raw_frame_workspace,
    const LzssContextualTansStreamAdmission admission) noexcept
    : limits_(limits),
    ...
    const bool valid_extents = core::checked_multiply(
        ...
        sizeof(entropy::internal::TansDecodeEntry), table_bytes)
...
core::ProcessResult LzssContextualTansFrameStreamingDecoder::fail(
...
bool LzssContextualTansFrameStreamingDecoder::
parse_collected_stream_header() noexcept {
    std::size_t consumed{};
    const auto error = parse_lzss_contextual_tans_stream_header(
        stream_header_bytes_, limits_, stream_, consumed);
    preparation_error_ =
        error == LzssContextualTansStreamHeaderError::limit_exceeded
        ? core::ErrorCode::limit_exceeded
        : core::ErrorCode::malformed_stream;
    if (error != LzssContextualTansStreamHeaderError::none
        || consumed != lzss_contextual_tans_stream_header_size) {
        return false;
    }
    switch (admission_) {
    case LzssContextualTansStreamAdmission::any:
        return true;
    case LzssContextualTansStreamAdmission::field_context_64k:
    ...
    case LzssContextualTansStreamAdmission::field_context_16m:
        return stream_.dictionary_variant == 5
            && stream_.context_variant == 4;
    case LzssContextualTansStreamAdmission::field_context_64m:
...
bool LzssContextualTansFrameStreamingDecoder::prepare_collected_frame()
    noexcept {
    ...
    if (header_error != LzssContextualTansFrameHeaderError::none
        || consumed != lzss_contextual_tans_frame_header_size) {
    ...
    if (!core::checked_add(
            ...
            static_cast<std::size_t>(frame_.descriptor_size), serialized_size)
        || !core::checked_add(
    ...
    std::memmove(
        ...
        lzss_contextual_tans_frame_header_size);
    frame_collected_ = lzss_contextual_tans_frame_header_size;
    return true;
}

bool LzssContextualTansFrameStreamingDecoder::decode_collected_frame()
    noexcept {
    const LzssContextualTansFrameValidationContext context{
        stream_, limits_, frame_sequence_, output_committed_};
    ...
    if (decoded.error != LzssContextualTansFrameDecodeError::none) {
        preparation_error_ = decoded.error
                == LzssContextualTansFrameDecodeError::reconstruction_error
...
core::ProcessResult LzssContextualTansFrameStreamingDecoder::process(
    ...
    if (std::ranges::find_if(output_overlaps, [](const auto value) {
            return value != OverlapCheck::disjoint;
        }) != output_overlaps.end()) {
        return fail(core::ErrorCode::invalid_argument, 0, 0);
    }
    ...
    while (true) {
        ...
        if (state_ == State::collecting_stream_header) {
            const auto needed =
                lzss_contextual_tans_stream_header_size - header_collected_;
            ...
            if (header_collected_ != lzss_contextual_tans_stream_header_size) {
        ...
        if (state_ == State::collecting_frame_header) {
            const auto needed =
                lzss_contextual_tans_frame_header_size - header_collected_;
            const auto count = std::min(needed, input.size() - consumed);
            if (count != 0) {
                std::memmove(
            ...
            if (header_collected_ != lzss_contextual_tans_frame_header_size) {
                if ((end_seen_ || end_requested) && consumed == input.size()) {
                    return fail(
                        core::ErrorCode::malformed_stream, consumed, produced);
                }
