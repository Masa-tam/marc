...
LzwBlockedHuffmanFrameStreamingDecoder::LzwBlockedHuffmanFrameStreamingDecoder(
    ...
    : limits_(limits), frame_encoded_storage_(frame_encoded_storage),
      ...
      frame_decoded_storage_(frame_decoded_storage), frame_views_(frame_views),
...
core::ProcessResult LzwBlockedHuffmanFrameStreamingDecoder::fail(
...
bool LzwBlockedHuffmanFrameStreamingDecoder::parse_collected_prefix() noexcept {
  const std::span<const std::byte, stream_header_size> header{
      prefix_bytes_.data(), stream_header_size};
  if (parse_stream_header(header, limits_, stream_) != StreamHeaderError::none)
    return false;
  ...
  const std::span<const std::byte, dictionary::internal::lzw_parameter_size>
      parameter_bytes{prefix_bytes_.data() + stream_header_size,
                      dictionary::internal::lzw_parameter_size};
  return dictionary::internal::parse_lzw_parameters(parameter_bytes, limits_,
                                                    parameters_) ==
         dictionary::internal::LzwFormatError::none;
}

bool LzwBlockedHuffmanFrameStreamingDecoder::
    parse_collected_frame_header() noexcept {
  preparation_error_ = core::ErrorCode::malformed_stream;
  const FrameValidationContext context{stream_, limits_, frame_sequence_,
                                       output_committed_};
  ...
  if (!core::checked_multiply(
          ...
          view_bytes) ||
      !core::checked_multiply(static_cast<std::uint64_t>(required_phrases),
                              static_cast<std::uint64_t>(
...
bool LzwBlockedHuffmanFrameStreamingDecoder::decode_collected_frame() noexcept {
  const auto decoded = decode_lzw_blocked_huffman_frame(
  ...
  if (decoded.error != LzwBlockedHuffmanFrameValidationError::none)
    return false;
...
core::ProcessResult LzwBlockedHuffmanFrameStreamingDecoder::process(
  ...
  while (true) {
    if (state_ == State::draining_frame) {
      ...
      if (count != 0) {
        std::memmove(output.data() + produced,
                     frame_decoded_storage_.data() + output_offset_, count);
    ...
    if (state_ == State::collecting_prefix) {
      const auto needed =
          lzw_blocked_huffman_stream_prefix_size - header_collected_;
      ...
      if (header_collected_ != lzw_blocked_huffman_stream_prefix_size) {
    ...
    const bool final_frame =
        output_committed_ + frame_.uncompressed_size == stream_.original_size;
