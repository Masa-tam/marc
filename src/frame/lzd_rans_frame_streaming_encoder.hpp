#ifndef MARC_FRAME_LZD_RANS_FRAME_STREAMING_ENCODER_HPP
#define MARC_FRAME_LZD_RANS_FRAME_STREAMING_ENCODER_HPP
...
inline constexpr std::size_t lzd_rans_stream_prefix_size =
    stream_header_size + dictionary::internal::lzd_parameter_size;

class LzdRansFrameStreamingEncoder final : public core::Transform {
public:
    LzdRansFrameStreamingEncoder(
        StreamHeader stream,
...
private:
    ...
    std::array<std::byte, lzd_rans_stream_prefix_size> prefix_{};
    std::size_t frame_input_size_{};
    std::size_t pending_size_{lzd_rans_stream_prefix_size};
