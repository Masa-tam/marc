...
[[nodiscard]] bool region_sizes(
    const std::span<dictionary::internal::LzssTypedToken> tokens,
    const std::span<entropy::internal::AdaptiveHuffmanNode> nodes,
    const std::span<std::uint16_t> symbols, RegionSizes& sizes) noexcept {
    return extent(tokens.size(), sizeof(dictionary::internal::LzssTypedToken),
                  sizes.tokens)
        && extent(nodes.size(), sizeof(entropy::internal::AdaptiveHuffmanNode),
                  sizes.nodes)
        && extent(symbols.size(), sizeof(std::uint16_t), sizes.symbols);
}
...
[[nodiscard]] bool is_limit_failure(
    ...
    return result.error
        ...
        || (result.error
                == LzssContextualAdaptiveHuffmanFrameEncodeError::header_error
            && result.header_error
        ...
        || (result.error
            ...
            && result.descriptor_error
                == entropy::internal::ContextualAdaptiveHuffmanFormatError::
                    limit_exceeded);
}
...
core::ProcessResult
LzssContextualAdaptiveHuffmanFrameStreamingEncoder::fail(
...
bool LzssContextualAdaptiveHuffmanFrameStreamingEncoder::output_is_disjoint(
    ...
    const std::array overlaps{
        regions_overlap(output.data(), output.size(),
                        raw_frame_workspace_.data(),
                        raw_frame_workspace_.size()),
        regions_overlap(output.data(), output.size(), token_workspace_.data(),
                        sizes.tokens),
        regions_overlap(output.data(), output.size(), node_workspace_.data(),
                        sizes.nodes),
...
core::ProcessResult
LzssContextualAdaptiveHuffmanFrameStreamingEncoder::process(
