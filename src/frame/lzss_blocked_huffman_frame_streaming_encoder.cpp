...
LzssBlockedHuffmanFrameStreamingEncoder::
LzssBlockedHuffmanFrameStreamingEncoder(
    ...
    const bool workspace_valid = core::checked_multiply(
        largest_frame,
        static_cast<std::uint64_t>(dictionary::internal::lzss_literal_size),
        dictionary_bytes);
...
core::ProcessResult LzssBlockedHuffmanFrameStreamingEncoder::fail(
...
bool LzssBlockedHuffmanFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    const auto plan = use_hash_chain_
        ? plan_lzss_blocked_huffman_frame_hash_chain(
            ...
            dictionary_staging_, match_finder_storage_)
        : plan_lzss_blocked_huffman_frame(
            stream_, parameters_, limits_, frame_sequence_, input_committed_,
            frame_input_storage_.first(frame_input_size_),
            dictionary_staging_);
    if (plan.error != LzssBlockedHuffmanFrameValidationError::none) {
        if (plan.error
            ...
            preparation_error_ = core::ErrorCode::out_of_memory;
                   ...
                   && plan.dictionary_encode_error
                       == dictionary::internal::LzssEncodeError::
                           match_finder_error
    ...
    const auto encoded = use_hash_chain_
        ...
        : encode_lzss_blocked_huffman_frame(
            ...
            dictionary_staging_,
            frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzssBlockedHuffmanFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzssBlockedHuffmanFrameStreamingEncoder::process(
