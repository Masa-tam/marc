...
Lz77RansFrameStreamingDecoder::Lz77RansFrameStreamingDecoder(
...
core::ProcessResult Lz77RansFrameStreamingDecoder::fail(
...
bool Lz77RansFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool Lz77RansFrameStreamingDecoder::parse_collected_frame_header() noexcept {
...
bool Lz77RansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lz77_rans_frame_to_staging(
    ...
    if (decoded.error != Lz77RansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult Lz77RansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lz77_rans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lz77_rans_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream,
                                consumed, produced);
