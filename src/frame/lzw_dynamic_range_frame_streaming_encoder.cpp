...
LzwDynamicRangeFrameStreamingEncoder(
    ...
    std::uint64_t rounded_dictionary_bits{};
    const bool workspace_valid = core::checked_multiply(
        static_cast<std::uint64_t>(largest_frame),
        static_cast<std::uint64_t>(parameters_.maximum_code_width),
        dictionary_bits)
        && core::checked_add(
            dictionary_bits, UINT64_C(7), rounded_dictionary_bits);
    const auto dictionary_bytes = rounded_dictionary_bits / 8;
    ...
    if (validate_stream_header(stream_, limits_) != StreamHeaderError::none
        ...
        || dictionary::internal::validate_lzw_parameters(parameters_, limits_)
...
core::ProcessResult LzwDynamicRangeFrameStreamingEncoder::fail(
...
bool LzwDynamicRangeFrameStreamingEncoder::prepare_frame() noexcept {
    preparation_error_ = core::ErrorCode::internal_error;
    const auto plan = plan_lzw_dynamic_range_frame(
        ...
        dictionary_staging_);
    if (plan.error != LzwDynamicRangeFrameValidationError::none) {
        switch (plan.error) {
        case LzwDynamicRangeFrameValidationError::input_size_mismatch:
        case LzwDynamicRangeFrameValidationError::unsupported_pipeline:
            preparation_error_ = core::ErrorCode::invalid_argument;
            break;
        case LzwDynamicRangeFrameValidationError::
        ...
        case LzwDynamicRangeFrameValidationError::
            ...
            break;
        case LzwDynamicRangeFrameValidationError::workspace_limit:
        case LzwDynamicRangeFrameValidationError::
            invalid_dictionary_extent:
        case LzwDynamicRangeFrameValidationError::invalid_entropy_extent:
        case LzwDynamicRangeFrameValidationError::dictionary_encode_error:
        case LzwDynamicRangeFrameValidationError::entropy_encode_error:
        case LzwDynamicRangeFrameValidationError::header_error:
        case LzwDynamicRangeFrameValidationError::arithmetic_overflow:
    ...
    const auto encoded = encode_lzw_dynamic_range_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_), encoder_workspace_,
        dictionary_staging_,
        frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzwDynamicRangeFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzwDynamicRangeFrameStreamingEncoder::process(
