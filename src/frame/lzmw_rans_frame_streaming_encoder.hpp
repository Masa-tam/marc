#ifndef MARC_FRAME_LZMW_RANS_FRAME_STREAMING_ENCODER_HPP
#define MARC_FRAME_LZMW_RANS_FRAME_STREAMING_ENCODER_HPP
...
inline constexpr std::size_t lzmw_rans_stream_prefix_size =
    stream_header_size + dictionary::internal::lzmw_parameter_size;

class LzmwRansFrameStreamingEncoder final : public core::Transform {
public:
    LzmwRansFrameStreamingEncoder(
        StreamHeader stream,
...
private:
    ...
    std::array<std::byte, lzmw_rans_stream_prefix_size> prefix_{};
    std::size_t frame_input_size_{};
    std::size_t pending_size_{lzmw_rans_stream_prefix_size};
