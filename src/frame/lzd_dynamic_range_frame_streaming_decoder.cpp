...
core::ProcessResult LzdDynamicRangeFrameStreamingDecoder::fail(
...
bool LzdDynamicRangeFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool LzdDynamicRangeFrameStreamingDecoder::
parse_collected_frame_header() noexcept {
    ...
    if (!core::checked_add(
        ...
        || !core::checked_multiply(
            ...
            UINT64_C(2), maximum_payload_size)
        || !core::checked_add(
            maximum_payload_size, UINT64_C(5), maximum_payload_size)
...
bool LzdDynamicRangeFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzd_dynamic_range_frame_to_staging(
    ...
    if (decoded.error != LzdDynamicRangeFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzdDynamicRangeFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzd_dynamic_range_stream_prefix_size
                - header_collected_;
            ...
            if (header_collected_ != lzd_dynamic_range_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream,
                                consumed, produced);
