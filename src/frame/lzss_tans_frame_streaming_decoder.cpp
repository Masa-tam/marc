...
core::ProcessResult LzssTansFrameStreamingDecoder::fail(
...
bool LzssTansFrameStreamingDecoder::parse_collected_prefix() noexcept {
    ...
    if (stream_.dictionary_algorithm != DictionaryAlgorithm::lzss
    ...
    const std::span<const std::byte,
        ...
        parameter_bytes{prefix_bytes_.data() + stream_header_size,
                        dictionary::internal::lzss_parameter_size};
    return dictionary::internal::parse_lzss_parameters(
               parameter_bytes, limits_, parameters_)
        == dictionary::internal::LzssFormatError::none;
}

bool LzssTansFrameStreamingDecoder::parse_collected_frame_header() noexcept {
...
bool LzssTansFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzss_tans_frame_to_staging(
    ...
    if (decoded.error != LzssTansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzssTansFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed =
                lzss_tans_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzss_tans_stream_prefix_size) {
                if ((end_seen_ || end_requested)
                    && consumed == input.size()) {
                    return fail(core::ErrorCode::malformed_stream,
                                consumed, produced);
