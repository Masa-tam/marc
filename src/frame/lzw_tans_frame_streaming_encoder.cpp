...
LzwTansFrameStreamingEncoder::LzwTansFrameStreamingEncoder(
...
core::ProcessResult LzwTansFrameStreamingEncoder::fail(
...
bool LzwTansFrameStreamingEncoder::prepare_frame() noexcept {
    ...
    const auto plan = plan_lzw_tans_frame(
        ...
        dictionary_staging_);
    if (plan.error != LzwTansFrameValidationError::none) {
        switch (plan.error) {
        case LzwTansFrameValidationError::input_size_mismatch:
        case LzwTansFrameValidationError::unsupported_pipeline:
            preparation_error_ = core::ErrorCode::invalid_argument;
            break;
        case LzwTansFrameValidationError::encoder_workspace_too_small:
        case LzwTansFrameValidationError::dictionary_staging_too_small:
        case LzwTansFrameValidationError::serialized_output_too_small:
            preparation_error_ = core::ErrorCode::out_of_memory;
            break;
        case LzwTansFrameValidationError::workspace_limit:
        case LzwTansFrameValidationError::invalid_dictionary_extent:
        case LzwTansFrameValidationError::invalid_entropy_extent:
        case LzwTansFrameValidationError::dictionary_encode_error:
        case LzwTansFrameValidationError::entropy_encode_error:
        case LzwTansFrameValidationError::header_error:
        case LzwTansFrameValidationError::arithmetic_overflow:
    ...
    const auto encoded = encode_lzw_tans_frame(
        stream_, parameters_, limits_, frame_sequence_, input_committed_,
        frame_input_storage_.first(frame_input_size_), encoder_workspace_,
        dictionary_staging_,
        frame_encoded_storage_.first(plan.serialized_size));
    if (encoded.error != LzwTansFrameValidationError::none) {
        return false;
    }
...
core::ProcessResult LzwTansFrameStreamingEncoder::process(
