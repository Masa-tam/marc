...
inline constexpr std::uint64_t payload_bytes_per_reference_byte = 2;
...
LzmwDynamicRangeProfileError make_lzmw_dynamic_range_profile(
    ...
    if (core::validate_limits(limits) != core::LimitError::none
        || config.frame_size == 0) {
        return LzmwDynamicRangeProfileError::invalid_configuration;
    }
    ...
    if (config.original_size > limits.max_total_output_size
        ...
        return LzmwDynamicRangeProfileError::limit_exceeded;
    }
    ...
    if (validate_stream_header(stream, limits) != StreamHeaderError::none) {
        return LzmwDynamicRangeProfileError::unsupported;
    }

    const auto largest_frame = std::min<std::uint64_t>(
        config.original_size, config.frame_size);
    if (largest_frame == 0) {
        return LzmwDynamicRangeProfileError::none;
    }
    ...
    if (!core::checked_multiply(
            ...
            dictionary_bytes)) {
        return LzmwDynamicRangeProfileError::arithmetic_overflow;
    }
    ...
    if (!core::checked_multiply(
        ...
        || !core::checked_multiply(
            dictionary_bytes, payload_bytes_per_reference_byte,
            payload_bytes)
        || !core::checked_add(
...
LzmwDynamicRangeWorkspaceError
partition_lzmw_dynamic_range_encoder_views(
    ...
    if (!core::checked_multiply(
            ...
            sizeof(dictionary::internal::LzmwEncoderEntry),
            expected_bytes)) {
        return LzmwDynamicRangeWorkspaceError::arithmetic_overflow;
    }
    if (expected_bytes == 0) {
    ...
    if (expected_bytes != requirements.views_bytes
        ...
        return LzmwDynamicRangeWorkspaceError::invalid_requirements;
    }
    if (storage.size() < expected_bytes) {
        return LzmwDynamicRangeWorkspaceError::too_small;
    }
    if (!aligned(storage.data(), requirements.views_alignment)) {
        return LzmwDynamicRangeWorkspaceError::misaligned;
    }
    views.entries = {
        reinterpret_cast<dictionary::internal::LzmwEncoderEntry*>(
            storage.data()),
        requirements.encoder_entry_count};
    return LzmwDynamicRangeWorkspaceError::none;
}
...
core::ErrorCode lzmw_dynamic_range_profile_error_code(
    const LzmwDynamicRangeProfileError error) noexcept {
    switch (error) {
    case LzmwDynamicRangeProfileError::none:
        return core::ErrorCode::none;
    case LzmwDynamicRangeProfileError::invalid_configuration:
        return core::ErrorCode::invalid_argument;
    case LzmwDynamicRangeProfileError::unsupported:
        return core::ErrorCode::unsupported;
    case LzmwDynamicRangeProfileError::limit_exceeded:
    case LzmwDynamicRangeProfileError::arithmetic_overflow:
