...
LzmwFrameStreamingDecoder::LzmwFrameStreamingDecoder(
...
core::ProcessResult LzmwFrameStreamingDecoder::fail(
...
bool LzmwFrameStreamingDecoder::parse_collected_prefix() noexcept {
...
bool LzmwFrameStreamingDecoder::parse_collected_frame_header() noexcept {
    ...
    if (phrase_entries > phrase_workspace_.size()) {
        preparation_error_ = core::ErrorCode::out_of_memory;
        return false;
    }
    const auto expansion_entries =
        dictionary::internal::lzmw_expansion_workspace_entries(
            phrase_entries, frame_.uncompressed_size != 0);
    if (expansion_entries > expansion_workspace_.size()) {
        preparation_error_ = core::ErrorCode::out_of_memory;
        return false;
    }
    ...
    if (!core::checked_multiply(
        ...
        || !core::checked_multiply(
            ...
            expansion_bytes)
        || !core::checked_add(static_cast<std::uint64_t>(serialized_size),
                              static_cast<std::uint64_t>(
                                  frame_.uncompressed_size),
                              buffered_bytes)
...
bool LzmwFrameStreamingDecoder::decode_collected_frame() noexcept {
    const auto decoded = decode_lzmw_frame(
    ...
    if (decoded.error != LzmwFrameCodecError::none) return false;
...
core::ProcessResult LzmwFrameStreamingDecoder::process(
    ...
    while (true) {
        ...
        if (state_ == State::collecting_prefix) {
            const auto needed = lzmw_stream_prefix_size - header_collected_;
            ...
            if (header_collected_ != lzmw_stream_prefix_size) {
