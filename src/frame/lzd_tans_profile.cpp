...
LzdTansProfileError make_lzd_tans_profile(
    ...
    if (core::validate_limits(limits) != core::LimitError::none
        ...
        return LzdTansProfileError::invalid_configuration;
    }
    ...
    if (config.original_size > limits.max_total_output_size
        ...
        return LzdTansProfileError::limit_exceeded;
    }
    ...
    return LzdTansProfileError::none;
}

LzdTansProfileError calculate_lzd_tans_decoder_workspace(
    const core::DecoderLimits& limits,
...
LzdTansWorkspaceError partition_lzd_tans_encoder_views(
    ...
    if (!core::checked_multiply(
        ...
        return LzdTansWorkspaceError::arithmetic_overflow;
    }
    if (expected_bytes == 0) {
    ...
    if (expected_bytes != requirements.views_bytes
        ...
        return LzdTansWorkspaceError::invalid_requirements;
    }
    if (storage.size() < expected_bytes) {
        return LzdTansWorkspaceError::too_small;
    }
    if (!aligned(storage.data(), requirements.views_alignment)) {
        return LzdTansWorkspaceError::misaligned;
    }
    views.entries = {
        reinterpret_cast<dictionary::internal::LzdEncoderEntry*>(
            storage.data()),
        requirements.encoder_entry_count};
    return LzdTansWorkspaceError::none;
}

LzdTansWorkspaceError partition_lzd_tans_decoder_views(
...
core::ErrorCode lzd_tans_profile_error_code(
    const LzdTansProfileError error) noexcept {
    switch (error) {
    case LzdTansProfileError::none:
        return core::ErrorCode::none;
    case LzdTansProfileError::invalid_configuration:
        return core::ErrorCode::invalid_argument;
    case LzdTansProfileError::unsupported:
        return core::ErrorCode::unsupported;
    case LzdTansProfileError::limit_exceeded:
    case LzdTansProfileError::arithmetic_overflow:
