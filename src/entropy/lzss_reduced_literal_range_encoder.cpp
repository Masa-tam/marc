...
struct Models {
    ...
    std::array<std::uint32_t,
               context::internal::lzss_reduced_literal_context_count> totals{};

    Models() noexcept {
        frequencies.fill(1);
        for (std::size_t index = 0; index < totals.size(); ++index) {
            totals[index] = context::internal::lzss_reduced_literal_alphabets[index];
        }
    }

    void update(const std::uint16_t context_id,
                const std::uint32_t symbol) noexcept {
        const auto offset = context::internal::lzss_reduced_literal_offsets[
            context_id];
        ...
        total = 0;
        const auto alphabet = context::internal::lzss_reduced_literal_alphabets[
            context_id];
    ...
    for (const auto& operation : operations) {
        ...
        if (operation.kind == context::internal::ModeledOperationKind::symbol) {
            ...
            if (operation.alphabet_size
                != context::internal::lzss_reduced_literal_alphabets[
...
std::size_t lzss_reduced_literal_range_encoder_state_bytes() noexcept {
    return sizeof(Models) + sizeof(RangeWriter);
}

ContextualDynamicRangeEncodeResult plan_lzss_reduced_literal_range_operations(
    const std::span<const context::internal::ModeledOperation> operations,
    ...
    if (!core::checked_add(operation_bytes,
            lzss_reduced_literal_range_encoder_state_bytes(), working_bytes)) {
...
ContextualDynamicRangeEncodeResult encode_lzss_reduced_literal_range_operations(
    const std::span<const context::internal::ModeledOperation> operations,
    const core::DecoderLimits& limits,
    const std::span<std::byte> payload_output,
    ContextualDynamicRangeDescriptor& descriptor) noexcept {
    ContextualDynamicRangeDescriptor planned{};
    const auto plan = plan_lzss_reduced_literal_range_operations(
