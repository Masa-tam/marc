...
ContextualAdaptiveHuffmanTreeError
ContextualAdaptiveHuffmanTree::initialize(
    ...
    if (alphabet_size < 2
        || alphabet_size > contextual_adaptive_huffman_max_alphabet) {
        return ContextualAdaptiveHuffmanTreeError::invalid_alphabet;
    }
    const auto required_nodes = static_cast<std::size_t>(2U * alphabet_size + 1U);
    if (node_storage.size() < required_nodes
        || symbol_storage.size() < alphabet_size) {
        return ContextualAdaptiveHuffmanTreeError::insufficient_workspace;
    }
    nodes_ = node_storage.first(required_nodes);
    symbols_ = symbol_storage.first(alphabet_size);
    alphabet_size_ = alphabet_size;
    reset();
    return ContextualAdaptiveHuffmanTreeError::none;
}

void ContextualAdaptiveHuffmanTree::reset() noexcept {
    if (alphabet_size_ == 0) return;
    std::fill(nodes_.begin(), nodes_.end(), AdaptiveHuffmanNode{});
    std::fill(symbols_.begin(), symbols_.end(), adaptive_huffman_invalid_node);
    count_ = 1;
    nyt_ = 0;
    nodes_[0] = {0, static_cast<std::uint16_t>(2U * alphabet_size_),
                 adaptive_huffman_invalid_node, adaptive_huffman_invalid_node,
                 adaptive_huffman_invalid_node, 0,
                 AdaptiveHuffmanNodeKind::nyt};
}

bool ContextualAdaptiveHuffmanTree::initialized() const noexcept {
    return alphabet_size_ != 0;
}

std::uint16_t ContextualAdaptiveHuffmanTree::alphabet_size() const noexcept {
    return alphabet_size_;
}

bool ContextualAdaptiveHuffmanTree::contains(
    const std::uint16_t symbol) const noexcept {
    return symbol < alphabet_size_
        && symbols_[symbol] != adaptive_huffman_invalid_node;
}

std::size_t ContextualAdaptiveHuffmanTree::node_count() const noexcept {
    return count_;
}

std::uint16_t ContextualAdaptiveHuffmanTree::root() const noexcept { return 0; }
std::uint16_t ContextualAdaptiveHuffmanTree::nyt() const noexcept { return nyt_; }

const AdaptiveHuffmanNode& ContextualAdaptiveHuffmanTree::node(
    const std::uint16_t index) const noexcept {
    return nodes_[index];
}

ContextualAdaptiveHuffmanTreeError ContextualAdaptiveHuffmanTree::make_path(
    std::uint16_t index, const std::span<std::uint8_t> path,
    std::size_t& path_size) const noexcept {
    path_size = 0;
    if (!initialized()) return ContextualAdaptiveHuffmanTreeError::invalid_tree;
    std::array<std::uint8_t, contextual_adaptive_huffman_max_alphabet> reversed{};
    while (index != 0) {
        if (index >= count_ || path_size == reversed.size()) {
            path_size = 0;
            return ContextualAdaptiveHuffmanTreeError::invalid_tree;
        }
    ...
    if (path.size() < path_size) {
        path_size = 0;
        return ContextualAdaptiveHuffmanTreeError::path_capacity;
    }
...
ContextualAdaptiveHuffmanTreeError
ContextualAdaptiveHuffmanTree::path_for_symbol(
    const std::uint16_t symbol, const std::span<std::uint8_t> path,
    std::size_t& path_size) const noexcept {
    if (!contains(symbol)) {
        path_size = 0;
        return ContextualAdaptiveHuffmanTreeError::symbol_not_present;
    }
    return make_path(symbols_[symbol], path, path_size);
}

ContextualAdaptiveHuffmanTreeError ContextualAdaptiveHuffmanTree::path_for_nyt(
    const std::span<std::uint8_t> path, std::size_t& path_size) const noexcept {
    return make_path(nyt_, path, path_size);
}

bool ContextualAdaptiveHuffmanTree::is_ancestor(
...
std::uint16_t ContextualAdaptiveHuffmanTree::leader(
    const std::uint16_t index) const noexcept {
...
bool ContextualAdaptiveHuffmanTree::swap_nodes(
    ...
    if (first_parent == second_parent) {
        ...
        if (!((parent.left == first && parent.right == second)
              || (parent.left == second && parent.right == first))) return false;
...
ContextualAdaptiveHuffmanTreeError ContextualAdaptiveHuffmanTree::update_from(
    std::uint16_t index) noexcept {
    while (index != adaptive_huffman_invalid_node) {
        if (index >= count_) return ContextualAdaptiveHuffmanTreeError::invalid_tree;
        const auto block_leader = leader(index);
        if (block_leader != index && !swap_nodes(index, block_leader)) {
            return ContextualAdaptiveHuffmanTreeError::invalid_tree;
        }
        if (nodes_[index].weight == std::numeric_limits<std::uint32_t>::max()) {
            return ContextualAdaptiveHuffmanTreeError::weight_overflow;
        }
        ++nodes_[index].weight;
        index = nodes_[index].parent;
    }
...
ContextualAdaptiveHuffmanTreeError
ContextualAdaptiveHuffmanTree::observe_existing(
    const std::uint16_t symbol) noexcept {
    if (!contains(symbol)) {
        return ContextualAdaptiveHuffmanTreeError::symbol_not_present;
    }
    return update_from(symbols_[symbol]);
}

ContextualAdaptiveHuffmanTreeError ContextualAdaptiveHuffmanTree::observe_new(
    const std::uint16_t symbol) noexcept {
    if (symbol >= alphabet_size_) {
        return ContextualAdaptiveHuffmanTreeError::symbol_not_present;
    }
    if (contains(symbol)) {
        return ContextualAdaptiveHuffmanTreeError::symbol_already_present;
    }
    if (count_ + 2 > nodes_.size()) {
        return ContextualAdaptiveHuffmanTreeError::tree_full;
    }
    const auto old_nyt = nyt_;
    const auto former_parent = nodes_[old_nyt].parent;
    const auto old_order = nodes_[old_nyt].order;
    if (old_order < 2) return ContextualAdaptiveHuffmanTreeError::invalid_tree;
    ...
    nyt_ = new_nyt;
    const auto error = update_from(former_parent);
    if (error != ContextualAdaptiveHuffmanTreeError::none) return error;
    return ContextualAdaptiveHuffmanTreeError::none;
}

bool ContextualAdaptiveHuffmanTree::validate() const noexcept {
    if (!initialized() || count_ == 0 || count_ > nodes_.size()
        || (count_ & 1U) == 0 || nyt_ >= count_
        || nodes_[0].parent != adaptive_huffman_invalid_node
        || nodes_[0].order != 2U * alphabet_size_
        || nodes_[nyt_].kind != AdaptiveHuffmanNodeKind::nyt
        || nodes_[nyt_].weight != 0) return false;
    std::array<bool, 2 * contextual_adaptive_huffman_max_alphabet + 1> orders{};
    std::array<bool, contextual_adaptive_huffman_max_alphabet> seen{};
    for (std::uint16_t index = 0; index < count_; ++index) {
        const auto& current = nodes_[index];
        if (current.order > 2U * alphabet_size_ || orders[current.order]) {
            return false;
        }
        ...
        if (current.kind == AdaptiveHuffmanNodeKind::internal) {
            ...
            if (current.kind == AdaptiveHuffmanNodeKind::symbol) {
                if (current.symbol >= alphabet_size_ || seen[current.symbol]
                    || symbols_[current.symbol] != index) return false;
                seen[current.symbol] = true;
            } else if (current.kind != AdaptiveHuffmanNodeKind::nyt
                       || index != nyt_) {
                return false;
            }
        }
