...
ContextualDynamicRangeDecodeResult LzssPositionDistanceRangeDecoder::result()
    const noexcept {
    return {state_.event_count, state_.decision_count, state_.payload_offset, state_.error};
}

ContextualDynamicRangeDecodeResult LzssPositionDistanceRangeDecoder::fail(
    const ContextualDynamicRangeDecodeError error) noexcept {
    state_.error = error;
    return result();
}

ContextualDynamicRangeDecodeResult LzssPositionDistanceRangeDecoder::begin(
    ...
    for (std::size_t index = 0; index < state_.totals.size(); ++index) {
        state_.totals[index] = context::internal::lzss_position_distance_alphabets[index];
    }
...
bool LzssPositionDistanceRangeDecoder::decode_interval(
...
ContextualDynamicRangeDecodeResult LzssPositionDistanceRangeDecoder::decode_symbol(
    ...
    if (context_id >= context::internal::lzss_position_distance_context_count) {
        return fail(ContextualDynamicRangeDecodeError::invalid_context);
    }
    if (alphabet != context::internal::lzss_position_distance_alphabets[context_id]) {
    ...
    const auto offset = context::internal::lzss_position_distance_offsets[context_id];
...
ContextualDynamicRangeDecodeResult LzssPositionDistanceRangeDecoder::decode_bypass(
...
ContextualDynamicRangeDecodeResult LzssPositionDistanceRangeDecoder::decode_next_impl(
    ...
    if (state_.error != ContextualDynamicRangeDecodeError::none) return result();
    if (state_.finished) return fail(ContextualDynamicRangeDecodeError::already_finished);
    const auto request = state_.cursor.next();
    auto decoded = request.shape;
    ContextualDynamicRangeDecodeResult checked{};
    ...
    if (checked.error != ContextualDynamicRangeDecodeError::none) return checked;
    if (state_.cursor.accept(decoded) != context::internal::LzssFieldContextError::none)
        return fail(ContextualDynamicRangeDecodeError::invalid_interval);
    operation = decoded;
    return result();
}

void LzssPositionDistanceRangeDecoder::canonical_emit(const std::uint8_t value) noexcept {
...
bool LzssPositionDistanceRangeDecoder::canonical_shift_low() noexcept {
...
bool LzssPositionDistanceRangeDecoder::validate_models() const noexcept {
    for (std::size_t context_id = 0; context_id < state_.totals.size(); ++context_id) {
        const auto alphabet =
            context::internal::lzss_position_distance_alphabets[context_id];
        ...
        for (std::size_t symbol = 0; symbol < alphabet; ++symbol) {
            const auto frequency = state_.frequencies[
                context::internal::lzss_position_distance_offsets[context_id] + symbol];
...
ContextualDynamicRangeDecodeResult LzssPositionDistanceRangeDecoder::finish(
    ...
    if (state_.cursor.finish() != context::internal::LzssFieldContextError::none)
