...
[[nodiscard]] bool read_symbol(
    ContextualRansDecoder& decoder, const std::uint16_t context,
    ...
    if (result.entropy.error == ContextualRansDecodeError::none) return true;
    result.error = LzssContextualRansDecodeError::entropy_error;
    return false;
}

[[nodiscard]] bool read_bypass(
    ContextualRansDecoder& decoder, const std::uint8_t bits,
    std::uint32_t& value,
    LzssContextualRansDecodeResult& result) noexcept {
...
[[nodiscard]] bool checked_table_bytes(
    ...
    return core::checked_multiply(tables.size(), sizeof(RansDecodeEntry), bytes);
}
...
[[nodiscard]] LzssContextualRansDecodeResult decode_started_pass(
    ...
    while (result.token_count < context.declared_token_count) {
        ...
        if (result.token_error != LzssTypedTokenError::none) {
            if (result.token_error == LzssTypedTokenError::limit_exceeded) {
                ...
                result.error = LzssContextualRansDecodeError::invalid_token;
            }
            return result;
        }
    ...
    if (result.raw_size != context.declared_raw_size) {
        result.error = LzssContextualRansDecodeError::raw_size_mismatch;
    }
    return result;
}
...
[[nodiscard]] LzssContextualRansDecodeResult preflight_workspace(
    ...
    if (tables.size() < entropy::internal::contextual_rans_decode_table_entries) {
        result.error = LzssContextualRansDecodeError::table_output_too_small;
        return result;
    }
