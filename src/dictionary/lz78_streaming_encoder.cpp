...
Lz78StreamingEncoder::Lz78StreamingEncoder(
    ...
    if (raw_storage_.size() < frame_size) {
        state_ = State::error;
        terminal_error_ = {core::ErrorCode::out_of_memory, 0, 0};
        return;
    }
    const auto required_entries =
        lz78_encoder_workspace_entries(frame_size, parameters_);
    ...
    if (!core::checked_multiply(
            ...
            static_cast<std::uint64_t>(sizeof(Lz78EncoderEntry)),
            dictionary_workspace_bytes_)) {
        state_ = State::error;
        terminal_error_ = {core::ErrorCode::limit_exceeded, 0, 0};
        return;
    }
    std::uint64_t initial_buffered{};
    if (!core::checked_add(declared_frame_size_, dictionary_workspace_bytes_,
                           initial_buffered)
        || initial_buffered > limits_.max_internal_buffered_bytes) {
        state_ = State::error;
        terminal_error_ = {core::ErrorCode::limit_exceeded, 0, 0};
        return;
    }
...
core::ProcessResult Lz78StreamingEncoder::fail(
...
bool Lz78StreamingEncoder::prepare_encoded_frame() noexcept {
    preparation_error_ = core::ErrorCode::internal_error;
    const auto raw = std::span<const std::byte>{raw_storage_}.first(
        raw_collected_);
    const auto planned = plan_lz78_token_stream(
        raw, parameters_, limits_, dictionary_workspace_);
    if (planned.error != Lz78EncodeError::none) {
        switch (planned.error) {
        case Lz78EncodeError::serialized_limit_exceeded:
        case Lz78EncodeError::input_limit_exceeded:
        case Lz78EncodeError::workspace_limit_exceeded:
            preparation_error_ = core::ErrorCode::limit_exceeded;
            break;
        case Lz78EncodeError::workspace_too_small:
    ...
    if (!core::checked_add(declared_frame_size_, dictionary_workspace_bytes_,
                           buffered_bytes)
        || !core::checked_add(
            buffered_bytes, static_cast<std::uint64_t>(planned.output_size),
            buffered_bytes)
    ...
    const auto encoded = encode_lz78_token_stream(
        raw, parameters_, limits_, dictionary_workspace_,
...
core::ProcessResult Lz78StreamingEncoder::process(
