...
Lz77StreamingEncoder::Lz77StreamingEncoder(
...
core::ProcessResult Lz77StreamingEncoder::fail(
...
bool Lz77StreamingEncoder::prepare_encoded_frame() noexcept {
    preparation_error_ = core::ErrorCode::internal_error;
    const auto raw = std::span<const std::byte>{raw_storage_}.first(
        raw_collected_);
    const auto planned = plan_lz77_token_stream(raw, parameters_, limits_);
    if (planned.error != Lz77EncodeError::none) {
        preparation_error_ =
            planned.error == Lz77EncodeError::serialized_limit_exceeded
                || planned.error == Lz77EncodeError::input_limit_exceeded
    ...
    const auto encoded = encode_lz77_token_stream(
        raw, parameters_, limits_,
...
core::ProcessResult Lz77StreamingEncoder::process(
