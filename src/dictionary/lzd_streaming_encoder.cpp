...
LzdStreamingEncoderWorkspaceRequirements
lzd_streaming_encoder_workspace_requirements(
    ...
    LzdStreamingEncoderWorkspaceRequirements result{};
    ...
    result.dictionary_entries = lzd_encoder_workspace_entries(
        result.raw_bytes, parameters);
    result.supported = true;
    return result;
}

LzdStreamingEncoder::LzdStreamingEncoder(
    ...
    const auto required = lzd_streaming_encoder_workspace_requirements(
    ...
    if (!core::checked_multiply(
            ...
            static_cast<std::uint64_t>(sizeof(LzdEncoderEntry)),
            dictionary_bytes)
        || !core::checked_add(
...
core::ProcessResult LzdStreamingEncoder::fail(
...
bool LzdStreamingEncoder::prepare_encoded_frame() noexcept {
    preparation_error_ = core::ErrorCode::internal_error;
    const auto raw = std::span<const std::byte>{raw_storage_}.first(
        raw_collected_);
    const auto planned = plan_lzd_token_stream(
        raw, parameters_, limits_, dictionary_workspace_);
    if (planned.error != LzdEncodeError::none) {
        switch (planned.error) {
        case LzdEncodeError::input_limit_exceeded:
        case LzdEncodeError::serialized_limit_exceeded:
        case LzdEncodeError::workspace_limit_exceeded:
            preparation_error_ = core::ErrorCode::limit_exceeded;
            break;
        case LzdEncodeError::workspace_too_small:
        case LzdEncodeError::output_too_small:
    ...
    const auto encoded = encode_lzd_token_stream(
        raw, parameters_, limits_, dictionary_workspace_,
...
core::ProcessResult LzdStreamingEncoder::process(
