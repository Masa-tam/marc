...
LzwStreamingEncoder::LzwStreamingEncoder(
    ...
    const auto required_entries =
        lzw_encoder_workspace_entries(frame_size, parameters_);
    ...
    if (!core::checked_multiply(
            ...
            static_cast<std::uint64_t>(sizeof(LzwEncoderEntry)),
...
core::ProcessResult LzwStreamingEncoder::fail(
...
bool LzwStreamingEncoder::prepare_encoded_frame() noexcept {
    preparation_error_ = core::ErrorCode::internal_error;
    const auto raw = std::span<const std::byte>{raw_storage_}.first(
        raw_collected_);
    const auto planned = plan_lzw_code_stream(
        raw, parameters_, limits_, dictionary_workspace_);
    if (planned.error != LzwEncodeError::none) {
        switch (planned.error) {
        case LzwEncodeError::serialized_limit_exceeded:
        case LzwEncodeError::input_limit_exceeded:
        case LzwEncodeError::workspace_limit_exceeded:
            preparation_error_ = core::ErrorCode::limit_exceeded;
            break;
        case LzwEncodeError::workspace_too_small:
    ...
    const auto encoded = encode_lzw_code_stream(
        raw, parameters_, limits_, dictionary_workspace_,
...
core::ProcessResult LzwStreamingEncoder::process(
