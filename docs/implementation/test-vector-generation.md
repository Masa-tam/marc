...
For both Dynamic Range and rANS, initialize encode and decode configurations,
set distinctive original-size and total-output values, and apply profiles
0, 1, and 2 in sequence. Require exact frame/window/match/LZ limits and the
backend-specific block, payload, aggregate, and model limits from DD-955.
Reapply each selector and compare the complete structure byte-for-byte.
...
External validation: all four verifier passes report 62 archives and revision
`42783b435976777c40259ab5e90b616146535854`. The Windows/MSVC and Ubuntu 24.04
...
Generate the five-byte canonical stream independently through all five public
Contextual rANS window selectors. For selector 4 require exact identity
`2/6 + 1/5 + 4/3`; reject every proper truncation, extreme frame lengths, and
a nonzero descriptor flag atomically through both decoder paths. Require all
twenty ordered cross-profile mismatches to publish no raw byte and preserve
the repeated terminal error.

For the live harness retain 32-KiB input, four-KiB output, one-KiB
frame/token/raw staging, fixed 126,976-entry table storage, and the existing
finite call bound. Set validation ceilings to `8*1024` decisions,
`16*1024+8` payload bytes, and 67,108,864-byte distance. Require warning-clean
MSVC and ClangCL compile smoke, then run Clang 22 with the matching sanitizer
runtime, seed 20260902, 1,000 inputs, 32-KiB maximum input, five-second
timeout, and 512-MiB RSS limit. Retain no generated corpus or artifact without
a finding, and keep schema 53 unchanged.

### TVG-0909

Generate schema 54 from the repository-owned interoperability fixture with the
frozen schema-53 profile order followed only by `lzss-contextual-rans-64m` as
archive 64. Before accepting the archive require header identity
`2/6 + 1/5 + 4/3`, then require an immediate CLI round trip, recorded size,
and SHA-256.

Verify all 64 archives in exact order by foreign decode equality and
byte-identical local re-encoding. Swap the first two manifest entries and
require rejection. Remove only archive 64, rewrite the manifest as
`schema_version=53` and `codec_set=marc-cli-v53`, verify it, then traverse the
...
For the six 67,108,864-byte streams, fix SHA-256 as follows:
...
Require the same byte stream and digest for chunk capacities 1, 7, 1,024,
4,097, and whole-input on a smaller 12,345-byte vector. Check explicit prefixes
at periodic wrap, shared-record boundaries, collision-unit wrap, phase-block
...
Private short-match token-validation gate:

- First require the new length-three parameters and `Literal('a'), Match(1,3)`
  frame to fail against the old validator. Add variant-7-only validation and
  reconstruct the four raw bytes `aaaa` by overlap copy without touching a
  trailing output sentinel.
- Reject minimum lengths other than three, maximum below three or above 258,
  window above 65,536, nonzero flags, and stricter caller hard limits.
  Reject too-large raw frames, references beyond reconstructed bytes, and
  length two without changing the caller's output-size result.
- Retain the old validator's rejection of length-three parameters and assert
  that the public typed-context parser still rejects both individual reserved
  IDs and the complete `2/7 + 1/6` pair before publication. No archive is
  generated by this stage.

### TVG-1049

Private short-match layout and semantic preflight checks:

- Assert the old count 31 and 4,518 entries remain fixed; require the new
  alphabets `2,256,9,17` over context groups `0..2`, `3..19`, `20..22`,
  `23..31`, with offsets 4,358 at context 20, 4,385 at context 23, and
  4,538 at the end.
- For the reserved `aaaa` two-token vector, require a seven-byte payload,
  87-byte serialized frame, and checked aggregate of frame, token array,
  four raw bytes, and the 32-context Range model.
- Reject crossed variants, model count 31, bad minimum length, overlarge
  frame, excess events/decisions/payload, descriptor mismatch, sequence or
  raw partition mismatch, and insufficient aggregate workspace without
  changing caller requirements. At the 65,536-byte frame boundary, admit
