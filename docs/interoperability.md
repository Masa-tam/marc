...
Each current schema-10 bundle contains the same generated `input.bin`, one
archive for every public CLI profile, and `manifest.json`. The manifest declares
codec set `marc-cli-v10` and records
...
2. decodes all twenty-one foreign archives and compares their output byte for byte
   with `input.bin`;
...
result: Verified 21 archives from windows-msvc-x64 (...), revision <Git object ID>
```
...
archives, appending `lz78-adaptive-huffman` to the frozen schema-9 order. No
