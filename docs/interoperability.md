...
Each current schema-23 bundle contains the same generated `input.bin`, one
archive for every public CLI profile, and `manifest.json`. The manifest declares
codec set `marc-cli-v23` and records
...
2. decodes all thirty-four foreign archives and compares their output byte
...
result: Verified 34 archives from windows-msvc-x64 (...), revision <Git object ID>
```
...
order. No schema silently inherits profiles added by a later schema.
...
Schema 23 has local generation, exact-order verification, byte-identical
re-encoding, reordered-manifest rejection, and schemas 1 through 22
compatibility evidence. External schema-23 verification remains pending.
Schema 22's completed Windows/MSVC, Ubuntu 24.04/Ninja, and Ubuntu
