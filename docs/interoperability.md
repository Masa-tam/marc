...
Each current schema-51 bundle contains the same generated `input.bin`, the
frozen 42 stable-profile archives, nineteen experimental Format 2 archives,
and `manifest.json`. The manifest declares codec set `marc-cli-v51` and records
...
2. decodes all sixty-one foreign archives and compares their output byte
...
result: Verified 61 archives from windows-msvc-x64 (...), revision <Git object ID>
```
...
Schema 51 has local generation, exact-order verification, byte-identical
re-encoding, reordered-manifest rejection, and schemas 1 through 50
compatibility evidence. Schema 50 has complete Windows/MSVC, Ubuntu
24.04/Ninja, and Ubuntu 26.04/Clang four-direction evidence; schema 51 external
