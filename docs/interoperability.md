...
Each current schema-27 bundle contains the same generated `input.bin`, one
archive for every public CLI profile, and `manifest.json`. The manifest declares
codec set `marc-cli-v27` and records
...
2. decodes all thirty-eight foreign archives and compares their output byte
...
result: Verified 38 archives from windows-msvc-x64 (...), revision <Git object ID>
```
...
to the frozen schema-26 order. No schema
...
Schema 27 has local generation, exact-order verification, byte-identical
re-encoding, reordered-manifest rejection, and schemas 1 through 26
compatibility evidence. External schema-27 exchange remains pending. The
completed schema-26 Windows/MSVC, Ubuntu 24.04/Ninja, and Ubuntu
