...
Each current schema-19 bundle contains the same generated `input.bin`, one
archive for every public CLI profile, and `manifest.json`. The manifest declares
codec set `marc-cli-v19` and records
...
2. decodes all thirty foreign archives and compares their output byte
...
result: Verified 30 archives from windows-msvc-x64 (...), revision <Git object ID>
```
...
archives, appending `lzmw-dynamic-range` to the frozen schema-18 order. No
...
Schema 19 has local generation, exact-order verification, byte-identical
re-encoding, reordered-manifest rejection, and schemas 1 through 18
