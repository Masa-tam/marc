...
valid 112-byte stream header and both strict public C streaming profiles for
every case. The supplied input is capped above the selected 9,089-byte maximum
descriptor so complete frames remain reachable. Serialized staging, the fixed
126,976-entry decode tables, typed tokens, private raw staging, public output,
and process calls all have predetermined ceilings. The 1 MiB identity and
distance hard limit do not enlarge the 1 KiB raw-frame or token arrays.
...
violations. Deterministic regressions cover both canonical profiles and their
reciprocal strict rejection. Compile-smoke remains distinct from a separately
bounded sanitizer campaign.
...
The Contextual Adaptive Huffman streaming encoder now selects HashChain Exact
at the same typed-token boundary as its proven private frame route. Its opaque
encoder views retain token staging and fixed node and symbol banks, then append
separately aligned finder storage; profile calculation charges the exact
largest-frame extent, and the C factory passes every span without exposing its
representation. Constructor and process-time checks keep raw, token, node,
symbol, finder, serialized, and caller-output regions disjoint. Short finder
...
The CLI boundary now exposes the explicit experimental selector
...
The benchmark boundary mirrors those two exact CLI names without adding
...
The interoperability boundary freezes schema 39's 49 archives and
...
The interoperability boundary freezes schema 40's exact 50-archive
