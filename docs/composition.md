...
cell is a currently published CLI and C ABI profile. `Profile` means the exact
known-size and incremental codecs plus an internal normalized configuration
and workspace contract exist in both directions, but the C ABI, CLI, and
release evidence do not. `Candidate` means both components exist and meet at
...
| LZSS | `lzss` | Profile | Candidate | Candidate | Candidate | Candidate |
