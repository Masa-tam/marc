...
The table shows every baseline byte-stream dictionary/entropy pairing. Every
name in backticks is currently published through both the CLI and C ABI. The
`None` row contains the raw-checksum profile and standalone entropy codecs; the
remaining rows contain each standalone dictionary codec and all thirty
dictionary/entropy compositions.
...
If revisited, a generator could reduce repetition once the profile semantics
are represented declaratively. Suitable generated outputs would include:
...
A safe adoption experiment would be:
...
No future candidate cell is a release commitment. This deferred path records
an architectural possibility rather than an active baseline requirement.

## Profile admission history

These records preserve the staged implementation and publication history in
Git introduction order. Intermediate wording such as `Specified`, `Candidate`,
or `reserved` describes the state at that point; the current matrix above is
authoritative for present availability.
...
Its bounded fuzz boundary drives both private complete-frame reconstruction
...
caller-owned regions while keeping every typed layout private. Its public-ABI
completion matrix now proves
...
The experimental `lzss-contextual-dynamic-range` composition now has a
dependency-free benchmark adapter over its public C lifecycle. It verifies a
complete byte-exact round trip before timing and reports ratio, directional
...
Interoperability schema 55 appends `lzss-contextual-tans-64m` once after the
frozen 64-entry schema-54 order. The generator requires identity
`2/6 + 1/5 + 5/2` and an immediate round trip; the verifier requires exact
65-entry order, foreign decode equality, and byte-identical local re-encoding.
