...
Contextual tANS now routes its public streaming encoder through the byte-
identical HashChain Exact typed-token producer. Its opaque encoder views retain
the fixed encode tables and append the exact largest-frame finder workspace,
while the decoder, stream identity, CLI selector, and interoperability schema
remain unchanged. Direct profile and streaming tests prove bounded layout and
...
Byte-oriented LZSS plus rANS now selects HashChain Exact through its public
...
in secondary storage and aggregate limits. Twelve focused profile and
streaming tests plus the public C lifecycle pass under MSVC and ClangCL; the
4,326-byte README retains its 3,734-byte stream. CLI and benchmark aggregate
bounds include finder storage. Full-suite admission is established by all
...
Interoperability schema 44 appends only `lzss-contextual-rans-4m` as archive
54 and freezes every schema-43 archive byte and order. Generation verifies
exact `2/4 + 1/3 + 4/3` identity and an immediate round trip. Both local
compilers generate and verify all 54 archives, reject reordered current
manifests, remove only the new final entry to recover schema 43, and verify
every schema through version 1 in 79.34 and 77.11 seconds. Complete registered-
suite validation passes all 3,172 tests under MSVC and ClangCL in 189.68 and
...
Schema 47 now has complete four-direction external evidence at revision
`c107daf9be319523402b0a4b4d9089c46a702ced`. All 57 archives from the
Windows/MSVC, Ubuntu 24.04/Ninja, and Ubuntu 26.04/Clang producers decode and
re-encode byte-identically across the tested Windows and WSL2 Linux x86-64
paths. This completes the cross-platform admission required for the four-MiB
Contextual Adaptive Huffman profile.
...
Private complete-frame planning and encoding now admit exact Contextual tANS
identity `2/6 + 1/5 + 5/2` through HashChain Exact and BinaryTree Exact. The
two routes produce one byte-identical canonical frame which decodes to the
...
The 64-MiB Contextual Blocked Huffman model/operation and direct typed-token
paths are proven through the existing immutable layout plumbing. Class-26
LSB-first bytes and both first-new and maximum distances round trip without
raw-history storage. Local hard limits and crossed-profile publication
checks remain enforced. Complete-frame and later public gates stay closed.

### BR-0251

Bounded Contextual Blocked Huffman frame validation and decoding now admit
64 MiB. Exact descriptor, decision, and payload ceilings and first-new-distance
decoding are covered. Frame encoding remains explicitly closed pending its
separate tests; no public profile or schema-55 archive has been added.

### BR-0252

64-MiB Contextual Blocked Huffman Exact finder frame encoding is admitted.
HashChain and BinaryTree yield identical decodable bytes, and exact/one-short
workspace and output checks pass. Exhaustive remains closed. Long-distance
encoder coverage and profile/streaming admission remain subsequent work;
public APIs, applications, and schema 55 are unchanged.

### BR-0253

The 64-MiB Contextual Blocked Huffman frame encoder has long-distance coverage
...
The bounded dual-path Contextual Blocked Huffman decoder fuzzer admits 64M
without profile-sized allocation. Fixed-storage public streaming and private
frame decode paths, deterministic malformed regressions, all reciprocal
profile crossings, and a 1,000-run ASan/UBSan campaign passed. Interoperability
schema admission remains pending; schema 55 is unchanged.

### BR-0258

Interoperability schema 56 freezes all 65 schema-55 archives and appends only
`lzss-contextual-blocked-huffman-64m` as archive 66. Generation validates exact
identity `2/6 + 1/5 + 2/2` and immediate round trip. Verification enforces
exact order, size, SHA-256, foreign decode equality, and byte-identical local
re-encoding; removing only archive 66 reconstructs schema 55 before the
unchanged chain through schema 1. No codec byte, ABI, profile resource,
default, or finder selection changes. External cross-platform exchange remains
pending.

### BR-0259

Schema 56 now has complete four-direction external evidence at revision
`c6bb7a62c7bfdcf6eef157ae1642f5de9576c182`. Ubuntu 26.04/Clang 21.1.8
verified all 66 archives from the Windows/MSVC and Ubuntu 24.04/Ninja CI
artifacts, generated and self-verified its own 66-archive bundle, and supplied
that bundle for reverse verification under Windows/MSVC. Every pass reported
the same full revision and exact manifest, digest, decode, and byte-identical
