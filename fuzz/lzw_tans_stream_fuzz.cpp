...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    ...
    const std::span<const std::byte,
        ...
        parameter_bytes{
            input.data() + marc::frame::stream_header_size,
            marc::dictionary::internal::lzw_parameter_size};
    if (marc::dictionary::internal::parse_lzw_parameters(
    ...
    static_cast<void>(marc::frame::decode_lzw_tans_frame_to_staging(
        stream, parameters, limits, 0, 0,
        input.subspan(marc::frame::lzw_tans_stream_prefix_size),
        views, dictionary, phrases, raw));
}

void exercise_public_streaming(
    const std::span<const std::byte> input) noexcept {
    marc_lzw_tans_config config{};
    if (marc_lzw_tans_config_init(MARC_DIRECTION_DECODE, &config)
        != MARC_STATUS_OK) {
        std::abort();
    }
    ...
    config.max_dictionary_entries = maximum_dictionary_entries;
    config.max_blocks_per_frame = maximum_blocks;

    marc_workspace_requirements requirements{};
    if (marc_lzw_tans_workspace_requirements(&config, &requirements)
            != MARC_STATUS_OK
