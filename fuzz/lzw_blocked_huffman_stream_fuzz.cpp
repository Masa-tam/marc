...
using BlockView = marc::entropy::internal::BlockedHuffmanBlockView;
...
constexpr std::size_t maximum_encoded_frame = marc::frame::frame_header_size
    + maximum_blocks * 16 + maximum_payload;
constexpr std::size_t maximum_internal = maximum_encoded_frame
    + maximum_dictionary + maximum_frame
    + maximum_blocks * sizeof(BlockView)
...
void exercise_streaming(const std::span<const std::byte> input) noexcept {
    ...
    std::array<BlockView, maximum_blocks> views{};
    std::array<Phrase, maximum_phrases> phrases{};
    std::array<std::byte, maximum_total_output> output{};
    marc::frame::LzwBlockedHuffmanFrameStreamingDecoder decoder{
