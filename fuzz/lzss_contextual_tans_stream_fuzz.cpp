...
constexpr std::size_t maximum_views_alignment =
    std::max(alignof(TansDecodeEntry), alignof(Token));
constexpr std::size_t maximum_views = table_entries * sizeof(TansDecodeEntry)
...
struct FuzzWorkspace {
    std::array<TansDecodeEntry, table_entries> private_tables{};
...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    ...
    const marc::frame::internal::LzssContextualTansFrameValidationContext
...
void exercise_public_streaming(
    ...
    if (marc_lzss_contextual_tans_create(
            &config,
