...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    if (input.size() < marc::frame::lzw_rans_stream_prefix_size) {
        return;
    }
    ...
    static_cast<void>(marc::frame::decode_lzw_rans_frame_to_staging(
        stream, parameters, limits, 0, 0,
        input.subspan(marc::frame::lzw_rans_stream_prefix_size),
...
void exercise_public_streaming(
    ...
    if (marc_lzw_rans_config_init(MARC_DIRECTION_DECODE, &config)
        != MARC_STATUS_OK) {
        std::abort();
    }
    ...
    if (marc_lzw_rans_workspace_requirements(&config, &requirements)
            != MARC_STATUS_OK
