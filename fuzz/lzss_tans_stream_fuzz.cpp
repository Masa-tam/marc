...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    if (input.size() < marc::frame::lzss_tans_stream_prefix_size) {
        return;
    }
    ...
    static_cast<void>(marc::frame::decode_lzss_tans_frame_to_staging(
        stream, parameters, limits, 0, 0,
        input.subspan(marc::frame::lzss_tans_stream_prefix_size),
