...
constexpr std::uint64_t maximum_model_total = UINT64_C(1) << 15;
...
[[nodiscard]] marc::core::DecoderLimits fuzz_limits() noexcept {
    ...
    limits.max_range_model_total = maximum_model_total;
...
void exercise_streaming(const std::span<const std::byte> input) noexcept {
    ...
    marc::frame::DynamicRangeFrameStreamingDecoder decoder{
        fuzz_limits(), encoded_frame, decoded_frame};
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data,
    ...
    static_cast<void>(marc::frame::decode_dynamic_range_stream(
        input, fuzz_limits(), output, stream));
