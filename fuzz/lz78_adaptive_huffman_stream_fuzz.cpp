...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    if (input.size()
        < marc::frame::lz78_adaptive_huffman_stream_prefix_size) {
        return;
    }
    ...
    static_cast<void>(
        marc::frame::decode_lz78_adaptive_huffman_frame_to_staging(
            stream, parameters, limits, 0, 0,
            input.subspan(
                marc::frame::lz78_adaptive_huffman_stream_prefix_size),
            dictionary, phrases, raw));
}
...
void exercise_streaming(const std::span<const std::byte> input) noexcept {
    ...
    marc::frame::Lz78AdaptiveHuffmanFrameStreamingDecoder decoder{
        fuzz_limits(), encoded_frame, dictionary, decoded_frame, phrases};
