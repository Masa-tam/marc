...
constexpr std::size_t maximum_views_alignment =
    std::max(alignof(Table), alignof(Token));
constexpr std::size_t maximum_views = table_count * sizeof(Table)
...
struct FuzzWorkspace {
    std::array<Table, table_count> private_tables{};
...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    ...
    marc::frame::internal::LzssContextualBlockedHuffmanStreamHeader stream{};
    std::size_t consumed{};
...
void exercise_public_streaming(
    ...
    if (marc_lzss_contextual_blocked_huffman_workspace_requirements(
    ...
    if (marc_lzss_contextual_blocked_huffman_create(
            &config,
