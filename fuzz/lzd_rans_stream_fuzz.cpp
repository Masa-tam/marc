...
constexpr std::size_t maximum_phrases = maximum_frame / 2;
...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    if (input.size() < marc::frame::lzd_rans_stream_prefix_size) {
        return;
    }
    ...
    const std::span<const std::byte,
        ...
        parameter_bytes{
            input.data() + marc::frame::stream_header_size,
            marc::dictionary::internal::lzd_parameter_size};
    if (marc::dictionary::internal::parse_lzd_parameters(
    ...
    static_cast<void>(marc::frame::decode_lzd_rans_frame_to_staging(
        stream, parameters, limits, 0, 0,
        input.subspan(marc::frame::lzd_rans_stream_prefix_size),
...
void exercise_public_streaming(
    ...
    if (marc_lzd_rans_config_init(MARC_DIRECTION_DECODE, &config)
        != MARC_STATUS_OK) {
        std::abort();
    }
    ...
    if (marc_lzd_rans_workspace_requirements(&config, &requirements)
            != MARC_STATUS_OK
