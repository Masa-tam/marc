...
constexpr std::size_t maximum_payload = 8192;
constexpr std::size_t maximum_dictionary = maximum_frame * 2;
...
constexpr std::size_t maximum_views = maximum_blocks * sizeof(View);
...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    if (input.size() < marc::frame::lzss_rans_stream_prefix_size) {
        return;
    }
    ...
    const std::span<const std::byte,
        ...
        parameter_bytes{
            input.data() + marc::frame::stream_header_size,
            marc::dictionary::internal::lzss_parameter_size};
    if (marc::dictionary::internal::parse_lzss_parameters(
    ...
    static_cast<void>(marc::frame::decode_lzss_rans_frame_to_staging(
        stream, parameters, limits, 0, 0,
        input.subspan(marc::frame::lzss_rans_stream_prefix_size),
        views, dictionary, raw));
}

void exercise_public_streaming(
    const std::span<const std::byte> input) noexcept {
    marc_lzss_rans_config config{};
    if (marc_lzss_rans_config_init(MARC_DIRECTION_DECODE, &config)
        != MARC_STATUS_OK) {
        std::abort();
    }
    ...
    config.max_lz_match_length = maximum_frame;

    marc_workspace_requirements requirements{};
    if (marc_lzss_rans_workspace_requirements(&config, &requirements)
            != MARC_STATUS_OK
    ...
    alignas(View) std::array<std::uint8_t, maximum_views> views{};
    marc_transform* decoder{};
    if (marc_lzss_rans_create(
            &config,
