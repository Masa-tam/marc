...
constexpr std::size_t maximum_encoded_frame =
    ...
    + maximum_payload;
constexpr std::size_t maximum_dictionary = maximum_frame * 16;
...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    if (input.size() < marc::frame::lz77_dynamic_range_stream_prefix_size) {
        return;
    }
    ...
    const std::span<const std::byte,
        ...
        parameter_bytes{
            input.data() + marc::frame::stream_header_size,
            marc::dictionary::internal::lz77_parameter_size};
    if (marc::dictionary::internal::parse_lz77_parameters(
    ...
    static_cast<void>(marc::frame::decode_lz77_dynamic_range_frame_to_staging(
        stream, parameters, limits, 0, 0,
        input.subspan(marc::frame::lz77_dynamic_range_stream_prefix_size),
        dictionary, raw));
}
...
void exercise_streaming(const std::span<const std::byte> input) noexcept {
    ...
    marc::frame::Lz77DynamicRangeFrameStreamingDecoder decoder{
