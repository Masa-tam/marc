...
constexpr std::size_t maximum_dictionary_entries =
    maximum_payload / marc::dictionary::internal::lz78_token_size;
...
[[nodiscard]] marc::core::DecoderLimits fuzz_limits() noexcept {
    ...
    limits.max_dictionary_entries = UINT32_C(1) << 16;
...
void exercise_streaming(const std::span<const std::byte> input) noexcept {
    ...
    std::array<marc::dictionary::internal::Lz78PhraseEntry,
               maximum_dictionary_entries>
        dictionary{};
    std::array<std::byte, maximum_total_output> output{};
    marc::frame::Lz78FrameStreamingDecoder decoder{
        fuzz_limits(), encoded_frame, decoded_frame, dictionary};
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data,
    ...
    static_cast<void>(marc::frame::decode_lz78_stream(
        input, fuzz_limits(), dictionary, output, stream, parameters));
