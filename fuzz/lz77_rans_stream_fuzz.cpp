...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    if (input.size() < marc::frame::lz77_rans_stream_prefix_size) {
        return;
    }
    ...
    static_cast<void>(marc::frame::decode_lz77_rans_frame_to_staging(
        stream, parameters, limits, 0, 0,
        input.subspan(marc::frame::lz77_rans_stream_prefix_size),
