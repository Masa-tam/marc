...
constexpr std::size_t maximum_block = 256;
constexpr std::size_t maximum_payload = 4096;
constexpr std::size_t maximum_internal = 8192;
constexpr std::size_t maximum_blocks = 8;
constexpr std::size_t maximum_table_entries = 512;
constexpr std::uint32_t maximum_code_length = 24;
...
[[nodiscard]] marc::core::DecoderLimits fuzz_limits() noexcept {
    ...
    limits.max_block_size = maximum_block;
    limits.max_compressed_payload_size = maximum_payload;
    limits.max_dictionary_serialized_size = maximum_frame;
    limits.max_huffman_code_length = maximum_code_length;
    limits.max_entropy_table_entries = maximum_table_entries;
...
void exercise_streaming(const std::span<const std::byte> input) noexcept {
    ...
    marc::frame::BlockedHuffmanFrameStreamingDecoder decoder{
        fuzz_limits(), encoded_frame, decoded_frame, views};
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data,
    ...
    static_cast<void>(marc::frame::decode_blocked_huffman_stream(
        input, fuzz_limits(), views, output, stream));
