...
constexpr std::size_t maximum_fuzz_input = 65536;
...
constexpr std::size_t maximum_views = node_count * sizeof(Node)
    + symbol_count * sizeof(std::uint16_t)
    + maximum_frame * sizeof(Token)
    + 2 * (maximum_views_alignment - 1);
...
struct FuzzWorkspace {
    std::array<Node, node_count> private_nodes{};
    std::array<std::uint16_t, symbol_count> private_symbols{};
...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    if (input.size() < marc::frame::internal::
            lzss_contextual_adaptive_huffman_stream_header_size) {
        return;
    }
    const auto limits = fuzz_limits();
    marc::frame::internal::LzssContextualAdaptiveHuffmanStreamHeader stream{};
    std::size_t consumed{};
    ...
    const marc::frame::internal::
        LzssContextualAdaptiveHuffmanFrameValidationContext context{
            stream, limits, 0, 0};
    static_cast<void>(marc::frame::internal::
        decode_lzss_contextual_adaptive_huffman_frame(
            input.subspan(consumed), context, workspace.private_nodes,
            workspace.private_symbols, workspace.private_tokens,
...
void exercise_public_streaming(
    ...
    if (marc_lzss_contextual_adaptive_huffman_workspace_requirements(
    ...
    if (marc_lzss_contextual_adaptive_huffman_create(
            &config,
