...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    if (input.size() < marc::frame::lzss_dynamic_range_stream_prefix_size) {
        return;
    }
    ...
    static_cast<void>(marc::frame::decode_lzss_dynamic_range_frame_to_staging(
        stream, parameters, limits, 0, 0,
        input.subspan(marc::frame::lzss_dynamic_range_stream_prefix_size),
        dictionary, raw));
}
...
void exercise_streaming(const std::span<const std::byte> input) noexcept {
    ...
    marc::frame::LzssDynamicRangeFrameStreamingDecoder decoder{
        fuzz_limits(), encoded_frame, dictionary, decoded_frame};
