...
void exercise_complete_frame(const std::span<const std::byte> input) noexcept {
    if (input.size()
        < marc::frame::lz77_adaptive_huffman_stream_prefix_size) {
        return;
    }
    ...
    static_cast<void>(
        marc::frame::decode_lz77_adaptive_huffman_frame_to_staging(
...
void exercise_streaming(const std::span<const std::byte> input) noexcept {
    ...
    marc::frame::Lz77AdaptiveHuffmanFrameStreamingDecoder decoder{
