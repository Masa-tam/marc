...
constexpr std::uint64_t lzss_contextual_adaptive_huffman_buffered_size =
    UINT64_C(8) << 20;
constexpr std::uint64_t lzss_contextual_adaptive_huffman_model_entries =
    UINT64_C(9067) + UINT64_C(4518);
...
struct CodecConfig {
    ...
    marc_lzss_contextual_rans_compact_config lzss_contextual_rans_compact{};
...
[[nodiscard]] const char* codec_name(const Codec codec) noexcept {
    ...
    if (codec == Codec::lzss_contextual_rans_compact)
        return "lzss-contextual-rans-compact";
...
[[nodiscard]] std::uint64_t payload_factor(const Codec codec) noexcept {
    ...
    if (codec == Codec::lzss_contextual_adaptive_huffman) return UINT64_C(34);
...
[[nodiscard]] bool configure(const Codec codec, const marc_direction direction,
    ...
    if (codec == Codec::checksum_raw) {
        ...
        maximum_buffered = lzss_contextual_dynamic_range_buffered_size;
        ...
        maximum_buffered = lzss_contextual_blocked_huffman_buffered_size;
    } else if (codec == Codec::lzss_contextual_adaptive_huffman) {
        maximum_buffered = lzss_contextual_adaptive_huffman_buffered_size;
    ...
    if (codec == Codec::checksum_raw) {
        ...
        if (marc_lzss_contextual_dynamic_range_config_init(
            ...
            return false;
        config.original_size = original_size;
        config.frame_size = static_cast<std::uint32_t>(
            lzss_contextual_dynamic_range_frame_size);
        config.max_frame_size = lzss_contextual_dynamic_range_frame_size;
        config.max_block_size = lzss_contextual_dynamic_range_frame_size;
        config.max_compressed_payload_size = maximum_payload;
        config.max_internal_buffered_bytes = maximum_buffered;
        config.max_lz_distance = UINT64_C(1) << 16;
        config.max_lz_match_length = 258;
        ...
        auto& config = result.lzss_contextual_rans_compact;
        if (marc_lzss_contextual_rans_compact_config_init(
                direction, &config) != MARC_STATUS_OK) {
            return false;
        }
        config.original_size = original_size;
        ...
        config.original_size = original_size;
        config.frame_size =
            static_cast<std::uint32_t>(lzss_contextual_tans_frame_size);
        config.max_frame_size = lzss_contextual_tans_frame_size;
        config.max_block_size = lzss_contextual_tans_frame_size * UINT64_C(6);
        config.max_compressed_payload_size = maximum_payload;
        config.max_internal_buffered_bytes = maximum_buffered;
        config.max_lz_distance = UINT64_C(1) << 16;
        config.max_lz_match_length = 258;
        ...
        config.frame_size = static_cast<std::uint32_t>(
            lzss_contextual_blocked_huffman_frame_size);
        config.max_frame_size = lzss_contextual_blocked_huffman_frame_size;
        ...
        config.frame_size = static_cast<std::uint32_t>(
            lzss_contextual_adaptive_huffman_frame_size);
        config.max_frame_size = lzss_contextual_adaptive_huffman_frame_size;
        config.max_block_size = lzss_contextual_adaptive_huffman_frame_size;
        config.max_compressed_payload_size = maximum_payload;
        config.max_internal_buffered_bytes = maximum_buffered;
        config.max_lz_distance = UINT64_C(1) << 16;
        config.max_lz_match_length = 258;
        config.max_entropy_table_entries =
            lzss_contextual_adaptive_huffman_model_entries;
    } else if (codec == Codec::lzss_rans) {
...
[[nodiscard]] marc_status query_workspace(
    ...
    if (config.codec == Codec::lzss_contextual_dynamic_range)
    ...
    if (config.codec == Codec::lzss_contextual_rans_compact)
        return marc_lzss_contextual_rans_compact_workspace_requirements(
            &config.lzss_contextual_rans_compact, &requirements);
...
[[nodiscard]] marc_status create_transform(
    ...
    if (config.codec == Codec::lzss_contextual_rans_compact)
        return marc_lzss_contextual_rans_compact_create(
            &config.lzss_contextual_rans_compact, primary, secondary, views,
            transform);
    if (config.codec == Codec::lzss_contextual_tans)
...
[[nodiscard]] bool maximum_encoded_size(const Codec codec,
    ...
    if (codec == Codec::lzss_contextual_dynamic_range) {
        ...
        if (input_size > (std::numeric_limits<std::size_t>::max()
            ...
            return false;
        const auto payload = input_size * payload_factor;
    ...
    if (codec == Codec::lzss_contextual_rans) {
        constexpr auto prefix_size = std::size_t{112};
        constexpr auto payload_factor = std::size_t{12};
        constexpr auto per_frame = std::size_t{9124};
        if (input_size > (std::numeric_limits<std::size_t>::max()
                          - prefix_size) / payload_factor)
            return false;
        const auto payload = input_size * payload_factor;
    ...
    if (codec == Codec::lzss_contextual_tans) {
        constexpr auto prefix_size = std::size_t{112};
        constexpr auto payload_factor = std::size_t{9};
        constexpr auto per_frame = std::size_t{9095};
        if (input_size > (std::numeric_limits<std::size_t>::max()
                          - prefix_size) / payload_factor) {
            return false;
        }
    ...
    if (codec == Codec::lzss_contextual_blocked_huffman) {
        constexpr auto prefix_size = std::size_t{112};
        constexpr auto payload_factor = std::size_t{12};
        constexpr auto per_frame = std::size_t{2625};
        if (input_size > (std::numeric_limits<std::size_t>::max()
                          - prefix_size) / payload_factor) {
            return false;
        }
